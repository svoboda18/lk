...
int target_fdt_dram_memtest(void *fdt)
{
	...
	int i, j, nodeoffset, parentoffset, ret;
	dt_dram_info rsv_mem_reg_property[2] = {0};
	dt_size_info rsv_mem_size_property[2] = {0};
	...
	for (j = 0;j < orig_dram_info->rank_num; j++) {
		...
		if (j < (sizeof(memtest_reserved) / sizeof(*memtest_reserved))) {
			...
			sz = memtest_reserved[j] * 1024 * 1024;
		} else {
			break;
		}
...
#endif
#ifdef MTK_DRAMC_RSHMOO
	snprintf(name, sizeof(name), "mediatek,dram_memtest_info", j);
	nodeoffset = fdt_add_subnode(fdt, 0, name);
	if (nodeoffset < 0) {
		dprintf(CRITICAL, "Warning: can't add dram memtest info node in device tree\n", j);
		return -1;
	}
