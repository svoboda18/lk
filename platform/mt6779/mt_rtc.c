...
bool rtc_reset_bbpu_alarm_status(void)
{
	u16 bbpu;
	ulong begin = get_timer(0);

	bbpu = RTC_BBPU_KEY | RTC_BBPU_PWREN | RTC_BBPU_RESET_ALARM;
	RTC_Write(RTC_BBPU, bbpu);
	rtc_write_trigger();

	while ((RTC_Read(RTC_BBPU) & RTC_BBPU_RESET_ALARM))
	{
		if (get_timer(begin) > 500)
		{
			DBG_RTC_I("%s time out\n", __func__);
			return false;
		}
	}
...
bool rtc_boot_check(bool can_alarm_boot)
{
	...
	if ((pdn1 & RTC_PDN1_RECOVERY_MASK) == RTC_PDN1_FAC_RESET) {    /* factory data reset */
		...
		rtc_write_trigger();
		DBG_RTC_I("%s:factory data reset\n", __func__);
		return true;
	}
	...
	if (pdn1 & RTC_PDN1_BYPASS_PWR) {   /* bypass power key detection */
		...
		rtc_write_trigger();
		DBG_RTC_I("%s:bypass power key detection, boot_reason=%d\n", __func__, g_boot_arg->boot_reason);
		return true;
	}
