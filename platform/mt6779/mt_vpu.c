...
int mt_load_vpu(void)
{
	...
	nodeoffset = fdt_node_offset_by_compatible(kernel_fdt, -1, "mediatek,vpu_core0");  // get vpu node offset
	dprintf(INFO, "[%s] nodeoffset %d\n", MODULE_NAME, nodeoffset);
	if (nodeoffset < 0) { // node add sub-node to /
		int offset;
		offset = fdt_path_offset(kernel_fdt, "/");
		...
		if (offset < 0) {
			...
			nodeoffset = fdt_add_subnode(kernel_fdt, offset, "vpu_core0");
			dprintf(INFO, "[%s] nodeoffset %d\n", MODULE_NAME, nodeoffset);
			if (nodeoffset < 0) {
				ret = -EINVAL;
				...
				fdt_setprop_string(kernel_fdt, nodeoffset, "compatible", "mediatek,vpu_core0");
			}
		}
	}

	value = cpu_to_fdt32(dstBase);
	fdt_setprop(kernel_fdt, nodeoffset, "bin-phy-addr", (char *)&value, sizeof(u32));
	size = cpu_to_fdt32(VPU_DRAM_SIZE);
	fdt_setprop(kernel_fdt, nodeoffset, "bin-size", (char *)&size, sizeof(u32));
#endif
