...
{0, 0, SMEM_MD_PADDING_SIZE,	 SMEM_USER_ALIGN_PADDING},
...
static void set_gear_id_list()
{
	...
	ret = set_env("md1_ccb_gear_list",
		"1(2,20);2(2,10);3(0,0);4(2,30);11(2,2)");
...
static int cal_share_mem_layout(int load_flag)
{
	...
	ALWAYS_LOG("env[md1_ccb_cap_gear]%x.\n", md1_ccb_cap_gear);
	/* MD Share memory layout */
	...
	for (i = 0; i < sizeof(csmem_layout)/sizeof(csmem_item_t); i++) {
		...
		if ((csmem_layout[i].item_cnt == SMEM_USER_RAW_MD_CONSYS)
			...
			!= consys_size_at_img)
			&& ((consys_size_at_img == SMEM_MD_CONSYS_SIZE)
			|| (consys_size_at_img == 0))) {
				csmem_layout[i].csmem_buffer_size
						= consys_size_at_img;
		}
