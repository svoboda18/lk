...
static int brightness_set_pmic(enum mt65xx_led_pmic pmic_type, enum led_brightness level)
{
	LEDS_INFO("[LEDS]LK: brightness set by pmic does not support\n");
...
struct cust_mt65xx_led *get_cust_led_dtsi(void)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		offset = fdt_path_offset(lk_drv_fdt, leds_node[i]);
		if (offset < 0) {
			...
			isDTinited = true;
			pled_dtsi[i].mode = led_fdt_getprop_u32(lk_drv_fdt, offset, "led_mode");
			pled_dtsi[i].data = led_fdt_getprop_u32(lk_drv_fdt, offset, "data");
			led_fdt_getprop_char_array(lk_drv_fdt, offset, "pwm_config", (char *)pwm_config);
