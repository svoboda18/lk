...
extern unsigned int crypto_hw_engine_disable(void);
extern void msdc_spm_hw_res_req_en(void);
...
void init_storage(void)
{
	...
	if ((DRV_Reg32(GPIO_BANK) & TRAP_UFS_FIRST) == 0)
		...
		mmc_legacy_init(1);
	#endif
	/* Always enable MSDC SPM REQ bits, otherwise SPM will see msdc signal forever busy */
	msdc_spm_hw_res_req_en();
#else
#ifndef MACH_FPGA
	nand_init();
	nand_driver_test();
...
#if (defined(MTK_UFS_SUPPORT) && defined(MTK_UFS_AWP))
	if ((DRV_Reg32(GPIO_BANK) & TRAP_UFS_FIRST) == 0) {
...
{
	bool bearly_backlight_on = false, backlight_already_on = false;
	int logo_size;
...
#ifdef MTK_KERNEL_POWER_OFF_CHARGING
	else if (g_boot_mode != META_BOOT && g_boot_mode != ALARM_BOOT && g_boot_mode != FASTBOOT && g_boot_mode != KERNEL_POWER_OFF_CHARGING_BOOT && g_boot_mode != LOW_POWER_OFF_CHARGING_BOOT && mtk_bat_allow_backlight_enable()) {
#ifndef MACH_FPGA_NO_DISPLAY
		PROFILING_START("show logo");
		mt_disp_show_boot_logo();
		PROFILING_END();
