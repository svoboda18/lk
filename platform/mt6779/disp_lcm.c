...
static LCM_PARAMS _disp_lcm_params[MAX_LCM_NUMBER];
...
disp_path_handle _display_interface_path_init(disp_lcm_handle *plcm)
{
	...
	memcpy(&(data_config.round_corner_params), &(plcm->params->round_corner_params), sizeof(LCM_ROUND_CORNER));
#endif
...
void _dump_lcm_info(disp_lcm_handle *plcm)
{
	...
	if (l && p) {
		...
		DISPCHECK("[LCM][RC] en:%d, w:%d, h:%d\n", p->round_corner_params.round_corner_en, p->round_corner_params.w, p->round_corner_params.h);
		DISPCHECK("[LCM][RC] size:%d, addr:0x%p\n", p->round_corner_params.tp_size, p->round_corner_params.lt_addr);
#endif
...
int check_lcm_node_from_DT(void)
{
	...
	offset = fdt_node_offset_by_compatible(lk_drv_fdt, -1, lcm_node);
...
disp_lcm_handle* disp_ext_lcm_probe(char* plcm_name, LCM_INTERFACE_ID lcm_id)
{
	DISPFUNC();
	...
	if (_lcm_count() < 2) {
		DISPERR("no second lcm driver defined in linux kernel driver\n");
		return NULL;
				...
				if (lcm_id == LCM_INTERFACE_NOTDEFINED ||(lcm_id != LCM_INTERFACE_NOTDEFINED && plcm->lcm_if_id == lcm_id)) {
					...
					if (lcm_drv->compare_id != NULL) {
						if (lcm_drv->compare_id() != 0) {
							isLCMFound = true;
							_display_interface_path_deinit(handle);
							dprintf(CRITICAL, "we will use lcm: %s\n", lcm_drv->name);
							break;
						}
					}
	...
	plcm = &_disp_lcm_driver[1];
	lcm_param = &_disp_lcm_params[1];
	if (plcm && lcm_param) {
		plcm->params = lcm_param;
		plcm->drv = lcm_drv;
	} else {
...
int disp_lcm_update(disp_lcm_handle *plcm, int x, int y, int w, int h, int force)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (disp_lcm_is_video_mode(plcm)) {
			/*  Caution! process LCM update callback is for
			 *  LCM_SET_DISPLAY_ON_DELAY improve LK boot time.
			 *  please remove VDO mode's LCM driver update callback
			 *  if unnecessary.
			 */
			if (lcm_drv->update)
