...
int verify_load_adsp_image(char *part_name, void *addr)
{
	...
	if (sys_prog_size != ADSP_ITCM_SIZE) {
		dprintf(CRITICAL, "[ADSP] load_adsp_image fail %s(addr=0x%x, size=0x%x)\n", IRAM_NAME_ADSP_A, (uint32_t)sys_prog_ptr, sys_prog_size);
		return -1;
	} else {
		dprintf(INFO, "[ADSP] load_adsp_image success %s(0x%x)\n", IRAM_NAME_ADSP_A, (uint32_t)sys_prog_ptr);
	}
	...
	if (sys_data_size != ADSP_DTCM_SIZE) {
		dprintf(CRITICAL, "[ADSP] load_adsp_image fail %s(addr=0x%x, size=0x%x)\n", DRAM_NAME_ADSP_A, (uint32_t)sys_data_ptr, sys_data_size);
		return -1;
	} else {
		dprintf(INFO, "[ADSP] load_adsp_image success %s(0x%x)\n", DRAM_NAME_ADSP_A, (uint32_t)sys_data_ptr);
	}
