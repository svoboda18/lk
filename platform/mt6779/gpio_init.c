...
int mt_gpio_get_default_chip_from_dtb(void)
{
	...
	off = fdt_path_offset(lk_drv_fdt, GPIO_DT_NODE_NAME);
	dprintf(INFO, "[GPIO] Found " GPIO_DT_NODE_NAME "at offset %d\n", off);

	/* if default name is not found, retry for common kernel */
	if (off < 0) {
		off = fdt_path_offset(lk_drv_fdt, GPIO_DT_NODE_NAME_SOC);
		dprintf(INFO, "[GPIO] Found " GPIO_DT_NODE_NAME_SOC "at offset %d\n", off);

		/* fatal return if fail again */
		if (off < 0) {
			dprintf(CRITICAL, "[GPIO] Failed to find " GPIO_DT_NODE_NAME_SOC " in dtb\n");
			return -1;
		}
	}
	...
	dprintf(INFO, "[GPIO] Found property at %p, len %d\n", data, len);
