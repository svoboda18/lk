...
#if defined(MMC_MSDC_DRV_LK)
/* buffer needs to align to cpu cache line size */
/*
 * The varibiles and DMA buffer shared with the same
 * cpu cache line may cause unpredictable behavier.
 */
#define RPMB_BUF_ALIGN 64
...
int mmc_rpmb_read_data(u32 blkAddr, u32 *buf, u32 bufLen)
{
	...
	rpmb_frame = memalign(RPMB_BUF_ALIGN, RPMB_SZ_DATA * 2);
	if (!rpmb_frame) {
		msdc_pr_err("%s, malloc failed\n", __func__);
		ret = -1;
		goto out;
	}
...
int mmc_rpmb_write_data(u32 blkAddr, u32 *buf, u32 bufLen)
{
	...
	/* mmc_rpmb_get_wc() will Switch to RPMB partition */
