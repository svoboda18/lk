...
int mmc_switch_security(struct mmc_host *host, struct mmc_card *card,
...
int mmc_set_boot_wp(struct mmc_card *card, STORAGE_WP_TYPE type, Region partition)
{
	...
	value = ext_csd[EXT_CSD_PART_CFG] & 0x7;
	if (value != EXT_CSD_PART_CFG_DEFT_PART) {
		value = (ext_csd[EXT_CSD_PART_CFG] & ~0x7) | EXT_CSD_PART_CFG_DEFT_PART;
		err = mmc_set_part_config(card, value);
		if (err) {
...
int mmc_set_user_wp(struct mmc_card *card, STORAGE_WP_TYPE type, unsigned long blknr, u32 blkcnt)
{
	int err = MMC_ERR_FAILED;
	u8 *ext_csd = &card->raw_ext_csd[0];
	u8 value;
	...
	if (card->ext_csd.usr_wp & US_PERM_WP_EN) {
		...
		err = mmc_switch_security(card->host, card, EXT_CSD_CMD_SET_NORMAL,
