...
#define MPU_REGION_ID_MD_MTEE           22
...
static const mpu_att_t mpu_att_default[MPU_REGION_ID_TOTAL_NUM][MPU_MDOMAIN_ID_TOTAL_NUM] = {
	...
	/* 7*/{}, /*Trusted UI*/
	/* 8*/{}, /*MD-Dynamic*/
	/* 9*/{}, /*Security CCCI share memory(MD0 Secure Share)*/
	/*10*/{	SEC_RW,	NO_PROTECTION, 	[2 ... 15] = FORBIDDEN},
	...
	/*18*/{ SEC_R_NSEC_R,	FORBIDDEN,    	[2 ... 15] = FORBIDDEN},
	/*19*/{ SEC_R_NSEC_R,	FORBIDDEN,    	[2 ... 15] = FORBIDDEN},
	/*20*/{ NO_PROTECTION,	FORBIDDEN,    	[2 ... 15] = FORBIDDEN},
	/*21*/{	NO_PROTECTION,	FORBIDDEN,    	[2 ... 15] = FORBIDDEN},
	/*22*/{	SEC_R_NSEC_R,	NO_PROTECTION,	[2 ... 6] = FORBIDDEN,	NO_PROTECTION,	[8 ... 15] = FORBIDDEN},
	/*23*/{ NO_PROTECTION,	NO_PROTECTION,  [2 ... 6] = FORBIDDEN,	NO_PROTECTION,	[8 ... 15] = FORBIDDEN},
	/*24*/{ NO_PROTECTION,	NO_PROTECTION,  FORBIDDEN, NO_PROTECTION, [4 ... 6] = FORBIDDEN,	NO_PROTECTION,
		[8 ... 15] = FORBIDDEN},
	/*25*/{}, /*WIFI EMI FW*/
	/*26*/{}, /*WMT*/
	/*27*/{}, /*Consys-MD share memory*/
	/*28*/{ NO_PROTECTION,	SEC_R_NSEC_R,	[2 ... 3] = FORBIDDEN,		NO_PROTECTION,	FORBIDDEN,	SEC_R_NSEC_RW,
	...
	unsigned int apc[EMI_MPU_DGROUP_NUM], int region, int lock)
{
	...
	ALWAYS_LOG("[%s] apc:%p region:%d lock:%d\n",
			__func__, apc, region, lock);
...
static int cal_share_mem_layout(int load_flag)
{
	...
	/* slot 6 */
		...
		if (g_boot_arg->gz_md_shm_pa && g_boot_arg->gz_md_shm_sz) {
			...
			if (insert_ccci_tag_inf("mtee_support", (char *)&mtee_support, sizeof(mtee_support)) < 0)
...
#if WITH_GZ_MD_SHAREMEM
		if (g_boot_arg->gz_md_shm_pa && g_boot_arg->gz_md_shm_sz) {
			md_bank4_remapping_by_slot(MD_SYS1, g_boot_arg->gz_md_shm_pa, 6);
		}
#endif
