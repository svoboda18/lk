...
#define MMC_ERR_WP_VIOLATION  13
#define MMC_ERR_ERASE_SEQ     14
...
#define EXT_CSD_CACHE_FLUSH             32  /* W/E_P */
#define EXT_CSD_CACHE_CONTROL           33  /* R/W/E_P */
...
#define EXT_CSD_CACHE_SIZE              249 /* R */
...
#define EXT_CSD_HS_TIMEING_HS           (1<<0) /* selecting high speed  interface timing */
#define EXT_CSD_HS_TIMEING_HS200        (1<<2) /* selecting hs200 interface timing */
#define EXT_CSD_HS_TIMEING_HS400        (1<<3) /* selecting hs400 interface timing */
...
#define EXT_CSD_BOOT_WP_WP_SEC_SEL      (1 << 1)
#define EXT_CSD_BOOT_WP_EN_PERM_WP      (1 << 2)
#define EXT_CSD_BOOT_WP_PERM_WP_SEC_SEL (1 << 3)
...
#define WP_NO    00  //not protect
#define WP_TEWP  01  //protected by temporary protection
#define WP_PWR   10  // protected by write protecion
#define WP_PSWP  11  // protected by permanent protection
...
struct mmc_host {
	...
	u32 base_top;     /* top_pad control base address */
	...
	u8 mmc_cache_enable;
	u8 mmc_cmdq_enable;
	u8 mmc_cmdq_qrdy_enable;
	u8 mmc_cmdq_first_task;
	u8 mmc_cmdq_last_task;
	u8 mmc_cmdq_queue_depth_managed;
	struct mmc_cmdq_task mmc_cmdq_req[MMC_MAX_QUEUE_DEPTH_USER]; //Only MMC_MAX_QUEUE_DEPTH_USER will be used
#endif
};
...
#ifdef FEATURE_MMC_CMDQ
	u8 cache_control_support;
	u8 cmdq_support;
	u8 cmdq_qrdy_support;
	u8 cmdq_queue_depth_supported;
#endif

	u8                      version;    /* the SD card version, 1.0, 2.0, or 3.0*/
	unsigned int            wp_size;   /* computed in ext_csd decode function */
};
...
typedef enum {
	...
	WP_POWER_ON,
	WP_PERMANENT_AWP,
	/* Important: do not change the enum above. They are used as assigned value. */
	WP_TEMPORARY,
	WP_DISABLE,
} STORAGE_WP_TYPE;
#endif
...
extern int mmc_init_host(struct mmc_host *host, int id, int clksrc, u32 mode);
...
#if defined(MMC_MSDC_DRV_PRELOADER) && defined(FEATURE_MMC_BOOT_ACCESS)
...
extern int mmc_switch_volt(struct mmc_host *host, struct mmc_card *card);
...
extern int mmc_init_sdio_card(struct mmc_host *host, struct mmc_card *card, u32 ocr);
...
extern int mmc_set_user_wp(struct mmc_card *card, STORAGE_WP_TYPE type, unsigned long blknr, u32 blkcnt);
extern int mmc_set_boot_wp(struct mmc_card *card, STORAGE_WP_TYPE type, Region partition);
#endif
