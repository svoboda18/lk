...
static ddp_path_handle_t g_handle[DDP_SCENARIO_MAX];
...
disp_path_handle dpmgr_create_path(DDP_SCENARIO_ENUM scenario, cmdqRecHandle cmdq_handle)
{
	...
	if (scenario == DDP_SCENARIO_PRIMARY_DISP)
		s = 0;
	else if (scenario == DDP_SCENARIO_SUB_DISP)
		s = 1;
	memset((void *)(&g_handle[s]), 0, sizeof(ddp_path_handle_t));
	path_handle  = &g_handle[s];
	if (NULL != path_handle) {
