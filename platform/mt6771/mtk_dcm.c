...
static unsigned int all_dcm_type = (ARMCORE_DCM_TYPE | MCUSYS_DCM_TYPE
				    ...
				    | GIC_SYNC_DCM | INFRA_DCM_TYPE
				    | DDRPHY_DCM_TYPE | EMI_DCM_TYPE
				    | MCSI_DCM_TYPE | RGU_DCM_TYPE);
...
typedef void (*DCM_PRESET_FUNC)(int on);
...
{
	dcm_mcu_misc1cfg_mcsib_dcm_preset(on);
	dcm_mcu_misc1cfg_mcsib_dcm(on);
	dcm_peri((ENUM_PERI_DCM) on);
...
int dcm_infra(ENUM_INFRA_DCM on)
{
	...
	dcm_infracfg_ao_infra_emi_local_dcm(0);
...
#endif
				if (dcm->preset_func)
					dcm->preset_func(dcm->current_state);
...
int mt_dcm_init(void)
{
	...
	reg_write(0x10006260, reg_read(0x10006260) & ~(0x1 << 2));
	dcm_info("%s: 0x10006260=0x%x\n", __func__, reg_read(0x10006260));
...
int sync_dcm_set_mp0_div(unsigned int mp0)
{
	...
	dcm_dbg("%s: MCUCFG_SYNC_DCM_MP0_REG=0x%X, mp0_div_sel=%u/%u\n",
...
#else
	dcm_dbg("%s: MCUCFG_SYNC_DCM_MP2_REG=0x%X, mp2_div_sel=%u,%u\n",
#endif
