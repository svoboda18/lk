...
extern int DISP_IsLcmFound(void);
...
static void _build_cmdq_trigger_loop(void)
{
	...
	DISPMSG("primary path trigger thread cmd handle=0x%8p\n", pgc->cmdq_handle_trigger);
...
static int _convert_disp_input_to_rdma(RDMA_CONFIG_STRUCT *dst, disp_input_config* src)
{
	if (src && dst) {
		...
		DISPERR("src(0x%8p) or dst(0x%8p) is null\n", src, dst);
		return -1;
	}
}
	...
	if (l && p) {
		...
		if (p->type == LCM_TYPE_DSI) {
			video_printf("[LCM] LANE_NUM: %d, data_format:(%d,%d,%d,%d)\n",
			             p->dsi.LANE_NUM, p->dsi.data_format.color_order, p->dsi.data_format.format,
...
int primary_display_init(char *lcm_name)
{
	...
	DISPCHECK("pgc->plcm->is_connected = %d \n",pgc->plcm->is_connected);
	if (primary_display_is_video_mode()) {
...
extern int disp_path_clock_off(char* name );
...
int primary_display_get_width(void)
{
	if (pgc->plcm == NULL) {
		...
		DISPMSG("lcm handle is null, after probe:0x%8p\n",pgc->plcm);
		if (pgc->plcm == NULL)
			return 0;
	}
...
int primary_display_get_vsync_interval(void)
{
	...
	if (!primary_display_is_video_mode()) {
		DISPCHECK("LCM cmd mode lcm status %d!\n",DISP_IsLcmFound());
		if (!DISP_IsLcmFound()) {
			DISPCHECK("LCM NOT CONNECTED not cal fps!\n");
			goto fail;
		}
	}
...
UINT32 DISP_GetActiveHeight(void)
{
	if (pgc->plcm == NULL) {
		pgc->plcm = disp_lcm_probe(NULL, LCM_INTERFACE_NOTDEFINED);
		DISPERR("lcm handle is null, after probe:0x%8p\n",pgc->plcm);
