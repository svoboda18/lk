...
static void mmc_prof_card_init(void *data, ulong id, ulong counts)
{
	int err = (int)data;
	if (!err) {
		msdc_printf("[SD%d] Init Card, %d counts, %d us\n",
...
static void mmc_prof_read(void *data, ulong id, ulong counts)
{
	...
	msdc_printf("[SD%d] Read %d bytes, %d counts, %d us, %d KB/s, Avg: %d KB/s\n",
...
static void mmc_prof_write(void *data, ulong id, ulong counts)
{
	...
	msdc_printf("[SD%d] Write %d bytes, %d counts, %d us, %d KB/s, Avg: %d KB/s\n",
	            perf->host->id, blkcnt * blksz, counts,
	            counts * 30 + counts * 16960 / 32768,
	            blkcnt * blksz * 32 / (counts ? counts : 1),
	            ((rpt->total_size / 1024) * 32768) / rpt->total_time);
...
static void mmc_dump_tuning_blk(u8 *buf)
{
	int i;
	for (i = 0; i < 16; i++) {
		msdc_printf("[TBLK%d] %x%x%x%x%x%x%x%x\n", i,
...
static void mmc_dump_csd(struct mmc_card *card)
{
	...
	if (mmc_card_sd(card)) {
		...
		msdc_printf("[CSD] CSD %s\n", sd_csd_ver[csd_struct]);
		msdc_printf("[CSD] TACC_NS: %d ns, TACC_CLKS: %d clks\n", csd->tacc_ns, csd->tacc_clks);
		if (csd_struct == 1) {
			msdc_printf("[CSD] Read/Write Blk Len = 512bytes\n");
		} else {
			msdc_printf("[CSD] Read Blk Len = %d, Write Blk Len = %d\n",
			            1 << csd->read_blkbits, 1 << csd->write_blkbits);
		}
		msdc_printf("[CSD] CMD Class:");
		for (i = 0; i < 12; i++) {
			if ((csd->cmdclass >> i) & 0x1)
				msdc_printf("'%s' ", sd_cmd_cls[i]);
		}
		msdc_printf("\n");
	} else {
		csd_struct = unstuff_bits(resp, 126, 2);
		msdc_printf("[CSD] CSD %s\n", mmc_csd_ver[csd_struct]);
		msdc_printf("[CSD] MMCA Spec v%d\n", csd->mmca_vsn);
		msdc_printf("[CSD] TACC_NS: %d ns, TACC_CLKS: %d clks\n", csd->tacc_ns, csd->tacc_clks);
		msdc_printf("[CSD] Read Blk Len = %d, Write Blk Len = %d\n",
		            1 << csd->read_blkbits, 1 << csd->write_blkbits);
		msdc_printf("[CSD] CMD Class:");
...
void mmc_dump_ext_csd(struct mmc_card *card)
{
	...
	msdc_printf("===========================================================\n");
	msdc_printf("[EXT_CSD] EXT_CSD rev.              : v1.%d (MMCv%s)\n",
	            ext_csd[EXT_CSD_REV], rev[ext_csd[EXT_CSD_REV]]);
	msdc_printf("[EXT_CSD] CSD struct rev.           : v1.%d\n", ext_csd[EXT_CSD_STRUCT]);
	msdc_printf("[EXT_CSD] Supported command sets    : %xh\n", ext_csd[EXT_CSD_S_CMD_SET]);
	msdc_printf("[EXT_CSD] HPI features              : %xh\n", ext_csd[EXT_CSD_HPI_FEATURE]);
	msdc_printf("[EXT_CSD] BG operations support     : %xh\n", ext_csd[EXT_CSD_BKOPS_SUPP]);
	msdc_printf("[EXT_CSD] BG operations status      : %xh\n", ext_csd[EXT_CSD_BKOPS_STATUS]);
	memcpy(&tmp, &ext_csd[EXT_CSD_CORRECT_PRG_SECTS_NUM], 4);
	msdc_printf("[EXT_CSD] Correct prg. sectors      : %xh\n", tmp);
	msdc_printf("[EXT_CSD] 1st init time after part. : %d ms\n", ext_csd[EXT_CSD_INI_TIMEOUT_AP] * 100);
	msdc_printf("[EXT_CSD] Min. write perf.(DDR,52MH,8b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_DDR_W_8_52]);
	msdc_printf("[EXT_CSD] Min. read perf. (DDR,52MH,8b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_DDR_R_8_52]);
	msdc_printf("[EXT_CSD] TRIM timeout: %d ms\n", ext_csd[EXT_CSD_TRIM_MULT] & 0xFF * 300);
	msdc_printf("[EXT_CSD] Secure feature support: %xh\n", ext_csd[EXT_CSD_SEC_FEATURE_SUPPORT]);
	msdc_printf("[EXT_CSD] Secure erase timeout  : %d ms\n", 300 *
	            ext_csd[EXT_CSD_ERASE_TIMEOUT_MULT] * ext_csd[EXT_CSD_SEC_ERASE_MULT]);
	msdc_printf("[EXT_CSD] Secure trim timeout   : %d ms\n", 300 *
	            ext_csd[EXT_CSD_ERASE_TIMEOUT_MULT] * ext_csd[EXT_CSD_SEC_TRIM_MULT]);
	msdc_printf("[EXT_CSD] Access size           : %d bytes\n", ext_csd[EXT_CSD_ACC_SIZE] * 512);
	msdc_printf("[EXT_CSD] HC erase unit size    : %d kbytes\n", ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE] * 512);
	msdc_printf("[EXT_CSD] HC erase timeout      : %d ms\n", ext_csd[EXT_CSD_ERASE_TIMEOUT_MULT] * 300);
	msdc_printf("[EXT_CSD] HC write prot grp size: %d kbytes\n", 512 *
	            ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE] * ext_csd[EXT_CSD_HC_WP_GPR_SIZE]);
	msdc_printf("[EXT_CSD] HC erase grp def.     : %xh\n", ext_csd[EXT_CSD_ERASE_GRP_DEF]);
	msdc_printf("[EXT_CSD] Reliable write sect count: %xh\n", ext_csd[EXT_CSD_REL_WR_SEC_C]);
	msdc_printf("[EXT_CSD] Sleep current (VCC) : %xh\n", ext_csd[EXT_CSD_S_C_VCC]);
	msdc_printf("[EXT_CSD] Sleep current (VCCQ): %xh\n", ext_csd[EXT_CSD_S_C_VCCQ]);
	msdc_printf("[EXT_CSD] Sleep/awake timeout : %d ns\n",
	            100 * (2 << ext_csd[EXT_CSD_S_A_TIMEOUT]));
	memcpy(&tmp, &ext_csd[EXT_CSD_SEC_CNT], 4);
	msdc_printf("[EXT_CSD] Sector count : %xh\n", tmp);
	msdc_printf("[EXT_CSD] Min. WR Perf.  (52MH,8b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_W_8_52]);
	msdc_printf("[EXT_CSD] Min. Read Perf.(52MH,8b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_R_8_52]);
	msdc_printf("[EXT_CSD] Min. WR Perf.  (26MH,8b,52MH,4b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_W_8_26_4_25]);
	msdc_printf("[EXT_CSD] Min. Read Perf.(26MH,8b,52MH,4b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_R_8_26_4_25]);
	msdc_printf("[EXT_CSD] Min. WR Perf.  (26MH,4b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_W_4_26]);
	msdc_printf("[EXT_CSD] Min. Read Perf.(26MH,4b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_R_4_26]);
	msdc_printf("[EXT_CSD] Power class: %x\n", ext_csd[EXT_CSD_PWR_CLASS]);
	msdc_printf("[EXT_CSD] Power class(DDR,52MH,3.6V): %xh\n", ext_csd[EXT_CSD_PWR_CL_DDR_52_360]);
	msdc_printf("[EXT_CSD] Power class(DDR,52MH,1.9V): %xh\n", ext_csd[EXT_CSD_PWR_CL_DDR_52_195]);
	msdc_printf("[EXT_CSD] Power class(26MH,3.6V)    : %xh\n", ext_csd[EXT_CSD_PWR_CL_26_360]);
	msdc_printf("[EXT_CSD] Power class(52MH,3.6V)    : %xh\n", ext_csd[EXT_CSD_PWR_CL_52_360]);
	msdc_printf("[EXT_CSD] Power class(26MH,1.9V)    : %xh\n", ext_csd[EXT_CSD_PWR_CL_26_195]);
	msdc_printf("[EXT_CSD] Power class(52MH,1.9V)    : %xh\n", ext_csd[EXT_CSD_PWR_CL_52_195]);
	msdc_printf("[EXT_CSD] Part. switch timing    : %xh\n", ext_csd[EXT_CSD_PART_SWITCH_TIME]);
	msdc_printf("[EXT_CSD] Out-of-INTR busy timing: %xh\n", ext_csd[EXT_CSD_OUT_OF_INTR_TIME]);
	msdc_printf("[EXT_CSD] Card type       : %xh\n", ext_csd[EXT_CSD_CARD_TYPE]);
	msdc_printf("[EXT_CSD] Command set     : %xh\n", ext_csd[EXT_CSD_CMD_SET]);
	msdc_printf("[EXT_CSD] Command set rev.: %xh\n", ext_csd[EXT_CSD_CMD_SET_REV]);
	msdc_printf("[EXT_CSD] HS timing       : %xh\n", ext_csd[EXT_CSD_HS_TIMING]);
	msdc_printf("[EXT_CSD] Bus width       : %xh\n", ext_csd[EXT_CSD_BUS_WIDTH]);
	msdc_printf("[EXT_CSD] Erase memory content : %xh\n", ext_csd[EXT_CSD_ERASED_MEM_CONT]);
	msdc_printf("[EXT_CSD] Partition config      : %xh\n", ext_csd[EXT_CSD_PART_CFG]);
	msdc_printf("[EXT_CSD] Boot partition size   : %d kbytes\n", ext_csd[EXT_CSD_BOOT_SIZE_MULT] * 128);
	msdc_printf("[EXT_CSD] Boot information      : %xh\n", ext_csd[EXT_CSD_BOOT_INFO]);
	msdc_printf("[EXT_CSD] Boot config protection: %xh\n", ext_csd[EXT_CSD_BOOT_CONFIG_PROT]);
	msdc_printf("[EXT_CSD] Boot bus width        : %xh\n", ext_csd[EXT_CSD_BOOT_BUS_WIDTH]);
	msdc_printf("[EXT_CSD] Boot area write prot  : %xh\n", ext_csd[EXT_CSD_BOOT_WP]);
	msdc_printf("[EXT_CSD] User area write prot  : %xh\n", ext_csd[EXT_CSD_USR_WP]);
	msdc_printf("[EXT_CSD] FW configuration      : %xh\n", ext_csd[EXT_CSD_FW_CONFIG]);
	msdc_printf("[EXT_CSD] RPMB size : %d kbytes\n", ext_csd[EXT_CSD_RPMB_SIZE_MULT] * 128);
	msdc_printf("[EXT_CSD] Write rel. setting  : %xh\n", ext_csd[EXT_CSD_WR_REL_SET]);
	msdc_printf("[EXT_CSD] Write rel. parameter: %xh\n", ext_csd[EXT_CSD_WR_REL_PARAM]);
	msdc_printf("[EXT_CSD] Start background ops : %xh\n", ext_csd[EXT_CSD_BKOPS_START]);
	msdc_printf("[EXT_CSD] Enable background ops: %xh\n", ext_csd[EXT_CSD_BKOPS_EN]);
	msdc_printf("[EXT_CSD] H/W reset function   : %xh\n", ext_csd[EXT_CSD_RST_N_FUNC]);
	msdc_printf("[EXT_CSD] HPI management       : %xh\n", ext_csd[EXT_CSD_HPI_MGMT]);
	memcpy(&tmp, &ext_csd[EXT_CSD_MAX_ENH_SIZE_MULT], 4);
	msdc_printf("[EXT_CSD] Max. enhanced area size : %xh (%d kbytes)\n",
	            tmp & 0x00FFFFFF, (tmp & 0x00FFFFFF) * 512 *
	            ext_csd[EXT_CSD_HC_WP_GPR_SIZE] * ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE]);
	msdc_printf("[EXT_CSD] Part. support  : %xh\n", ext_csd[EXT_CSD_PART_SUPPORT]);
	msdc_printf("[EXT_CSD] Part. attribute: %xh\n", ext_csd[EXT_CSD_PART_ATTR]);
	msdc_printf("[EXT_CSD] Part. setting  : %xh\n", ext_csd[EXT_CSD_PART_SET_COMPL]);
	msdc_printf("[EXT_CSD] General purpose 1 size : %xh (%d kbytes)\n",
	...
	msdc_printf("[EXT_CSD] General purpose 2 size : %xh (%d kbytes)\n",
	            ...
	            ext_csd[EXT_CSD_HC_WP_GPR_SIZE] *
	            ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE]);
	msdc_printf("[EXT_CSD] General purpose 3 size : %xh (%d kbytes)\n",
	            ...
	            ext_csd[EXT_CSD_HC_WP_GPR_SIZE] *
	            ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE]);
	msdc_printf("[EXT_CSD] General purpose 4 size : %xh (%d kbytes)\n",
	            ...
	            ext_csd[EXT_CSD_HC_WP_GPR_SIZE] *
	            ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE]);
	msdc_printf("[EXT_CSD] Enh. user area size : %xh (%d kbytes)\n",
	            ...
	            ext_csd[EXT_CSD_HC_WP_GPR_SIZE] *
	            ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE]);
	msdc_printf("[EXT_CSD] Enh. user area start: %xh\n",
	            (ext_csd[EXT_CSD_ENH_START_ADDR + 0] |
	             ext_csd[EXT_CSD_ENH_START_ADDR + 1] << 8 |
	             ext_csd[EXT_CSD_ENH_START_ADDR + 2] << 16 |
	             ext_csd[EXT_CSD_ENH_START_ADDR + 3]) << 24);
	msdc_printf("[EXT_CSD] Bad block mgmt mode: %xh\n", ext_csd[EXT_CSD_BADBLK_MGMT]);
	msdc_printf("===========================================================\n");
}
#endif
...
int mmc_go_irq_state(struct mmc_host *host, struct mmc_card *card)
{
	...
	if (!(card->csd.cmdclass & CCC_IO_MODE)) {
		msdc_printf("[SD%d] Card doesn't support I/O mode for IRQ state\n", host->id);
...
static void mmc_decode_cid(struct mmc_card *card)
{
	...
	if (mmc_card_sd(card)) {
		...
		switch (card->csd.mmca_vsn) {
			...
			default:
				msdc_printf("[SD%d] Unknown MMCA version %d\n",
...
static int mmc_decode_csd(struct mmc_card *card)
{
	...
	if ( ( mmc_card_mmc(card) &&
		...
		msdc_printf("[SD%d] Unknown CSD ver %d\n", mmc_card_id(card), csd_struct);
		return MMC_ERR_INVALID;
	}
...
static void mmc_decode_ext_csd(struct mmc_card *card)
{
	...
	if ((ext_csd[EXT_CSD_CARD_TYPE] & EXT_CSD_CARD_TYPE_HS400_1_2V) ||
		...
		msdc_printf("[SD%d] MMCv4 but HS unsupported\n", card->host->id);
	}
	...
	if (ext_csd[EXT_CSD_ERASE_GRP_DEF]&EXT_CSD_ERASE_GRP_DEF_EN) {
		...
		msdc_printf("EXT_CSD_ERASE_GRP_DEF is On, wp_size = %dMB\n", card->wp_size/2048);
	} else {
		card->wp_size =(card->csd.write_prot_grpsz+1) * card->csd.erase_sctsz;
		msdc_printf("EXT_CSD_ERASE_GRP_DEF is Off, wp_size = %dMB, csd.write_prot_grpsz = %d,csd.erase_sctsz = %d\n", card->wp_size/2048, card->csd.write_prot_grpsz, card->csd.erase_sctsz);
	}
	msdc_printf("[%s]: mmc_set_wp_size %dMB\n", __func__,card->wp_size/2048);
#endif
               ...
               u8 set, u8 index, u8 value)
{
	...
	do {
		...
		if (err) {
			msdc_printf("[SD%d] Fail to send status %d\n", host->id, err);
			break;
		}
		if (status & R1_SWITCH_ERROR) {
			msdc_printf("[SD%d] switch error. arg(0x%x)\n", host->id, cmd.arg);
			return MMC_ERR_FAILED;
		}
		if (count++ >= 600000) {
			msdc_printf("[%s]: timeout happend, count=%d, status=0x%x\n", __func__, count, status);
			break;
		}
...
int mmc_switch_hs(struct mmc_host *host, struct mmc_card *card)
{
	...
	if ((status[16] & 0xF) != 1) {
		msdc_printf("[SD%d] HS mode not supported!\n", host->id);
		err = MMC_ERR_FAILED;
	} else {
		msdc_printf("[SD%d] Switch to HS mode!\n", host->id);
...
int mmc_switch_uhs1(struct mmc_host *host, struct mmc_card *card, unsigned int mode)
{
	...
	if ((status[16] & 0xF) != mode) {
		msdc_printf("[SD%d] UHS-1 %s mode not supported!\n", host->id, smode[mode]);
		err = MMC_ERR_FAILED;
	} else {
		card->uhs_mode = mode;
		mmc_card_set_uhs1(card);
		msdc_printf("[SD%d] Switch to UHS-1 %s mode!\n", host->id, smode[mode]);
...
int mmc_switch_drv_type(struct mmc_host *host, struct mmc_card *card, int val)
{
	...
	if ((status[15] & 0xF) != val) {
		msdc_printf("[SD%d] UHS-1 %s drv not supported!\n", host->id, type[val]);
		err = MMC_ERR_FAILED;
	} else {
		msdc_printf("[SD%d] Switch to UHS-1 %s drv!\n", host->id, type[val]);
	}
...
int mmc_switch_max_cur(struct mmc_host *host, struct mmc_card *card, int val)
{
	...
	if (((status[15] >> 4) & 0xF) != val) {
		msdc_printf("[SD%d] UHS-1 %s max. current not supported!\n", host->id, curr[val]);
		err = MMC_ERR_FAILED;
	} else {
		msdc_printf("[SD%d] Switch to UHS-1 %s max. current!\n", host->id, curr[val]);
	}
...
static int mmc_read_scrs(struct mmc_host *host, struct mmc_card *card)
{
	...
	if (unstuff_bits(resp, 60, 4) != 0) {
		msdc_printf("[SD%d] Unknown SCR ver %d\n",
	...
	msdc_printf("[SD%d] SD_SPEC(%d) SD_SPEC3(%d) SD_BUS_WIDTH=%d\n",
	            mmc_card_id(card), scr->sda_vsn, scr->sda_vsn3, scr->bus_widths);
	msdc_printf("[SD%d] SD_SECU(%d) EX_SECU(%d), CMD_SUPP(%d): CMD23(%d), CMD20(%d)\n",
...
int mmc_read_ext_csd(struct mmc_host *host, struct mmc_card *card)
{
	...
	if (card->csd.mmca_vsn < CSD_SPEC_VER_4) {
		msdc_printf("[SD%d] MMCA_VSN: %d. Skip EXT_CSD\n", host->id, card->csd.mmca_vsn);
		return MMC_ERR_NONE;
	}
...
static int mmc_read_switch(struct mmc_host *host, struct mmc_card *card)
{
	...
	if (status[13] & 0x01) {
		msdc_printf("[SD%d] Support: Default/SDR12\n", host->id);
		card->sw_caps.hs_max_dtr = 25000000;  /* default or sdr12 */
	}
	if (status[13] & 0x02) {
		msdc_printf("[SD%d] Support: HS/SDR25\n", host->id);
		card->sw_caps.hs_max_dtr = 50000000;  /* high-speed or sdr25 */
	}
	if (status[13] & 0x10) {
		msdc_printf("[SD%d] Support: DDR50\n", host->id);
		card->sw_caps.hs_max_dtr = 50000000;  /* ddr50 */
		card->sw_caps.ddr = 1;
	}
#if defined(FEATURE_MMC_UHS1)
	if (status[13] & 0x04) {
		msdc_printf("[SD%d] Support: SDR50\n", host->id);
		card->sw_caps.hs_max_dtr = 100000000; /* sdr50 */
	}
	if (status[13] & 0x08) {
		msdc_printf("[SD%d] Support: SDR104\n", host->id);
		card->sw_caps.hs_max_dtr = 208000000; /* sdr104 */
	}
	if (status[9] & 0x01) {
		msdc_printf("[SD%d] Support: Type-B Drv\n", host->id);
	}
	if (status[9] & 0x02) {
		msdc_printf("[SD%d] Support: Type-A Drv\n", host->id);
	}
	if (status[9] & 0x04) {
		msdc_printf("[SD%d] Support: Type-C Drv\n", host->id);
	}
	if (status[9] & 0x08) {
		msdc_printf("[SD%d] Support: Type-D Drv\n", host->id);
	}
	if (status[7] & 0x01) {
		msdc_printf("[SD%d] Support: 200mA current limit\n", host->id);
	}
	if (status[7] & 0x02) {
		msdc_printf("[SD%d] Support: 400mA current limit\n", host->id);
	}
	if (status[7] & 0x04) {
		msdc_printf("[SD%d] Support: 600mA current limit\n", host->id);
	}
	if (status[7] & 0x08) {
		msdc_printf("[SD%d] Support: 800mA current limit\n", host->id);
	}
#endif
...
int mmc_erase_start(struct mmc_card *card, u64 addr)
{
	...
	if (!(card->csd.cmdclass & CCC_ERASE)) {
		msdc_printf("[SD%d] Card doesn't support Erase commands\n", card->host->id);
		return MMC_ERR_INVALID;
	}
...
int mmc_erase_end(struct mmc_card *card, u64 addr)
{
	...
	if (!(card->csd.cmdclass & CCC_ERASE)) {
		msdc_printf("[SD%d] Erase isn't supported\n", card->host->id);
		return MMC_ERR_INVALID;
	}
	...
	if (card && mmc_card_mmc(card)) {
		if ((clkhz > 52000000) && (host->caps & MMC_CAP_EMMC_HS400)) {
			if ((mmc_switch(host, card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_HS_TIMING, 1) == MMC_ERR_NONE) &&
				...
				msdc_printf("[SD%d] Switch to HS400 mode!\n", id);
			...
			if ((mmc_switch(host, card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_HS_TIMING, 1) == MMC_ERR_NONE) &&
				...
				msdc_printf("[SD%d] Switch to HS200 mode!\n", id);
				mmc_card_clr_speed_mode(card);
			...
			if ((mmc_switch(host, card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_HS_TIMING, 1) == MMC_ERR_NONE) &&
			        (mmc_switch(host, card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_BUS_WIDTH, EXT_CSD_BUS_WIDTH_8_DDR) == MMC_ERR_NONE)) {
				msdc_printf("[SD%d] Switch to DDR50 mode!\n", id);
			...
			if (mmc_switch(host, card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_HS_TIMING, 1) == MMC_ERR_NONE) {
				msdc_printf("[SD%d] Switch to High-Speed mode!\n", id);
				mmc_card_clr_speed_mode(card);
				mmc_card_set_highspeed(card);
			} else {
				result = -__LINE__;
			...
			if (mmc_switch(host, card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_HS_TIMING, 0) == MMC_ERR_NONE) {
				msdc_printf("[SD%d] Switch to Default mode!\n", id);
				mmc_card_clr_speed_mode(card);
...
int mmc_set_bus_width(struct mmc_host *host, struct mmc_card *card, int width)
{
	...
	if (mmc_card_sd(card)) {
		...
		if (err != MMC_ERR_NONE) {
			msdc_printf("[SD%d] Switch to bus width(%d) failed\n", host->id, arg);
			goto out;
		}
...
#if 0
	if (mmc_card_sd(card)) {
		msdc_printf("[info][%s %d] switch to %dbit bus width, arg=0x%x, err = %d\n", __func__, __LINE__, (arg == SD_BUS_WIDTH_4) ? 4 : 1, arg, err);
	} else {
		switch (arg) {
			case EXT_CSD_BUS_WIDTH_1:
				msdc_printf("[info][%s %d] switch to 1bit bus width, err = %d\n", __func__, __LINE__, err);
				break;
			case EXT_CSD_BUS_WIDTH_4:
				msdc_printf("[info][%s %d] switch to 4bit bus width, err = %d\n", __func__, __LINE__, err);
				break;
			case EXT_CSD_BUS_WIDTH_8:
				msdc_printf("[info][%s %d] switch to 8bit bus width, err = %d\n", __func__, __LINE__, err);
				break;
			case EXT_CSD_BUS_WIDTH_4_DDR:
				msdc_printf("[info][%s %d] switch to 4bit bus width(DDR), err = %d\n", __func__, __LINE__, err);
				break;
			case EXT_CSD_BUS_WIDTH_8_DDR:
				msdc_printf("[info][%s %d] switch to 8bit bus width(DDR), err = %d\n", __func__, __LINE__, err);
				break;
			default:
				msdc_printf("[info][%s %d] switch to ?bit bus width(DDR), err = %d\n", __func__, __LINE__, err);
				break;
		}
	}
#endif
...
int mmc_boot_config(struct mmc_card *card, u8 acken, u8 enpart, u8 buswidth, u8 busmode)
{
	...
	msdc_printf("=====Set boot Bus Width<%d>=======\n",buswidth);
	msdc_printf("=====Set boot Bus mode<%d>=======\n",busmode);
	...
	do {
		...
		if (err != MMC_ERR_NONE) {
			if (msdc_abort_handler(host, 1))
				msdc_printf("[SD%d] data abort failed\n",host->id);
			result = msdc_tune_read(host);
		}
#if defined(FEATURE_MMC_RD_TUNING)
	} while (err && result != MMC_ERR_READTUNEFAIL);
                                        ...
                                        u32 blkcnt, EMMC_WP_TYPE type)
{
	...
	for (i = 0; i < count_wp_group; i++) {
		...
		if (err) {
			msdc_printf("[%s]: mmc_send_write_prot_type err %d \n", __func__,err);
			goto out;
		}
                                   ...
                                   Region partition,unsigned long blknr,u32 blkcnt, EMMC_WP_TYPE type)
{
	...
	if (type == WP_PERMANENT || type == WP_DISABLE || type == WP_TEMPORARY) {
		msdc_printf("[%s]: Type Not Support\n", __func__);
		return 0;
	}
	if (type > WP_DISABLE || type < 0)  {
		msdc_printf("[%s]:%d type=%d\n", __func__,__LINE__,type);
		return err;
	}
	//check partition
	...
	if (err) {
		msdc_printf("[%s]: mmc_switch_part err %d\n", __func__,err);
		return err;
	}

	count_wp = blkcnt/card->wp_size;

	msdc_printf("[%s]: count_wp %d\n", __func__,count_wp);

	for (index=0; index<count_wp; index++) {
		err=mmc_set_write_prot(card->host,blknr+(index*card->wp_size));

		if (err) {
			msdc_printf("[%s]: mmc_set_write_prot err %d \n", __func__,err);
			return err;
		}
	}
...
//	struct mmc_host *host = mmc_get_host(dev_num);
	...
	if (type==WP_POWER_ON && partition == EMMC_PART_USER) {
		...
		if (err) {
			msdc_printf("[%s]: mmc_set_boot_wp err%d\n", __func__,err);
			return err;
		}
	} else if (type == WP_PERMANENT || type == WP_DISABLE || type == WP_TEMPORARY) {
		msdc_printf("[%s]: WP Type Not Support\n", __func__);
		return 0;
...
int mmc_set_boot_wp(struct mmc_card *card, EMMC_WP_TYPE type, Region partition)
{
	int err = MMC_ERR_FAILED;
	u8 value;
	...
	if (card->ext_csd.boot_wp & EXT_CSD_BOOT_WP_DIS_PWR_WP) {
		//msdc_printf(INFO, "[%s]: EXT_CSD_BOOT_WP_DIS_PWR_WP is err set \n", __func__);
		goto out;
	}
	...
	if (card->ext_csd.boot_wp== value) {
		//msdc_printf(INFO, "[%s]: EXT_CSD_BOOT_WP alread set ",__func__);
		return 0;
	}
	...
	if (err) {
		//msdc_printf(INFO, "[%s]: mmc_switch err %d\n", __func__,err);
		goto out;
	}
	...
	err = mmc_read_ext_csd(card->host, card);
	if (err) {
		msdc_printf(INFO, "[%s]: read ext_csd err %d\n", __func__,err);
	}
                    ...
                    u32 blkcnt,Region partition)
{
	...
	if (card->ext_csd.usr_wp & US_PWR_WP_DIS) {
		msdc_printf("[%s]: US_PWR_WP_DIS is err set \n", __func__);
		goto out;
	}
	...
	if (card->ext_csd.usr_wp == value) {
		msdc_printf("[%s]: EXT_CSD_USR_WP alread set \n",__func__);
	} else {
		err = mmc_switch(card->host, card, EXT_CSD_CMD_SET_NORMAL,
		                 EXT_CSD_USR_WP, value);
		if (err) {
			msdc_printf("[%s]: mmc_switch err %d\n", __func__,err);
			goto out;
		}
	...
	if (err) {
		msdc_printf("[%s]: restore US_PWR_WP_EN err %d\n", __func__,err);
		goto out;
	}
...
int mmc_dev_bread(struct mmc_card *card, unsigned long blknr, u32 blkcnt, u8 *dst)
{
	...
	int tune = 0;
#if defined(FEATURE_MMC_RD_TUNING) || defined(MSDC_SLT)
	int retry = 1;
#else
	int retry = 3;
#endif
	...
	do {
		...
		if (!tune) {
			...
			if (mmc_card_mmc(card) && mmc_card_hs400(card)) {
				msdc_printf("[tune][%s:%d] start hs400 read tune\n", __func__, __LINE__);
		...
		if (err == MMC_ERR_BADCRC || err == MMC_ERR_ACMD_RSPCRC || err == MMC_ERR_CMD_RSPCRC || err == MMC_ERR_TIMEOUT) {
			...
			msdc_printf("[SD%d] Fail to tuning,%s",host->id,(err == MMC_ERR_CMDTUNEFAIL)?"cmd tune failed!\n":"read tune failed!\n");
			break;
		}
...
int mmc_block_read(int dev_num, unsigned long blknr, u32 blkcnt, unsigned long *dst)
{
	...
	if ( blkcnt> maxblks ) {
		msdc_printf("[SD%d] Non-blocking RW does not allow blkcnt(%d) > maxblks(%d)\n", host->id, blkcnt, maxblks);
		BUG_ON(1);
	}
#endif
	...
	if (blknr * (blksz / MMC_BLOCK_SIZE) > card->nblks) {
		msdc_printf("[SD%d] Out of block range: blknr(%ld) > sd_blknr(%d)\n",
...
int mmc_get_sandisk_fwid(int id, u8* buf)
{
	...
	while (state != 4) {
		...
		msdc_printf("check card state<%d>\n", state);
		if (state == 5 || state == 6) {
			msdc_printf("state<%d> need cmd12 to stop\n", state);
			...
			msdc_printf("state<%d> card is busy\n", state);
			mdelay(100);
		} else if (state != 4) {
			msdc_printf("state<%d> ??? \n", state);
			return MMC_ERR_INVALID;
	...
	if (err) {
		msdc_printf("[SD%d] Fail to send(CMD50) sandisk fwid %d\n", host->id, err);
		return err;
	}

	err = msdc_dma_send_sandisk_fwid(host, buf,MMC_CMD21,1);
	if (err) {
		msdc_printf("[SD%d] Fail to get(CMD21) sandisk fwid %d\n", host->id, err);
		return err;
	}
...
int mmc_init_mem_card(struct mmc_host *host, struct mmc_card *card, u32 ocr)
{
	 ...
	 */
	if (ocr & 0x7F) {
		msdc_printf("card claims to support voltages "
	...
	if (err != MMC_ERR_NONE) {
		msdc_printf("[SD%d] Fail in SEND_OP_COND cmd\n", id);
		goto out;
	}
	...
	msdc_printf("[SD%d] ocr = 0x%X, card->ocr=0x%X, s18a = %d \n", id, ocr, card->ocr, s18a);
	...
	if (s18a) {
		...
		if (err != MMC_ERR_NONE) {
			msdc_printf("[SD%d] Fail in SWITCH_VOLT cmd\n", id);
			goto out;
		}
	}
#endif
	...
	if (err != MMC_ERR_NONE) {
		msdc_printf("[SD%d] Fail in SEND_CID cmd\n", id);
		goto out;
	}
	...
	if (err != MMC_ERR_NONE) {
		msdc_printf("[SD%d] Fail in SEND_RCA cmd\n", id);
		goto out;
	}
	...
	if (err != MMC_ERR_NONE) {
		msdc_printf("[SD%d] Fail in SEND_CSD cmd\n", id);
		goto out;
	}
	...
	if (err != MMC_ERR_NONE) {
		msdc_printf("[SD%d] Fail in decode csd\n", id);
		goto out;
	}
	...
	if (err != MMC_ERR_NONE) {
		msdc_printf("[SD%d] Fail in select card cmd\n", id);
		goto out;
	}
	...
	if (mmc_card_sd(card)) {
		...
		if (err != MMC_ERR_NONE) {
			msdc_printf("[SD%d] Fail in SEND_SCR cmd\n", id);
			goto out;
		}
#endif
		...
		if (err != MMC_ERR_NONE) {
			msdc_printf("[SD%d] Fail in SEND_EXT_CSD cmd\n", id);
			goto out;
		}
		...
		if ((card->ext_csd.hs_max_dtr > 52000000) && (host->caps & MMC_CAP_EMMC_HS400)) {
			...
			if ( err != MMC_ERR_NONE ) {
				msdc_printf("[SD%d] Switch to High-speed mode failed!\n", host->id);
				goto out;
			}
			...
			if (err != MMC_ERR_NONE) {
				msdc_printf("[SD%d] Switch to 8-bit failed!\n", host->id);
				goto out;
			}
			...
			if ( err != MMC_ERR_NONE ) {
				msdc_printf("[SD%d] Switch to HS400 mode failed!\n", host->id);
				goto out;
			}
	...
	if (!(mmc_card_mmc(card) && (mmc_card_ddr(card) || mmc_card_hs400(card)))) {
		err = mmc_set_blk_length(host, MMC_BLOCK_SIZE);
		if (err != MMC_ERR_NONE) {
			msdc_printf("[SD%d] Fail in set blklen cmd, card state=0x%x\n", id, card->state);
			goto out;
		}
	}
...
int mmc_init_card(struct mmc_host *host, struct mmc_card *card)
{
	...
	if (mmc_send_io_op_cond(host, 0, &ocr) == MMC_ERR_NONE) {
		...
		if (err != MMC_ERR_NONE) {
			msdc_printf("[SD%d] Fail in init sdio card\n", id);
			goto out;
		}
		/* no memory present */
		if ((ocr & 0x08000000) == 0) {
	...
	if (err != MMC_ERR_NONE) {
		...
		if (err != MMC_ERR_NONE) {
			msdc_printf("[SD%d] Fail in MMC_CMD_SEND_OP_COND/SD_ACMD_SEND_OP_COND cmd\n", id);
			goto out;
		}
...
out:
	...
	msdc_printf("[%s]: finish successfully\n", __func__);
	return 0;
}
...
int mmc_init(int id, u32 trans_mode)
{
	...
	msdc_printf("[%s]: msdc%d start mmc_init_card()\n", __func__, id);
	err = mmc_init_card(host, card);
	if (err != MMC_ERR_NONE)
		return err;

#ifdef MTK_EMMC_SUPPORT_OTP
	msdc_printf("[%s]: msdc%d, use hc erase size\n", __func__, id);
	if (mmc_card_mmc(card))
	...
	msdc_printf("[%s]: start r/w compare test \n", __func__);
