...
static int  mmsys_config_dump_analysis(void)
{
	unsigned int i = 0;
	unsigned int reg = 0;
	char clock_on[512]= {'\0'};
	...
	for (i = 0; i < 32; i++) {
		if ((reg & (1 << i)) == 0) {
			name = ddp_clock_0(i);
			ASSERT(strlen(clock_on) + strlen(name) < 512);
...
static void aal_dump_analysis(void)
{
	...
	DDPMSG("aal: bypass=%d, relay=%d, en=%d, w=%d, h=%d, inLCnt=%d, inPCnt=%d, outLCnt=%d, outPCnt=%d\n",
