...
void mtk_mculatch_en(unsigned int en)
{
	unsigned int tmp;

	tmp = DRV_Reg32(MTK_WDT_LATCH_CTL);
	tmp |= MTK_LATCH_CTL_KEY;

	if(0 == en)
		tmp &= (~MTK_RG_MCU_LATH_EN);
	else
		tmp |= (MTK_RG_MCU_LATH_EN);

	DRV_WriteReg32(MTK_WDT_LATCH_CTL, tmp);
}
...
void mtk_arch_full_reset(void)
{
	...
	WDTLOG("mtk_arch_full_reset\n");

	/* configure PMIC as full reset mode */
	pmic_cold_reset();

	/*
	 * trigger WDT SW reset.
	 *
	 * we expect to bypass power key in next boot-up but
	 * its flag will be reset by PMIC full reset.
	 */
	mtk_wdt_reset(1);
	...
	WDTLOG("mtk_arch_full_reset: PMIC full reset is not supported.\n");
