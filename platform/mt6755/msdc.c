...
#if MSDC_DEBUG
	if (resp & (1 << 7))
		msdc_printf("\t[OCR] Low Voltage Range\n");
	if (resp & (1 << 15))
		msdc_printf("\t[OCR] 2.7-2.8 volt\n");
	if (resp & (1 << 16))
		msdc_printf("\t[OCR] 2.8-2.9 volt\n");
	if (resp & (1 << 17))
		msdc_printf("\t[OCR] 2.9-3.0 volt\n");
	if (resp & (1 << 18))
		msdc_printf("\t[OCR] 3.0-3.1 volt\n");
	if (resp & (1 << 19))
		msdc_printf("\t[OCR] 3.1-3.2 volt\n");
	if (resp & (1 << 20))
		msdc_printf("\t[OCR] 3.2-3.3 volt\n");
	if (resp & (1 << 21))
		msdc_printf("\t[OCR] 3.3-3.4 volt\n");
	if (resp & (1 << 22))
		msdc_printf("\t[OCR] 3.4-3.5 volt\n");
	if (resp & (1 << 23))
		msdc_printf("\t[OCR] 3.5-3.6 volt\n");
	if (resp & (1 << 24))
		msdc_printf("\t[OCR] Switching to 1.8V Accepted (S18A)\n");
	if (resp & (1 << 30))
		msdc_printf("\t[OCR] Card Capacity Status (CCS)\n");
	if (resp & (1UL << 31))
		msdc_printf("\t[OCR] Card Power Up Status (Idle)\n");
	else
		msdc_printf("\t[OCR] Card Power Up Status (Busy)\n");
#endif
}
...
void msdc_dump_io_resp(u32 resp)
{
	...
	if (flags & (1 << 7))
		msdc_printf("\t[IO] COM_CRC_ERR\n");
	if (flags & (1 << 6))
		msdc_printf("\t[IO] Illgal command\n");
	if (flags & (1 << 3))
		msdc_printf("\t[IO] Error\n");
	if (flags & (1 << 2))
		msdc_printf("\t[IO] RFU\n");
	if (flags & (1 << 1))
		msdc_printf("\t[IO] Function number error\n");
	if (flags & (1 << 0))
		msdc_printf("\t[IO] Out of range\n");

	msdc_printf("[IO] State: %s, Data:0x%x\n", state[(resp >> 12) & 0x3], resp & 0xFF);
#endif
}
...
static void msdc_dump_dbg_register(struct mmc_host *host)
{
	...
	for (i = 0; i <= 0x3c; i++) {
		...
		msdc_printf("[SD%d]SW_DBG_SEL: write reg[%x] to 0x%x\n", host->id, OFFSET_MSDC_DBG_SEL, i);
		msdc_printf("[SD%d]SW_DBG_OUT: read  reg[%x] to 0x%x\n", host->id, OFFSET_MSDC_DBG_OUT, MSDC_READ32(MSDC_DBG_OUT));
	}
...
int msdc_register_partial_backup_and_reset(struct mmc_host* host)
{
	...
	for (i = 0; i < HS400_BACKUP_REG_NUM; i++) {
		...
		if (hs400_backup_reg_list[i].restore_func) {
			...
			if (err) {
				msdc_printf("[%s]: failed to restore reg[0x%x][0x%x], expected value[0x%x], actual value[0x%x] err=0x%x",
...
void msdc_enable_dcm(struct mmc_host *host, int on, u8 div1, u8 div2)
{
	...
	if (on) {
		...
		MSDC_SET_FIELD(MSDC_PATCH_BIT1, MSDC_PB1_DCM_EN, MSDC_DCM_EN);
		if (host->id == 0) {
			MSDC_SET_FIELD(MSDC_PATCH_BIT1, MSDC_PB1_DCM_DEV_SEL1, div1);
			MSDC_SET_FIELD(MSDC_PATCH_BIT1, MSDC_PB1_DCM_DEV_SEL2, div2);
		} else {
			MSDC_SET_FIELD(MSDC_PATCH_BIT1, MSDC_PB1_DCM_DEV_SEL1, div1);
		}
	} else {
		MSDC_SET_FIELD(MSDC_PATCH_BIT1, MSDC_PB1_DCM_EN, MSDC_DCM_DIS);
	}
}
...
void msdc_set_startbit(struct mmc_host *host, u8 start_bit)
{
	...
	switch (l_start_bit) {
		case 0:
			msdc_printf("[info][%s %d] read data start bit at rising edge\n", __func__, __LINE__);
			break;
		case 1:
			msdc_printf("[info][%s %d] read data start bit at falling edge\n", __func__, __LINE__);
			break;
		case 2:
			msdc_printf("[info][%s %d] read data start bit at rising & falling edge\n", __func__, __LINE__);
			break;
		case 3:
			msdc_printf("[info][%s %d] read data start bit at rising | falling edge\n", __func__, __LINE__);
			break;
	...
	switch (type) {
		case TYPE_CMD_RESP_EDGE:
			...
			if (mode == MSDC_SMPL_RISING || mode == MSDC_SMPL_FALLING) {
				...
				msdc_printf("[%s]: SD%d invalid resp parameter: HS400=%d, type=%d, mode=%d\n", __func__, host->id, HS400, type, mode);
			}
			break;
		...
		case TYPE_WRITE_CRC_EDGE:
			...
			if (priv->tuning_mode == MSDC_PATH_USE_ASYNC_FIFO) {
				if (mode == MSDC_SMPL_RISING || mode == MSDC_SMPL_FALLING) {
					if (HS400) {
					...
					msdc_printf("[%s]: SD%d invalid read parameter: HS400=%d, type=%d, mode=%d\n", __func__, host->id, HS400, type, mode);
				}
				...
				if (mode == MSDC_SMPL_RISING || mode == MSDC_SMPL_FALLING) {
					...
					msdc_printf("[%s]: SD%d invalid crc parameter: HS400=%d, type=%d, mode=%d\n", __func__, host->id, HS400, type, mode);
				}
			}
			break;
		...
		case TYPE_WRITE_DATA_EDGE:
			...
			if (mode == MSDC_SMPL_RISING|| mode == MSDC_SMPL_FALLING) {
				...
				msdc_printf("[%s]: SD%d invalid write parameter: HS400=%d, type=%d, mode=%d\n", __func__, host->id, HS400, type, mode);
			}
			break;

		default:
			msdc_printf("[%s]: SD%d invalid parameter: HS400=%d, type=%d, mode=%d\n", __func__, host->id, HS400, type, mode);
			break;
	}
}
...
int msdc_send_cmd(struct mmc_host *host, struct mmc_command *cmd)
{
	...
	if ( cmdq_reg_setting ) {
		...
		msdc_printf("busy status: %x\n", MSDC_READ32(SDC_STS));
		if (SDC_IS_CMD_BUSY()) {
			WAIT_COND(!SDC_IS_CMD_BUSY(), cmd->timeout, timeout);
			if (timeout == 0) {
				error = MMC_ERR_TIMEOUT;
				msdc_printf("[SD%d] CMD(%d): SDC_IS_CMD_BUSY timeout\n",
	...
	if (opcode == MMC_CMD_SEND_STATUS) {
		...
		if (SDC_IS_BUSY()) {
			...
			if (timeout == 0) {
				error = MMC_ERR_TIMEOUT;
				msdc_printf("[SD%d] CMD(%d): SDC_IS_BUSY timeout\n",
				       host->id, (opcode & ~(SD_CMD_BIT | SD_CMD_APP_BIT)));
				goto end;
			}
		}
...
int msdc_wait_rsp(struct mmc_host *host, struct mmc_command *cmd)
{
	...
	if ((status & MSDC_INT_RSPCRCERR) || (status & MSDC_INT_ACMDCRCERR)) {
		...
		msdc_printf("[SD%d] CMD(%d): RSP(%d) ERR(INVALID), Status:%x\n",
...
int msdc_cmd(struct mmc_host *host, struct mmc_command *cmd)
{
	...
	if (err == MMC_ERR_BADCRC) {
		...
		if ( host->app_cmd!=2 ) { //Light 20121225, to prevent recursive call path: msdc_tune_cmdrsp->msdc_app_cmd->msdc_cmd->msdc_tune_cmdrsp
			...
			if (err != MMC_ERR_NONE) {
				msdc_printf("[Err handle][%s:%d]tune cmd fail\n", __func__, __LINE__);
			}
		}
...
int msdc_abort_handler(struct mmc_host *host, int abort_card)
{
	...
	while (state != 4) { // until status to "tran"; //20130125 Comment out by Light
		...
		//#if defined(MMC_MSDC_DRV_CTP)
		if (err == MMC_ERR_BADCRC) {
			msdc_printf("[Err handle][%s:%d]cmd13 crc error\n", __func__, __LINE__);
		...
		if (err == MMC_ERR_TIMEOUT) {
			msdc_printf("[Err handle][%s:%d]cmd13 timeout\n", __func__, __LINE__);
			msdc_tune_update_cmdrsp(host, count++);
...
#else
		if (err != MMC_ERR_NONE) {
			msdc_printf("[Err handle][%s:%d]cmd13 fail\n", __func__, __LINE__);
			goto out;
		}
#endif
		...
		if (state == 5 || state == 6) {
			if (abort_card) {
				...
				//#if defined(MMC_MSDC_DRV_CTP)
				if (err == MMC_ERR_BADCRC) {
					msdc_printf("[Err handle][%s:%d]cmd12 crc error\n", __func__, __LINE__);
					msdc_tune_update_cmdrsp(host, count++);
				...
				if (err == MMC_ERR_TIMEOUT) {
					msdc_printf("[Err handle][%s:%d]cmd12 timeout\n", __func__, __LINE__);
					msdc_tune_update_cmdrsp(host, count++);
...
#else
				if (err != MMC_ERR_NONE) {
					msdc_printf("[Err handle][%s:%d]cmd12 fail\n", __func__, __LINE__);
					goto out;
				}
#endif
			}
...
void msdc_brk_cmd(struct mmc_host *host)
{
	...
	if (tmo == 0)
		msdc_printf("[%s]: SDC BUSY timeout happend, before send break cmd\n", __func__);
...
int msdc_pio_read(struct mmc_host *host, u32 *ptr, u32 size)
{
	...
	while (1) {
		...
		if (status & MSDC_INT_DATCRCERR) {
			...
			msdc_printf("[SD%d] DAT CRC error (0x%x), Left:%d/%d bytes, RXFIFO:%d,dcrc:0x%x\n",
			       host->id, status, size, totalsz, MSDC_RXFIFOCNT(),dcrc);
			err = MMC_ERR_BADCRC;
			break;
		} else if (status & MSDC_INT_DATTMO) {
			msdc_printf("[SD%d] DAT TMO error (0x%x), Left: %d/%d bytes, RXFIFO:%d\n",
			       host->id, status, size, totalsz, MSDC_RXFIFOCNT());
			err = MMC_ERR_TIMEOUT;
			break;
		} else if (status & MSDC_INT_ACMDCRCERR) {
			MSDC_GET_FIELD(SDC_DCRC_STS, SDC_DCRC_STS_POS|SDC_DCRC_STS_NEG, dcrc);
			msdc_printf("[SD%d] AUTOCMD CRC error (0x%x), Left:%d/%d bytes, RXFIFO:%d,dcrc:0x%x\n",
			       host->id, status, size, totalsz, MSDC_RXFIFOCNT(),dcrc);
		...
		if (size > 0) {
			...
			if (pio_bits == 8) {
				do {
					...
					msdc_printf("0x%x ",MSDC_FIFO_READ8());
#else
				...
				do {
					if (left> 1) {
#ifdef MTK_MSDC_DUMP_FIFO
						msdc_printf("0x%x ",MSDC_FIFO_READ16());
#else
...
#endif
			{
				...
				do {
					if (left> 3) {
#ifdef MTK_MSDC_DUMP_FIFO
						msdc_printf("0x%x ",MSDC_FIFO_READ32());
#else
	...
	if (err != MMC_ERR_NONE) {
		...
		msdc_printf("[SD%d] %d-bit PIO Read Error (%d)\n", host->id,
		       pio_bits, err);
	}
...
int msdc_pio_write(struct mmc_host *host, u32 *ptr, u32 size)
{
	...
	while (1) {
		...
		if (status & MSDC_INT_DATCRCERR) {
			msdc_printf("[SD%d] DAT CRC error (0x%x), Left DAT: %d bytes\n",
			       host->id, status, size);
			err = MMC_ERR_BADCRC;
			break;
		} else if (status & MSDC_INT_DATTMO) {
			msdc_printf("[SD%d] DAT TMO error (0x%x), Left DAT: %d bytes, MSDC_FIFOCS=%xh\n",
			       host->id, status, size, MSDC_READ32(MSDC_FIFOCS));
			err = MMC_ERR_TIMEOUT;
			break;
		} else if (status & MSDC_INT_ACMDCRCERR) {
			msdc_printf("[SD%d] AUTO CMD CRC error (0x%x), Left DAT: %d bytes\n",
			       host->id, status, size);
			err = MMC_ERR_ACMD_RSPCRC;
			break;
		} else if (status & MSDC_INT_XFER_COMPL) {
...
int msdc_pio_get_sandisk_fwid(struct mmc_host *host, uchar *dst)
{
	...
	if (err != MMC_ERR_NONE) {
		if (derr != MMC_ERR_NONE) {
			msdc_printf("[SD%d] Read data error (%d)\n", host->id, derr);
			msdc_abort_handler(host, 1);
		} else {
			msdc_printf("[SD%d] Read error (%d)\n", host->id, err);
		}
	}
...
int msdc_pio_send_sandisk_fwid(struct mmc_host *host,uchar *src)
{
	...
	if (err != MMC_ERR_NONE) {
		if (derr != MMC_ERR_NONE) {
			msdc_printf("[SD%d] Write data error (%d)\n", host->id, derr);
			msdc_abort_handler(host, 1);
		} else {
			msdc_printf("[SD%d] Write error (%d)\n", host->id, err);
		}
	}
	return (derr == MMC_ERR_NONE) ? err : derr;
}
...
void msdc_reset_timing_register(struct mmc_host *host)
{
	...
	MSDC_WRITE32(MSDC_PATCH_BIT2, 0x14801803);
	MSDC_WRITE32(MSDC_PAD_TUNE0, 0);
	MSDC_WRITE32(MSDC_PAD_TUNE1, 0);
}

void msdc_init_tune_path(struct mmc_host *host, unsigned int path, int hs400)
{
	u32 base = host->base;
	msdc_priv_t *priv = host->priv;

	priv->tuning_mode = path;
...
void msdc_config_clock(struct mmc_host *host, int ddr, u32 hz, u32 hs_timing)
{
	...
	msdc_init_tune_path(host, priv->tuning_mode, (mode ==3) ? 1 : 0);

	msdc_printf("[SD%d] SET_CLK(%dkHz): SCLK(%dkHz) MODE(%d) DDR(%d) DIV(%d) DS(%d) RS(%d)\n",
...
int msdc_switch_volt(struct mmc_host *host, int volt)
{
	...
	if ((MSDC_READ32(MSDC_PS) & ((1 << 24) | (0xF << 16))) == 0) {
		...
		msdc_config_clock(host, 0, 260000, 0);
		...
		if (status & MSDC_CFG_BV18PSS)
			...
			msdc_printf("[%s] sd%d v18 switch failed, MSDC_CFG=0x%x\n", __func__, host->id, status);
			...
			do {
				for (rsmpl = 0; rsmpl < 2; rsmpl++) {
					...
					/* for debugging */
					{
						...
						msdc_printf("[SD%d] <TUNE_CMD><%d><%s> CMD_RSP_TA_CNTR=%xh\n",
						       host->id, times, (result == MMC_ERR_NONE) ?
						       "PASS" : "FAIL", t_cmdrtc);
						if (host->cur_bus_clk > 100000000) {
							msdc_printf("[SD%d] <TUNE_CMD%d><%d><%s> CMD_RSP_TA_CNTR=%xh, INT_DAT_LATCH_CK_SEL=%xh\n",
							       host->id, (cmd->opcode & (~(SD_CMD_BIT | SD_CMD_APP_BIT))), times, (result == MMC_ERR_NONE) ?
			...
			do {
				for (rdsmpl = 0; rdsmpl < 2; rdsmpl++) {
					...
					/* for debugging */
					{
						...
						times++;
						msdc_printf("[SD%d] <TUNE_BREAD_%d><%s><cmd%d:0x%x> DCRC=%xh, ret=%d\n",
						       host->id, times, (result == MMC_ERR_NONE && dcrc == 0) ?
						       "PASS" : "FAIL", (nblks == 1 ? 17 : 18), src, dcrc, result);
						msdc_printf("[SD%d] <TUNE_BREAD_%d><%s><cmd%d:0x%x> DATRDDLY0=%xh, DATRDDLY1=%xh, DSMPL=%xh\n",
						       host->id, times, (result == MMC_ERR_NONE && dcrc == 0) ?
						       "PASS" : "FAIL", (nblks == 1 ? 17 : 18), src, MSDC_READ32(MSDC_DAT_RDDLY0), MSDC_READ32(MSDC_DAT_RDDLY1), t_dspl);

						if (host->cur_bus_clk >= 100000000) {
							msdc_printf("[SD%d] <TUNE_BREAD_%d><%s><cmd%d:0x%x> CKGEN_MSDC_DLY_SEL=%xh, INT_DAT_LATCH_CK_SEL=%xh\n",
							       host->id, times, (result == MMC_ERR_NONE && dcrc == 0) ?
	...
	do {
		...
		do {
			...
			/* for debugging */
			{
				times++;
				if (rw == 0) {
					msdc_printf("[SD%d] <TUNE_BREAD_%d><%s><cmd%d:0x%x> ret=%d, DS_DLY1=%d, DS_DLY3=%d\n",
					       host->id, times, result == MMC_ERR_NONE ? "PASS" : "FAIL", (nblks == 1 ? 17 : 18), (unsigned int)dst,
					       result, ds_dly1, ds_dly3);
				} else if (rw == 1) {
					msdc_printf("[SD%d] <TUNE_BEWRITE_%d><%s><cmd%d:0x%x> ret=%d, DS_DLY1=%d, DS_DLY3=%d\n",
			...
			do {
				for (wdsmpl = 0; wdsmpl < 2; wdsmpl++) {
					...
					/* for debugging */
					{
						...
						times++;
						msdc_printf("[SD%d] <TUNE_BWRITE_%d><%s><cmd%d:0x%x> ret=%d, DSPL=%d, WRRDLY=%d, MSDC_DAT_RDDLY0=%xh\n",
						       host->id, times, result == MMC_ERR_NONE ? "PASS" : "FAIL", (nblks == 1 ? 24 : 25), (unsigned int)dst,
						       result, t_dspl, t_wrrdly, MSDC_READ32(MSDC_DAT_RDDLY0));

						if (host->cur_bus_clk >= 100000000) {
							msdc_printf("[SD%d] <TUNE_BWRITE_%d><%s><cmd%d:0x%x> MSDC_PB1_WRDAT_CRCS_TA_CNTR=%xh\n",
							       host->id, times, (result == MMC_ERR_NONE) ? "PASS" : "FAIL", (nblks == 1 ? 24 : 25), (unsigned int)dst,
							       t_d_cntr);
						}
					}
...
int msdc_tune_uhs1(struct mmc_host *host, struct mmc_card *card)
{
	...
	for (i = 0; i < 13; i++) {
		...
		if (!status) {
			msdc_printf("[SD%d] ACMD19_TRG(%d), STS(0x%x) Failed\n", host->id, i,
			       status);
			err = MMC_ERR_FAILED;
			goto out;
		}
...
int msdc_emmc_boot_start(struct mmc_host *host, u32 hz, int ddr, int mode, int ackdis, u8 hostbuswidth, u64 size)
{
	...
	msdc_printf("[SD%d] EMMC BOOT ACK timeout: %d ms (clkcnt: %d)(host->cur_bus_clk = %d)\n", host->id,
	       (acktmo * 65536) / (host->cur_bus_clk / 1000), acktmo, host->cur_bus_clk);
	msdc_printf("[SD%d] EMMC BOOT DAT timeout: %d ms (clkcnt: %d)\n", host->id,
	       (dattmo * 65536) / (host->cur_bus_clk / 1000), dattmo);
	msdc_printf("[SD%d] EMMC BOOT ACK %s on host\n", host->id, (ackdis ? "disabled" : "enabled"));
	...
	if (!ackdis) {
		do {
			...
			if (sts & EMMC_STS_BOOTACKERR) {
				msdc_printf("[%s]: [SD%d] EMMC_STS(0x%x): boot up ack error\n", __func__, host->id, sts);
				err = MMC_ERR_BADCRC;
				goto out;
			} else if (sts & EMMC_STS_BOOTACKRCV) {
				msdc_printf("[%s]: [SD%d] EMMC_STS(0x%x): boot ack received\n",  __func__,host->id, sts);
				break;
			} else if (sts & EMMC_STS_BOOTACKTMO) {
#if 0
				test_timer2 = MSDC_READ32(0x10008048);
				test_timer1 = (test_timer2 - test_timer1) /6000;
				msdc_printf("[SD%d] EMMC_STS(%x): boot up ack timeout(%d ms)\n", host->id, sts,test_timer1);
				//test MT6583 ACK/DAT timeour modification test case
#endif
				msdc_printf("[%s]: [SD%d] EMMC_STS(0x%x): boot up ack timeout\n",  __func__,host->id, sts);
				err = MMC_ERR_TIMEOUT;
				goto out;
			} else if (sts & EMMC_STS_BOOTUPSTATE) {
				//msdc_printf("[%s]: [SD%d] EMMC_STS(%x): boot up mode state\n",  __func__, host->id, sts);
			} else {
				msdc_printf("[%s]: [SD%d] EMMC_STS(0x%x): boot up unexpected\n",  __func__,host->id, sts);
			}
		} while (1);
	}
	//msdc_printf("ackdis(%d) err(%d)\n",ackdis,err);
	...
	do {
		...
		if (sts & EMMC_STS_BOOTDATRCV) {
			msdc_printf("[%s]: [SD%d] EMMC_STS(0x%x): boot dat received\n",  __func__,host->id, sts);
			break;
		}
		if (sts & EMMC_STS_BOOTCRCERR) {
			msdc_printf("[%s]: [SD%d] EMMC_STS(0x%x): boot up data crc error\n",  __func__,host->id, sts);
			err = MMC_ERR_BADCRC;
			goto out;
			...
			test_timer2 = MSDC_READ32(0x10008048);
			test_timer1 = (test_timer2 - test_timer1) /6000;
			msdc_printf("[%s]: [SD%d] EMMC_STS(%x): boot up data timeout(%d s)\n",  __func__,host->id, sts,test_timer1);
			//test MT6583 ACK/DAT timeour modification test case
#endif
			msdc_printf("[%s]: [SD%d] EMMC_STS(0x%x): boot up data timeout\n",  __func__,host->id, sts);
			err = MMC_ERR_TIMEOUT;
			goto out;
		}
	} while (1);
...
int msdc_emmc_boot_read(struct mmc_host *host, u64 size, u32 *to, int read_mode)
{
	...
	if (read_mode == MSDC_MODE_PIO) {
		...
		while (size) {
			sts = MSDC_READ32(EMMC_STS);
			if (sts & EMMC_STS_BOOTCRCERR) {
				msdc_printf("[SD%d] EMMC_STS(0x%x): boot up data crc error\n", host->id, sts);
				err = MMC_ERR_BADCRC;
				goto out;
			} else if (sts & EMMC_STS_BOOTDATTMO) {
				msdc_printf("[SD%d] EMMC_STS(0x%x): boot up data timeout error\n", host->id, sts);
				err = MMC_ERR_TIMEOUT;
				goto out;
			}
			/* Note. RXFIFO count would be aligned to 4-bytes alignment size */
			if ((size >=  MSDC_FIFO_THD) && (MSDC_RXFIFOCNT() >= MSDC_FIFO_THD)) {
				...
				do {
#ifdef MTK_MSDC_DUMP_FIFO
					MSDC_EMMC50_CFG_CRC_STS_SELmsdc_printf("0x%x ",MSDC_FIFO_READ32());
#else
					*to++ = MSDC_FIFO_READ32();
#endif
				} while (--left);
...
out:
		if (err) {
			msdc_printf("[SD%d] EMMC_BOOT: read boot code fail(%d), FIFOCNT=%d\n",
		...
		while (left_sz) {
			...
			u32 base = host->base;
			cfg->xfersz = xfer_sz;
			//msdc_printf("to (0x%x) xfer_sz(0x%x)\n",to,xfer_sz);
			...
			MSDC_DMA_ON();
			/*msdc_printf("xfer_sz(%d),left_sz(%d)\n", (u32)xfer_sz, (u32)left_sz);*/
		...
		if (derr != MMC_ERR_NONE) {
			msdc_printf("[SD%d] EMMC boot read error(%d)\n", host->id,derr);
			msdc_abort_handler(host, 1);
		}
	}
	...
	switch (reset) {
		...
		case EMMC_BOOT_PRE_IDLE_CMD:
			...
			if (status & MSDC_INT_CMDTMO) {
				msdc_printf("[SD%d] CMD0:ERR(CMDTO)\n", host->id);
			}
...
int msdc_init(int id, struct mmc_host *host, int clksrc, int mode)
{
	...
	msdc_printf("[%s]: msdc%d Host controller intialization start \n", __func__, id);
	...
	//msdc_printf("set up PWR GPIO for MSDC\n");
#endif
	...
	/* Turn on to match sw status and hw status
	   since VEMC/VMC/VMCH are default on */
	...
	/* Commentted out and-then moved into msdc_init_tune_path
	MSDC_CLR_BIT32(MSDC_PATCH_BIT2, MSDC_PB2_CFGCRCSTS);
	MSDC_SET_BIT32(MSDC_PATCH_BIT2, MSDC_PB2_CFGRESP);
	*/
...
#if defined(FEATURE_ASYNC_PATH_ENABLE)
	msdc_init_tune_path(host, MSDC_PATH_USE_ASYNC_FIFO, 0);
#else
	msdc_init_tune_path(host, MSDC_PATH_USE_DELAY_LINE, 0);
#endif
...
#error please turn on DCM driver before enable MSDC DCM
	...
	msdc_printf("[%s]: msdc%d Host controller intialization done\n", __func__, id);
	return 0;
}
