...
void msdc_apply_timing(unsigned int id)
{
	...
	if (priv->tuning_mode == MSDC_PATH_USE_DELAY_LINE) {
		if (id==0 && mode == 3) {
			...
			MSDC_SET_FIELD(host->base + 0x188, (0x1 << 0), 0x1);    /* EMMC50_PAD_DS_TUNE[MSDC_EMMC50_PAD_DS_TUNE_DLY1_SEL] */
			MSDC_SET_FIELD(host->base + 0x188, (0x1 << 1), 0x0);    /* EMMC50_PAD_DS_TUNE[MSDC_EMMC50_PAD_DS_TUNE_DLY2_SEL] */
			MSDC_SET_FIELD(host->base + 0x188, (0x1f << 2), 0x6);   /* EMMC50_PAD_DS_TUNE[MSDC_EMMC50_PAD_DS_TUNE_DLY1] */
			MSDC_SET_FIELD(host->base + 0x188, (0x1f << 7), 0x0);   /* EMMC50_PAD_DS_TUNE[MSDC_EMMC50_PAD_DS_TUNE_DLY2] */
			MSDC_SET_FIELD(host->base + 0x188, (0x1f << 12), 0x17); /* EMMC50_PAD_DS_TUNE[MSDC_EMMC50_PAD_DS_TUNE_DLY3] */
			MSDC_SET_FIELD(host->base + 0x4, (0x1 << 2), 0);        /* MSDC_IOCON[MSDC_IOCON_R_D_SMPL] */
			MSDC_SET_FIELD(host->base + 0xf0, (0x3f << 16), 0x2C);  /* PAD_TUNE[MSDC_PAD_TUNE_CMDRDLY] */
			MSDC_SET_FIELD(host->base + 0xf4, (0x3f << 16), 0x0);   /* PAD_TUNE[MSDC_PAD_TUNE_CMDRRDLY] */
			MSDC_SET_FIELD(host->base + 0xf0, (0x3f << 8), 0x0);    /* PAD_TUNE[MSDC_PAD_TUNE_DATRDLY] */
			MSDC_SET_FIELD(host->base + 0xf4, (0x3f << 8), 0x0);    /* PAD_TUNE[MSDC_PAD_TUNE_DATRRDLY] */
			MSDC_SET_FIELD(host->base + 0x18C, (0x1f << 6), 0xA);   /* PAD_TUNE[MSDC_PAD_TUNE_DATRRDLY] */
		} else if (id==0) {
			MSDC_SET_FIELD(host->base + 0xb0, (0x7 <<7), 0); /* PATCH_BIT0[MSDC_PB0_INT_DAT_LATCH_CK_SEL] */
			MSDC_SET_FIELD(host->base + 0xb0, (0x1F <<10), 0); /* PATCH_BIT0[MSDC_PB0_CKGEN_MSDC_DLY_SEL] */
			MSDC_SET_FIELD(host->base + 0xb4, (0x7 <<3), 0x1); /* PATCH_BIT1[MSDC_PB1_CMD_RSP_TA_CNTR] */
			MSDC_SET_FIELD(host->base + 0xb4, (0x7 <<0), 0x1); /* PATCH_BIT1[MSDC_PB1_WRDAT_CRCS_TA_CNTR] */
			MSDC_SET_FIELD(host->base + 0x4, (0x1 << 1), 0x0); /* MSDC_IOCON[MSDC_IOCON_RSPL] */
			MSDC_SET_FIELD(host->base + 0xf0, (0x1f << 16), 0xf); /* PAD_TUNE[MSDC_PAD_TUNE_CMDRDLY] */
			MSDC_SET_FIELD(host->base + 0xf0, (0x1f << 22), 0x0); /* PAD_TUNE[MSDC_PAD_TUNE_CMDRRDLY] */
			MSDC_SET_FIELD(host->base + 0xb4, (0x7 << 0), 0x1); /* PATCH_BIT1[MSDC_PB1_WRDAT_CRCS_TA_CNTR] */
			MSDC_SET_FIELD(host->base + 0xf0, (0x1f << 0), 0x0);    /* PAD_TUNE[MSDC_PAD_TUNE_DATWRDLY] */
			MSDC_SET_FIELD(host->base + 0x4, (0x1 << 8), 0x1);  /* MSDC_IOCON[MSDC_IOCON_W_DSPL] */
			//MSDC_SET_FIELD(host->base + 0xf8, (0x1f << 24), 0x11);    /* DAT_RD_DLY0[MSDC_DAT_RDDLY0_D0] */
			MSDC_SET_FIELD(host->base + 0xf0, (0x1f << 8), 0x16);   /* PAD_TUNE[MSDC_PAD_TUNE_DATRRDLY] */
			MSDC_SET_FIELD(host->base + 0xf8, 0xffffffff, 0x16161616);/* DAT_RD_DLY0[MSDC_DAT_RDDLY0] */
		...
		if (id==0 && mode == 3) {
			...
			MSDC_SET_FIELD(host->base + 0xb0, (0x7 <<7), 0x1); /* PATCH_BIT0[MSDC_PB0_INT_DAT_LATCH_CK_SEL] */
...
int mmc_switch_part(u32 part_id)
{
	...
	if (err)
		msdc_printf("[%s]: failed to switch part_id:%d, part:%d, err=%d\n", __func__, part_id, part, err);
	...
	if (err == MMC_ERR_NONE) {
		...
		if (part_config == 1) {
			...
			*bootpart = EMMC_PART_USER;
			...
			msdc_printf("[PL MTK] fail to get boot part\n");
			return -1;
		}
	}
...
u32 mmc_init_device(void)
{
	...
	if (!blkdev_get(BOOTDEV_SDMMC)) {
		...
		if (ret != 0) {
			msdc_printf("[SD0] init card failed\n");
			return ret;
		}
...
int mmc_bread_boot(blkdev_t *bdev, u32 blknr, u32 blks, u8 *buf)
{
	...
	if (bdev && bdev->priv) {
		...
		msdc_printf("[%s]: invalid bdev, force the host id to 0\n", __func__);
		id = 0;
	}

	if (mmc_get_boot_part(&pid)) {
		msdc_printf("[%s]: invalid pid=%d\n", __func__, pid);
		return -1;
	}
...
int mmc_legacy_init(int verbose)
{
	...
	if (err == MMC_ERR_NONE && !boot_dev_found) {
		...
		if (host->boot_type == RAW_BOOT) {
			...
			boot_dev_found = 1;
			msdc_printf("[SD%d] boot device found\n", id);
		} else if (host->boot_type == FAT_BOOT) {
#if (CONFIG_COMMANDS & CFG_CMD_FAT)
			if (0 == fat_register_device(bdev, 1)) {
				boot_dev_found = 1;
				msdc_printf("[SD%d] FAT partition found\n", id);
			}
#endif
		}
	}
...
static int __mmc_do_erase(struct mmc_host *host,struct mmc_card *card, u64 start_addr, u64 len)
{
	...
	if (end_addr/card->blklen > card->nblks - 1) {
		msdc_printf("[MSDC%d]Erase address out of range! start<0x%llx>,len<0x%llx>,card_nblks<0x%x>\n",host->id, start_addr, len, card->nblks);
		return MMC_ERR_INVALID;
	}
resend:
	...
	if (err != MMC_ERR_NONE) {
		msdc_printf("[MSDC%d]Fail in read card\n", host->id);
		goto out;
	}

	err = mmc_erase_start(card, start_addr);
	if (err != MMC_ERR_NONE) {
		msdc_printf("[MSDC%d]Set erase start addrees 0x%llx failed,Err<%d>\n", host->id, start_addr, err);

		if (err == MMC_ERR_ERASE_SEQ) {
			msdc_printf("[MSDC%d]Erase sequence error, retry erase.\n", host->id);
			goto resend;
		} else {
			goto out;
		}
	...
	if (err != MMC_ERR_NONE) {
		msdc_printf("[MSDC%d]Set erase end addrees 0x%llx + 0x%llx failed,Err<%d>\n", host->id, start_addr, len, err);
	...
	if (err != MMC_ERR_NONE) {
		msdc_printf("[MSDC%d]Set erase <0x%llx - 0x%llx> failed,Err<%d>\n", host->id, start_addr, start_addr + len, err);
		goto out;
	}

	msdc_printf("[MSDC%d]0x%llx - 0x%llx Erased\n", host->id, start_addr, start_addr + len);
out:
	return err;
}
...
int mmc_do_erase(int dev_num,u64 start_addr,u64 len,u32 part_id)
{
	...
	u32 err;
	if ((!card) || (!host) ) {
		msdc_printf("[mmc_do_erase] card or host is NULL\n");
		return MMC_ERR_INVALID;
	}

	if (!len ) {
		msdc_printf("[MSDC%d] invalid erase size! len<0x%llx>\n",host->id,len);
		return MMC_ERR_INVALID;
	}
	if ((start_addr % card->blklen) || (len % card->blklen)) {
		msdc_printf("[MSDC%d] non-alignment erase address! start<0x%llx>,len<0x%llx>,card_nblks<0x%x>\n",host->id,start_addr,len,card->nblks);
		return MMC_ERR_INVALID;
	}
	...
	if ((err = mmc_switch_part(part_id))) {
		msdc_printf("[MSDC%d] mmc swtich failed.part<%d> error <%d> \n", host->id, part_id, err);
		return err;
	}

	if ((err = __mmc_do_erase(host, card, start_addr, len))) {
		msdc_printf("[MSDC%d] mmc erase failed.error <%d> \n",host->id, err);
		return err;
	}
