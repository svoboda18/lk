...
kal_uint32 mt6353_upmu_get_rgs_chrdet(void)
{
	kal_uint32 ret=0;
	kal_uint32 val=0;
...
#else
	tmp32 = mt6353_upmu_get_rgs_chrdet();
#endif
...
U32 pmic_init (void)
{
	...
	dprintf(INFO, "[pmic_init] MT6353 CHIP Code = 0x%x\n", get_mt6353_pmic_chip_version());
	/*pmic_auxadc_debug(2);*/
	...
	//mt6311_driver_probe();
#endif
	ret = pmic_read_interface(MT6353_TOP_RST_MISC, &val, 0xFFFF, 0x0);
...
#ifdef MT6351
	ret_val = pmic_read_interface(PMIC_RG_BATON_HT_EN_ADDR, &val1, 0xFFFF, 0x0);
...
#endif
	/*pmic_auxadc_debug(1);*/
	//ret=pmic_config_interface(MT6353_TOP_CLKSQ_SET,(1<<2),0xffff,0);
...
void vibr_Enable_HW(void)
{
	...
	//mt6353_upmu_set_rg_vibr_vosel(0x5); // 0x5: 2.8V, 0x6: 3V, 0x7: 3.3V


	//mt6353_upmu_set_rg_vibr_sw_mode(0);
	//mt6353_upmu_set_rg_vibr_fr_ori(1);

	pmic_set_register_value(PMIC_LDO_VIBR_EN,1);
	//mt6353_upmu_set_rg_vibr_en(1);
}

void vibr_Disable_HW(void)
{
	pmic_set_register_value(PMIC_LDO_VIBR_EN,0);
	//mt6353_upmu_set_rg_vibr_en(0);
}
...
void lcm_Enable_HW(int powerVolt)
{
	...
	pmic_set_register_value(PMIC_LDO_VCAMA_EN, 1);
}

void lcm_Disable_HW(void)
{
	dprintf(CRITICAL, "[lcm_Disable_HW]\n");
	pmic_set_register_value(PMIC_LDO_VCAMA_EN, 0);
}
