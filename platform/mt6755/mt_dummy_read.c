...
int target_fdt_dram_dummy_read(void *fdt, unsigned int rank_num)
{
	...
	for (j = 0; j < orig_dram_info->rank_num; j++) {
		rsv_mem_size_property[j].size_lo = 0;
		rsv_mem_size_property[j].size_hi = 0;
		for (i = 0; i < mblock_info->mblock_num; i++) {
			...
			/*rsv_mem_reg_property[j].size_hi = cpu_to_fdt32(0);
			rsv_mem_reg_property[j].size_lo = cpu_to_fdt32(0x00001000);*/
...
{
		dprintf(CRITICAL,"LK EMI MPUS=0x%x; MPUT=0x%x\n", DRV_Reg32(0x102031F0), DRV_Reg32(0x102031F8));
}
