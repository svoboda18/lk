...
static mout_t mout_map[DDP_MOUT_NUM] = {
	// OVL_MOUT
	{DISP_MODULE_OVL0,   {{DISP_MODULE_COLOR0,1<<0}, {DISP_MODULE_WDMA0,1<<1}, {-1,0}}, (volatile unsigned int *)DISP_REG_CONFIG_DISP_OVL0_MOUT_EN,   0},
	{DISP_MODULE_OVL1,   {{DISP_MODULE_RDMA1,1<<0}, {DISP_MODULE_WDMA1,1<<1}, {DISP_MODULE_OVL0,1<<2},{-1,0}}, (volatile unsigned int *)DISP_REG_CONFIG_DISP_OVL1_MOUT_EN,   0},
	// DITHER0_MOUT
	{DISP_MODULE_DITHER, {{DISP_MODULE_RDMA0,1<<0}, {DISP_MODULE_UFOE,1<<1},{DISP_MODULE_WDMA0,1<<2}, {-1,0}},   (volatile unsigned int *)DISP_REG_CONFIG_DISP_DITHER_MOUT_EN,    0},
	// UFOE_MOUT
	{DISP_MODULE_UFOE,   {{DISP_MODULE_DSI0,1<<0}, {DISP_MODULE_DPI,1<<1}, {DISP_MODULE_WDMA0,1<<2}, {-1,0}}, (volatile unsigned int *)DISP_REG_CONFIG_DISP_UFOE_MOUT_EN, 0},
};

static sel_t sel_out_map[DDP_SEL_OUT_NUM] = {
	// OVL0_SOUT
	{DISP_MODULE_OVL0,  {DISP_MODULE_COLOR0, DISP_MODULE_OVL1_2L, -1},  (volatile unsigned int *)DISP_REG_CONFIG_DISP_OVL0_SOUT_SEL_IN, 0},
	// OVL1_SOUT
	{DISP_MODULE_OVL1_2L, {DISP_MODULE_OVL1, DISP_MODULE_COLOR0, -1},   (volatile unsigned int*)DISP_REG_CONFIG_DISP_OVL1_SOUT_SEL_IN, 0},
	// RDMA_SOUT
	{DISP_MODULE_RDMA0,  {DISP_MODULE_UFOE, DISP_MODULE_COLOR0,DISP_MODULE_DSI0,DISP_MODULE_DPI, -1},   (volatile unsigned int *)DISP_REG_CONFIG_DISP_RDMA0_SOUT_SEL_IN, 0},
	{DISP_MODULE_RDMA1,  {DISP_MODULE_DSI0, DISP_MODULE_DPI, -1}, (volatile unsigned int *)DISP_REG_CONFIG_DISP_RDMA1_SOUT_SEL_IN, 0},
};
...
static sel_t sel_in_map[DDP_SEL_IN_NUM] = {
	// OVL0_SEL
	{DISP_MODULE_OVL0, {DISP_MODULE_COLOR0, DISP_MODULE_OVL1, DISP_MODULE_OVL1_2L, -1}, (volatile unsigned int *)DISP_REG_CONFIG_DISP_OVL0_SEL_IN, 0},

	// COLOR_SEL
	{DISP_MODULE_COLOR0, {DISP_MODULE_RDMA0, DISP_MODULE_OVL0, -1},   (volatile unsigned int *)DISP_REG_CONFIG_DISP_COLOR0_SEL_IN, 0},

	//UFOE_SEL
	{DISP_MODULE_UFOE,  {DISP_MODULE_RDMA0, DISP_MODULE_DITHER, -1}, (volatile unsigned int *)DISP_REG_CONFIG_DISP_UFOE_SEL_IN, 0},

	//DSI0_SEL
	{DISP_MODULE_DSI0, {DISP_MODULE_UFOE, DISP_MODULE_RDMA0,DISP_MODULE_RDMA1, -1},   (volatile unsigned int *)DISP_REG_CONFIG_DSI0_SEL_IN, 0},

	//DPI0_SEL
	{DISP_MODULE_DPI, {DISP_MODULE_UFOE, DISP_MODULE_RDMA0,DISP_MODULE_RDMA1, -1},    (volatile unsigned int *)DISP_REG_CONFIG_DPI0_SEL_IN, 0},

	// WDMA_SEL
	{DISP_MODULE_WDMA0, {DISP_MODULE_OVL0,DISP_MODULE_DITHER, DISP_MODULE_UFOE,DISP_MODULE_OVL1, -1}, (volatile unsigned int *)DISP_REG_CONFIG_DISP_WDMA0_SEL_IN, 0},
};
...
static void ddp_print_scenario(DDP_SCENARIO_ENUM scenario)
{
	int i =0;
	char path[512]= {'\0'};
	unsigned int left_len = sizeof(path);
	unsigned int scenario_len = 0;
	int num = ddp_get_module_num(scenario);
	for (i=0; i< num; i++) {
		scenario_len = strlen(ddp_get_mode_name(module_list_scenario[scenario][i]));
		if (scenario_len < left_len){
			strncat(path,ddp_get_module_name(module_list_scenario[scenario][i]),
				left_len);
			left_len -= scenario_len;
		} else {
			DDPERR("%s path string array size is not enough\n", path);
			break;
		}
        }
...
#ifdef MTKFB_NO_M4U
	DISP_REG_SET(0,DISP_REG_SMI_LARB_MMU_EN,0xfffffff8);//m4u disable
#endif
	return 0;
}
