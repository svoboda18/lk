...
static int brightness_set_pmic(enum mt65xx_led_pmic pmic_type, enum led_brightness level)
{
	...
	if (pmic_type == MT65XX_LED_PMIC_NLED_ISINK0) {
...
#else
			pmic_set_register_value(MT6351_PMIC_RG_DRV_32K_CK_PDN,0x0); // Disable power down
			pmic_set_register_value(MT6351_PMIC_ISINK_CH0_EN,0x1); // Turn on ISINK Channel 0
#endif
		} else {
#if defined(PMIC_CHIP_MT6353)
			pmic_set_register_value(PMIC_ISINK_CH0_EN, 0x0);    /* Turn off ISINK Channel 0 */
#else
			pmic_set_register_value(MT6351_PMIC_ISINK_CH0_EN,0x0); // Turn off ISINK Channel 0
#endif
		}
		return 0;
		...
		if (level) {
#if defined (PMIC_CHIP_MT6353)
			pmic_set_register_value(PMIC_ISINK_CH1_EN, 0x0);    /* Turn on ISINK Channel 1 */
#else
			pmic_set_register_value(MT6351_PMIC_RG_DRV_32K_CK_PDN,0x0); // Disable power down
			pmic_set_register_value(MT6351_PMIC_ISINK_CH1_EN,0x1); // Turn on ISINK Channel 0
#endif
		} else {
#if defined (PMIC_CHIP_MT6353)
			pmic_set_register_value(PMIC_ISINK_CH1_EN, 0x0);    /* Turn off ISINK Channel 1 */
#else
			pmic_set_register_value(MT6351_PMIC_ISINK_CH1_EN,0x0); // Turn off ISINK Channel 0
#endif
		}
		return 0;
	}
