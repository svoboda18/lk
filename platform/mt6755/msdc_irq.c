...
#include "isrentry.h"
//#include "inc/Jade/x_define_irq.h"
//For CTP only
typedef void (*irq_handler_t)(void);
static const u32 msdc_irq_line[MSDC_MAX_NUM] = {MSDC0_IRQ_B_ID+8, MSDC1_IRQ_B_ID+8};
...
static void msdc_irq_handler(void *data)
{
	...
	if (intsts & MSDC_INT_CDSC) {
		/* card detection */
		in_iqr++;
		msdc_printf("in_irq(%d) intsts(0x%x)",in_iqr,intsts);
...
u32 msdc_lk_intr_wait(struct mmc_host *host, u32 intrs)
{
	...
	if (ret != 0) {
		msdc_printf("[%s]: failed to get event\n", __func__);
	} else {
		MSG(INT, "[%s]: get event\n", __func__);
	}
	sts = g_int_status;
...
u32 msdc_intr_wait(struct mmc_host *host, u32 intrs)
{
...
#else
	{
		...
		if (tmo == 0) {
			msdc_printf("[SD%d] Wait INT timeout\n", host->id);
			MSDC_RESET();
		}
	}
