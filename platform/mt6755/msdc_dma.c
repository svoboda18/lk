...
void msdc_dump_dma_desc(struct mmc_host *host)
{
	...
	if (MSG_EVT_MASK & MSG_EVT_DMA) {
		for (i = 0; i < priv->alloc_gpd; i++) {
			...
			msdc_printf("[SD%d] GD[%d](0x%xh): %xh %xh %xh %xh %xh %xh %xh\n",
		...
		for (i = 0; i < priv->alloc_bd; i++) {
			...
			msdc_printf("[SD%d] BD[%d](0x%xh): %xh %xh %xh %xh\n",
...
int msdc_sg_init(struct scatterlist *sg, void *buf, u64 buflen)
{
	...
	BUG_ON(buflen >(u64) MAX_SG_POOL_SZ * MAX_SG_BUF_SZ);
...
int msdc_dma_cmd(struct mmc_host *host, struct dma_config *cfg, struct mmc_command *cmd)
{
	...
	MSG(CMD, "[SD%d] DMA CMD(%d), AUTOCMD12(%d), AUTOCMD23(%d)\n",
...
int msdc_dma_config(struct mmc_host *host, struct dma_config *cfg)
{
	...
	u32 sglen = cfg->sglen;
	u32 j;
#ifdef MSDC_ENABLE_ENH_DMA_MODE
	u32 i, num, bdlen, arg, xfersz;
#endif
...
int msdc_dma_wait_done(struct mmc_host *host, u32 timeout)
{
	...
	do {
...
#endif
		{
			if (cfg->inboot && cfg->mode == MSDC_MODE_DMA_BASIC) {
				//msdc_printf("Polling DMA tranfer done in eMMC boot mode\n");
				if (status & MSDC_INT_DXFER_DONE)
					break;
			}
	...
	do {
		...
		if (status & MSDC_INT_GPDCSERR) {
			...
			error = MMC_ERR_BADCRC;
			break;
		} else if ( (MSDC_READ32(MSDC_DMA_CFG) & MSDC_DMA_CFG_STS) == 0) {
			break;
		}
	} while (1);
end:
	if (error) {
		MSDC_RESET();
		msdc_printf("msdc_dma_wait_done status(%xh)\n", status);
	}
...

{
	...
	if (cfg->mode == MSDC_MODE_DMA_ENHANCED) {
		...
		msdc_dma_init(host, cfg, (void*)buf, totalsz);
	...
	if (derr != MMC_ERR_NONE) {
		msdc_printf("[SD%d] <CMD%d> IO DMA data error (%d)\n", host->id, cmd.opcode & ~SD_CMD_BIT, derr);
		msdc_abort(host);
	}
...
int msdc_dma_transfer(struct mmc_host *host, struct mmc_command *cmd, struct mmc_data *data)
{
	...
	totalsz = (u64) nblks * blksz;
	BUG_ON(totalsz > MAX_DMA_TRAN_SIZE);
	...
	if (derr != MMC_ERR_NONE) {
		...
		msdc_printf("[SD%d] <CMD%d> DMA data error (%d)\n", host->id, cmd->opcode, derr);
		msdc_abort_handler(host, 1);
...
#if defined(FEATURE_NONBLOCKING_RW) || defined(FEATURE_MMC_CMDQ)
int msdc_dma_nonblocking_check_done(struct mmc_host *host, struct mmc_cmdq_task *cmdq_taskp)
{
	int err= MMC_ERR_NONE, derr = MMC_ERR_NONE, cmderr= MMC_ERR_NONE;
	...
	err = derr = msdc_dma_wait_done(host, 0xFFFFFFFF);
	msdc_dma_stop(host);
	msdc_flush_membuf(cmdq_taskp->buf, cmdq_taskp->blkcnt * host->blklen);
	msdc_printf("cmdq task buf at %x, %d\n",cmdq_taskp->buf, cmdq_taskp->blkcnt);

	/*
	if (host->mmc_cmdq_req[host->mmc_cmdq_first_task].blkcnt>1 && (priv->autocmd == 0)) {
	    cmderr = msdc_cmd_stop(host, cmd);
	}*/

	host->mmc_cmdq_first_task++;
	if ( host->mmc_cmdq_first_task==MMC_MAX_QUEUE_DEPTH_USER ) {
		host->mmc_cmdq_first_task=0;
	}
