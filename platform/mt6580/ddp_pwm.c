    ...
    if (cust_led_list) {
        ...
        if ((strcmp(cust->name,"lcd-backlight") == 0) && (cust->mode == MT65XX_LED_MODE_CUST_BLS_PWM)) {
            ...
            if (config_data->clock_source >= 0 && config_data->clock_source <= 1) {
                unsigned int reg_cg = PWM_REG_GET(CLK_GATING_CTRL);
                unsigned int reg_val = PWM_REG_GET(CLK_MUX_SEL0);
                    PWM_REG_SET(CLK_GATING_CTRL, (reg_cg | 0x1));
                    if (config_data->clock_source == 0)
                        PWM_REG_SET(CLK_MUX_SEL0, (reg_val & 0xfffbffff));
                    else if (config_data->clock_source == 1)
                        PWM_REG_SET(CLK_MUX_SEL0, (reg_val | 0x00040000));
                    PWM_REG_SET(CLK_GATING_CTRL, (reg_cg & 0xfffffffe));
                PWM_MSG("disp_pwm_init : CLK_MUX_SEL0 0x%x => 0x%x", reg_val, PWM_REG_GET(CLK_MUX_SEL0));
            }
