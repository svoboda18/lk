...
static int _build_path_direct_link(void)
{
	...
	pgc->dpmgr_handle = dpmgr_create_path(DDP_SCENARIO_PRIMARY_DISP, NULL);
	if (pgc->dpmgr_handle) {
...
unsigned int mt_disp_parse_dfo_setting(void)
{
	...
	if (buffer != NULL) {
		for (i = 0; i< (sizeof(disp_dfo_setting)/sizeof(disp_dfo_item_t)); i++) {
			...
			DISPMSG("disp_dfo_setting[%d].name = [%s], name_lenght = %d\n", i, ptr, k);

			do { k--; } while (((*ptr) != '\0') && ((*ptr++) != ',') && (k >= 0));

			do {
				tmp[j++] = *ptr++;
				k--;
			} while (((*ptr) != '\0') && (*ptr != ',') && (j < sizeof(tmp)/sizeof(tmp[0])) && (k >= 0));
...
{
	/* size = pattern_w * pattern_h * 2(top/bottom) * BPP */
	return DISP_GetScreenWidth() * DISP_GetScreenHeight() * 4;
}
...
static UINT32 disp_fb_pages = 3;    //three for framebuffer, one for dim layer
...
#ifdef MTK_ROUND_CORNER_SUPPORT
	LCM_ROUND_CORNER *corner_params;
	char *pixel;
#endif
...
#ifdef MTK_ROUND_CORNER_SUPPORT
		corner_params = primary_display_get_corner_params();
