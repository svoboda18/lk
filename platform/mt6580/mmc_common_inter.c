...
int mmc_switch_part(u32 part_id)
{
    ...
    if (err)
        MSG(ERR, "[%s]: failed to switch part_id:%d, part:%d, err=%d\n", __func__, part_id, part, err);
    return err;
}
    ...
    if (err == MMC_ERR_NONE) {
        ...
        if (part_config == 1) {
            ...
            *bootpart = EMMC_PART_USER;
            ...
            MSG(ERR, "[PL MTK] fail to get boot part\n");
            return -1;
        }
    }
...
int mmc_set_boot_part(u32 bootpart)
{
	...
	if (((u32)cur_bootpart) == bootpart)
		goto out;
	printf("[SD0] Current boot part is %d\n", cur_bootpart);

	part_config = part_config & (~(0x7 << 3)) | (bootpart << 3);
	printf("[SD0] Set boot part as %d\n", bootpart);
...
int mmc_bread_boot(blkdev_t *bdev, u32 blknr, u32 blks, u8 *buf)
{
    ...
    if(bdev && bdev->priv){
        ...
        MSG(ERR, "[%s]: invalid bdev, force the host id to 0\n", __func__, pid);
        id = 0;
    }

    if(mmc_get_boot_part(&pid)){
        MSG(ERR, "[%s]: invalid pid=%d\n", __func__, pid);
        return -1;
    }
...
static int __mmc_do_erase(struct mmc_host *host,struct mmc_card *card, u64 start_addr, u64 len)
{
    ...
    if (end_addr/card->blklen > (card->nblks - 1)) {
        MSG(ERR, "[MSDC%d]Erase address out of range! start<0x%llx>,len<0x%llx>,card_nblks<0x%x>\n",host->id, start_addr, len, card->nblks);
        return MMC_ERR_INVALID;
    }
...
resend:
    ...
    if (err != MMC_ERR_NONE) {
        MSG(ERR, "[MSDC%d]Fail in read card\n", host->id);
        goto out;
    }

    err = mmc_erase_start(card, start_addr);
    if (err != MMC_ERR_NONE) {
        MSG(ERR, "[MSDC%d]Set erase start addrees 0x%llx failed,Err<%d>\n", host->id, start_addr, err);

        if (err == MMC_ERR_ERASE_SEQ) {
            MSG(ERR, "[MSDC%d]Erase sequence error, retry erase.\n", host->id);
            goto resend;
        } else {
            goto out;
        }
    ...
    if (err != MMC_ERR_NONE) {
        MSG(ERR, "[MSDC%d]Set erase end addrees 0x%llx + 0x%llx failed,Err<%d>\n", host->id, start_addr, len, err);
    ...
    if (err != MMC_ERR_NONE) {
        MSG(ERR "[MSDC%d]Set erase <0x%llx - 0x%llx> failed,Err<%d>\n", host->id, start_addr, start_addr + len, err);
        goto out;
    }
...
int mmc_do_erase(int dev_num,u64 start_addr,u64 len,u32 part_id)
{
    ...
    u32 err;
    if ((!card) || (!host) ) {
        MSG(ERR, "[mmc_do_erase] card or host is NULL\n");
        return MMC_ERR_INVALID;
    }

    if (!len ){
        MSG(ERR, "[MSDC%d] invalid erase size! len<0x%llx>\n",host->id,len);
        return MMC_ERR_INVALID;
    }
    if ((start_addr % card->blklen) || (len % card->blklen)) {
        MSG(ERR, "[MSDC%d] non-alignment erase address! start<0x%llx>,len<0x%llx>,card_nblks<0x%x>\n",host->id,start_addr,len,card->nblks);
        return MMC_ERR_INVALID;
    }
    ...
    if ((err = mmc_switch_part(part_id))) {
        MSG(ERR, "[MSDC%d] mmc swtich failed.part<%d> error <%d> \n", host->id, part_id, err);
        return err;
    }

    if ((err = __mmc_do_erase(host, card, start_addr, len))) {
        MSG(ERR "[MSDC%d] mmc erase failed.error <%d> \n",host->id, err);
        return err;
    }
