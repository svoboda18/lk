...
static void mmc_decode_ext_csd(struct mmc_card *card)
{
    ...
    if (ext_csd[EXT_CSD_ERASE_GRP_DEF] & EXT_CSD_ERASE_GRP_DEF_EN) {
        card->wp_size = card->ext_csd.hc_wp_grp_sz >> 9; /* blocks */
        MSG(INF, "EXT_CSD_ERASE_GRP_DEF is On, wp_size = %d MB\n", card->wp_size >> 11);
    } else {
        card->wp_size =(card->csd.write_prot_grpsz+1) * card->csd.erase_sctsz;
        MSG(INF, "EXT_CSD_ERASE_GRP_DEF is Off, wp_size = %d MB, csd.write_prot_grpsz = %d,csd.erase_sctsz = %d\n",
            card->wp_size >> 11, card->csd.write_prot_grpsz, card->csd.erase_sctsz);
    }
    MSG(INF, "[%s]: mmc_set_wp_size %dMB\n", __func__, card->wp_size >> 11);
#endif
...
static int mmc_read_switch(struct mmc_host *host, struct mmc_card *card)
{
    ...
    if (status[9] & 0x01) {
        MSG(INF, "[SD%d] Support: Type-B Drv\n", host->id);
    }
    if (status[9] & 0x02) {
        MSG(INF, "[SD%d] Support: Type-A Drv\n", host->id);
    }
    if (status[9] & 0x04) {
        MSG(INF, "[SD%d] Support: Type-C Drv\n", host->id);
    }
    if (status[9] & 0x08) {
        MSG(INF, "[SD%d] Support: Type-D Drv\n", host->id);
    }
    if (status[7] & 0x01) {
        MSG(INF, "[SD%d] Support: 200mA current limit\n", host->id);
    }
    if (status[7] & 0x02) {
        MSG(INF, "[SD%d] Support: 400mA current limit\n", host->id);
    }
    if (status[7] & 0x04) {
        MSG(INF, "[SD%d] Support: 600mA current limit\n", host->id);
    }
    if (status[7] & 0x08) {
        MSG(INF, "[SD%d] Support: 800mA current limit\n", host->id);
    }
#endif
...
static int mmc_dev_bwrite(struct mmc_card *card, unsigned long blknr, u32 blkcnt, u8 *src)
{
    ...
    do {
        ...
        if (!tune) {
            ...
            #if defined(MSDC0_EMMC50_SUPPORT)
            if (mmc_card_mmc(card) && mmc_card_hs400(card)){
                err = msdc_tune_rw_hs400(host, (uchar *) dst, (ulong) orisrc, blkcnt, 1);
            }
            else
            #endif
            {
...
int mmc_init_mem_card(struct mmc_host *host, struct mmc_card *card, u32 ocr)
{
    ...
    if (mmc_card_sd(card)) {
        ...
        if ((host->caps & MMC_CAP_EMMC_HS200) && !(host->ocr_avail & 0x80)){//(MMC_VDD_17_18 | MMC_VDD_18_19))){
            ...
            MSG(WRN, "[SD%d] can not switch to HS200:Host voltage not support!\n",id);
        }
...
int mmc_init_card(struct mmc_host *host, struct mmc_card *card)
{
    ...
    if (mmc_send_io_op_cond(host, 0, &ocr) == MMC_ERR_NONE) {
        ...
        if (err != MMC_ERR_NONE) {
            MSG(ERR, "[SD%d] Fail in init sdio card\n", id);
            goto out;
        }
        /* no memory present */
        if ((ocr & 0x08000000) == 0) {
    ...
    if (err != MMC_ERR_NONE) {
        ...
        if (err != MMC_ERR_NONE) {
            MSG(ERR, "[SD%d] Fail in MMC_CMD_SEND_OP_COND/SD_ACMD_SEND_OP_COND cmd\n", id);
            goto out;
        }
...
static void emmc_r_w_compare_test()
{
    ...
    MSG(INF, "[PL][%s:%d]eMMC simp test of user parittion start\n", __func__, __LINE__);
    ...
    for (i = 0; i < PL_MMC_TEST_SIZE; i++){
        if (g_mmc_buf[i] != 0x5a){
           MSG(ERR, "[PL][%s:%d]mmc simple r/w user partition compare failed(%d is %d, not 0x5a)\n", __func__, __LINE__, i, g_mmc_buf[i]);
           break;
        }
    }
    /* write 0x40000000(1G) with 256 block(0x5a) */
...
int mmc_set_boot_wp(struct mmc_card *card, EMMC_WP_TYPE type, Region partition)
{
    ...
    if (card->ext_csd.boot_wp == value){
        MSG(INF, "[%s]: EXT_CSD_BOOT_WP alread set \n",__func__);
    } else {
        err = mmc_switch(card->host, card, EXT_CSD_CMD_SET_NORMAL,
            EXT_CSD_BOOT_WP, value);
        if (err) {
            MSG(INF, "[%s]: mmc_switch err %d\n", __func__, err);
            goto out;
        }
    }
    ...
    u32 blkcnt, Region partition)
{
    ...
    if (card->ext_csd.usr_wp & US_PWR_WP_DIS) {
        MSG(ERR, "[%s]: US_PWR_WP_DIS is err set \n", __func__);
        goto out;
    }
    ...
    if (card->ext_csd.usr_wp == value) {
        MSG(ERR, "[%s]: EXT_CSD_USR_WP alread set \n", __func__);
    } else {
        err = mmc_switch(card->host, card, EXT_CSD_CMD_SET_NORMAL,
            EXT_CSD_USR_WP, value);
        if (err) {
            MSG(ERR, "[%s]: mmc_switch err %d\n", __func__, err);
            goto out;
        }
    }
    ...
    err = mmc_set_write_protect_by_group(card,
                        partition, blknr, blkcnt, type);
    if (err) {
    ...
    if (type == WP_POWER_ON) {
        if (partition == EMMC_PART_USER) {
            ...
            if (blknr % card->wp_size || blkcnt % card->wp_size
                || (blknr + blkcnt > card->nblks)) {
                MSG(ERR, "[%s]: alignment or capility error \n", __func__);
                goto done;
            }
