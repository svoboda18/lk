...
uint32_t get_policy_entry_idx(char *part_name)
{
	...
	for (idx = 0; idx < sec_policy_idx_table_sz; idx++) {
		if (0 == strcmp(part_name, g_sec_policy_idx_table[idx]))
			break;
	}
...
uint32_t get_vfy_policy(uint32_t policy_entry_idx)
{
	...
	switch (policy_entry_idx) {
		case RECOVERY_SEC_POLICY_IDX:
			img_auth_required = (SW_SEC_BOOT_LOCK == g_cur_sec_policy->boot_chk_recovery);
			break;
		case BOOT_SEC_POLICY_IDX:
		case DTBO_SEC_POLICY_IDX:
			img_auth_required = (SW_SEC_BOOT_LOCK == g_cur_sec_policy->boot_chk_bootimg);
			break;
		case LOGO_SEC_POLICY_IDX:
			img_auth_required = (SW_SEC_BOOT_LOCK == g_cur_sec_policy->boot_chk_logo);
			break;
		default:
			img_auth_required = (SW_SEC_BOOT_LOCK == g_cur_sec_policy->boot_chk_others);
			break;
	}
...
int img_auth_stor(char *part_name, char *img_name)
{
	...
	if (img_auth_required) {
		if (PART_OK == partition_exists(ab_part_name)) {
			...
			pal_log_err("[%s] %s does not exist, bypass\n", MOD, ab_part_name);
	}
