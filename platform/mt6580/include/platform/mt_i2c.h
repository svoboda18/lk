...
//#define I2C_DRIVER_IN_KERNEL
  ...
  #else
    #define I2C_CLK_DIV		10 /* frequency divisor */
    #define I2C_CLK_RATE		12500 /* TODO: Calculate from bus clock */
  #endif
...
#define I2C_2_SW_CG_BIT (0x1 << 16)

#define ENABLE_CLOCK(channel) \
	if (channel == I2C0) \
		mt_reg_sync_writel(I2C_0_SW_CG_BIT, CLR_CLK_GATING_CTRL1);\
	else if (channel == I2C1)\
		mt_reg_sync_writel(I2C_1_SW_CG_BIT, CLR_CLK_GATING_CTRL1);\
	else\
		mt_reg_sync_writel(I2C_2_SW_CG_BIT, CLR_CLK_GATING_CTRL1);

#define DISABLE_CLOCK(channel) \
	if (channel == I2C0) \
		mt_reg_sync_writel(I2C_0_SW_CG_BIT, SET_CLK_GATING_CTRL1);\
	else if (channel == I2C1)\
		mt_reg_sync_writel(I2C_1_SW_CG_BIT, SET_CLK_GATING_CTRL1);\
	else\
		mt_reg_sync_writel(I2C_2_SW_CG_BIT, SET_CLK_GATING_CTRL1);
