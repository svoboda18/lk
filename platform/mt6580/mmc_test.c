...
static reg_desc_t msdc0_reg_desc[] = {
    ...
    {OFFSET_SDC_CMD       ,        {"aaaaaaaaaaxaaaaaaaaaaaaaaaaaaaar"}, {"0000000000x00000000000000000000x"}},
    ...
    {OFFSET_MSDC_DMA_SA_HIGH4BIT,  {"aaaaxxxxxxxxxxxxxxxxxxxxxxxxxxxx"}, {"0000xxxxxxxxxxxxxxxxxxxxxxxxxxxx"}},
    ...
    {OFFSET_MSDC_DMA_LEN  ,        {"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}, {"00000000000000000000000000000000"}},
    ...
    {OFFSET_MSDC_PATCH_BIT2,       {"aaaaaaaaaaxaaaaaaaaxaaaaaaaaaaaa"}, {"1100000000x11000000x000100101000"}},
    ...
    {OFFSET_MSDC_VERSION  ,        {"rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr"}, {"01001000101000000010100000000100"}},//20140512
...
static reg_desc_t msdc1_reg_desc[] = {
    ...
    {OFFSET_MSDC_IOCON    ,        {"aaaaaaxxaaaaaaxxaaaaaaaaxxxxxxxx"}, {"000000xx000000xx0000000000xxxxxx"}},//bit[24][25] can not write(Not in MT8320 MSDC IP)
    {OFFSET_MSDC_PS       ,        {"atxxxxxxxxxxaaaattttxxxxtxxxxxxt"}, {"0xxxxxxxxxxx00001111xxxx1xxxxxx0"}},//bit[1]/bit[16-20]/bit[24]/bit[31] default to 0
    {OFFSET_MSDC_INT      ,        {"ccxccccccccrccccccccccccxxxxxxxx"}, {"00x000000000000000000000xxxxxxxx"}},
    {OFFSET_MSDC_INTEN    ,        {"aaxaaaaaaaaaaaaaaaaaxxxxxxxxxxxx"}, {"00x00000000000000000xxxxxxxxxxxx"}},//bit[2] to RW, default to 0
    {OFFSET_MSDC_FIFOCS   ,        {"rrrrrrrrxxxxxxxxrrrrrrrrxxxxxxxz"}, {"xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"}},//bit[0]~[7] are variable if you use memory tool in Code visor (memory tool must be closed)
    {OFFSET_MSDC_TXDATA   ,        {"wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww"}, {"00000000000000000000000000000000"}},
    //{OFFSET_MSDC_RXDATA   ,      {"rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr"}, {"00000000000000000000000000000000"}},/* Should not be touched. */
    {OFFSET_SDC_CFG       ,        {"aaxxxxxxxxxxxxxxaaxaaaxxaaaaaaaa"}, {"00xxxxxxxxxxxxxx00x010xx00000000"}},
...
//    {OFFSET_EMMC_IOCON    ,        {"axxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"}, {"0xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"}},
    ...
    {OFFSET_MSDC_PATCH_BIT2,       {"aaaaaaaaaaxaaaaaaaaxaaaaaaaaaaaa"}, {"1000000000x11000000x000100101000"}},
...
static reg_desc_t msdc2_reg_desc[] = {
    ...
    {OFFSET_MSDC_IOCON    ,        {"aaaaaaxxaaaaaaxxaaaaaaaaxxxxxxxx"}, {"000000xx000000xx0000000000xxxxxx"}},//bit[24][25] can not write(Not in MT8320 MSDC IP)
    {OFFSET_MSDC_PS       ,        {"atxxxxxxxxxxaaaattttxxxxtxxxxxxt"}, {"00xxxxxxxxxx00001111xxxx1xxxxxxx"}},//bit[1]/bit[16-20]/bit[24]/bit[31] default to 0
    {OFFSET_MSDC_INT      ,        {"ccxccccccccrccccccccccccxxxxxxxx"}, {"00x000000000000000000000xxxxxxxx"}},
...
int mmc_test_mem_card(struct mmc_test_config *cfg)
{
    ...
    while (forever || count--) {
        ...
        if (cfg->signal_burst) {
            printf("signal_burst=%d\n", (cfg->signal_burst));
            msdc_set_signal_burst(host, 1);
        }

        if (cfg->dcm_en) {
            printf("dcm_en=%d\n", (cfg->dcm_en));
            msdc_enable_dcm(host, 1);
        }
        ...
        if (cfg->tst_single) {
            ...
            for (i = 0; i < total_blks && !result; i++) {
                ...
                ret = mmc_block_read(id, blknr + i, 1, (unsigned long*)read_buf);
                if (ret != MMC_ERR_NONE) {
                    result = -__LINE__;
                    goto failure;
                }
                if (chk_result) {
                    for (j = 0; j < blksz; j++) {
                        if (buf[j] != read_buf[j]) {
                            printf("compare,i=%d, j=%d, buf[j]=%d, read_buf(%d), buf(0x%x)\n", i, j, buf[j], read_buf[j], (unsigned int)read_buf);
                            mmc_test_dump_buff(read_buf,j,blksz);
                            result = -__LINE__;
                            goto failure;
                        }
                    }
...
int mmc_test(int argc, char *argv[])
{
    ...
    if (cfg.buf == 0){
        printf("[%s]buf size:%x excced max available dram space \n",__func__,MMC_TST_SIZE);
        result = -1;
        goto exit;
    }
  #endif
...
int mmc_wp_test()
{
    ...
    MSG(INF, "[case %d] blknr=0x%x, blkctn=0x%x, partition=%d \n",
                case_id, blknr, blkcnt, part_id);
    err = mmc_set_write_protect(dev_num, part_id, blknr, blkcnt, WP_POWER_ON);
    if (err) {
        MSG(INF, "[%s]: (case %d) wp error\n", __func__, case_id++);
    } else {
        MSG(INF, "[%s]: (case %d) wp successfully\n", __func__, case_id++);
    }
    ...
    /* partiton test case */
    #if 0
    MSG(INF, "[%s]: (partition case: 1) preloader->lk\n", __func__);
    err = partition_write_prot_set("preloader", "lk", WP_POWER_ON);
    MSG(INF, "[%s]: (partition case: 1) preloader->lk, err=%d\n", __func__, err);

    MSG(INF, "[%s]: (partition case: 2) boot->recovery\n", __func__);
    err = partition_write_prot_set("boot", "recovery", WP_POWER_ON);
    MSG(INF, "[%s]: (partition case: 2) boot->recovery, err=%d\n", __func__, err);

    MSG(INF, "[%s]: (partition case: 3) keystore->oemkeystore\n", __func__);
    err = partition_write_prot_set("keystore", "oemkeystore", WP_POWER_ON);
    MSG(INF, "[%s]: (partition case: 3) keystore->oemkeystore, err=%d\n", __func__, err);

    MSG(INF, "[%s]: (partition case: 4) oemkeystore->secro\n", __func__);
    err = partition_write_prot_set("oemkeystore", "secro", WP_POWER_ON);
    MSG(INF, "[%s]: (partition case: 4) oemkeystore->secro, err=%d\n", __func__, err);

    MSG(INF, "[%s]: (partition case: 5) test_start->keystore\n", __func__);
    err = partition_write_prot_set("test_starte", "keystor", WP_POWER_ON);
    MSG(INF, "[%s]: (partition case: 5) test_start->keystore, err=%d\n", __func__, err);

        MSG(INF, "[%s]: (partition case: 6) oemkeystore->test_end\n", __func__);
    err = partition_write_prot_set("oemkeystore", "test_end", WP_POWER_ON);
    MSG(INF, "[%s]: (partition case: 6) oemkeystore->test_end, err=%d\n", __func__, err);
    #endif
