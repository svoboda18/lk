...
#define DL_INFO_SIZE    (8192) //Support EMMC GPT 128 partitions
#else
...
int mmc_get_dl_info(void)
{
	...
	dl_addr = partition_get_size_by_name("flashinfo") - DL_INFO_SIZE;
    dprintf(ALWAYS, "[DL_INFO]get dl info from offset 0x%llx\n", dl_addr);
	...
	err = partition_read("flashinfo", dl_addr, dl_buf, DL_INFO_SIZE);
    if (err != DL_INFO_SIZE) {
