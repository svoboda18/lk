...
#define I2C_SWITCHING_CHARGER_CHANNEL 3
#define DISABLE_NCP1854_FACTORY_MODE
				    ...
				    kal_uint8 SHIFT)
{
	...
	if (RegNum == NCP1854_CON1 && val == 1 && MASK == CON1_REG_RST_MASK
	    && SHIFT == CON1_REG_RST_SHIFT) {
		/* RESET bit */
...
kal_uint32 ncp1854_get_usb_cfg(void)
{
	...
	ret = ncp1854_read_interface((kal_uint8) (NCP1854_CON1),
				     (&val),
				     (kal_uint8) (CON1_USB_CFG_MASK),
				     (kal_uint8) (CON1_USB_CFG_SHIFT)
	    );
	return val;
}
...
/* ncp1854_set_tchg_rst(0x1); //reset charge timer */
	...
	ncp1854_set_ctrl_vfet(0x0);	/* VFET = 3.1V */
}
	...
	if (g_chr_type_num == STANDARD_CHARGER) {
		...
		if (g_std_ac_large_current_en == 1) {
			...
			if (bat_val < PRECC_BATVOL) {
				...
				ncp1854_set_ichg_high(0);	/* ICHG */
				ncp1854_set_ichg(0xB);	/* Fast Charging Current Limit at 1.5A */
			}
		}

	} else if (g_chr_type_num == STANDARD_HOST
		   || g_chr_type_num == CHARGING_HOST || g_chr_type_num == NONSTANDARD_CHARGER) {
		temp_CC_value = 500;
