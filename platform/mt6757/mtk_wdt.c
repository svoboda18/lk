...
static void mtk_wdt_pmic_full_reset(void)
{
#if defined(PMIC_CHIP_MT6355)
	/* PMIC will do full reset mmediately. */
	...
	pmic_config_interface(0x403C, 0x1, 0x1, 0x0); /* RTC write trigger */
	pmic_set_register_value(PMIC_RG_RSV_SWREG, 1); /* Set PMIC shutdown reboot status */
