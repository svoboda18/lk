...
void DSI_clk_HS_mode(DISP_MODULE_ENUM module, cmdqRecHandle cmdq, bool enter)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (enter) {
			...
			DSI_OUTREGBIT(NULL, DSI_PHY_LCCON_REG, DSI_REG[i]->DSI_PHY_LCCON, EARLY_HS_POE, 0);
...
//	unsigned int fmod = 30;//Fmod = 30KHz by default
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (dsi_params->lane_swap_en) {
			DISPDBG("MIPITX Lane Swap Enabled for DSI Port %d\n", i);
			DISPDBG("MIPITX Lane Swap mapping: %d|%d|%d|%d|%d|%d\n",
	...
	/* step 0 -- get from DE
	Efuse	MIPI TX CONFIG RT_code
	0x10206190 [7:4]  	 10215008[11:8]
	0x10206190 [3:0]  	 1021500C[11:8]
	0x10206194 [31:28]  	 10215010[11:8]
	0x10206194 [27:24]  	 10215014[11:8]
	0x10206194 [23:20]  	 10215004[11:8]
	0x10206194 [19:16]  	 10216008[11:8]
	0x10206194 [15:12]  	 1021600C[11:8]
	0x10206194 [11:8]  	 10216010[11:8]
	0x10206194 [7:4]  	 10216014[11:8]
	0x10206194 [3:0]  	 10216004[11:8] */
	lnt0 = MIPITX_INREG32(0x10206190);
	lnt1 = MIPITX_INREG32(0x10206194);
	if (DSI_MODULE_BEGIN(module) == 0) {
		if(lnt0) {
			MIPITX_OUTREGBIT(MIPITX_DSI_DATA_LANE0_REG, DSI_PHY_REG[0]->MIPITX_DSI_DATA_LANE0,
				RG_DSI_LNT0_RT_CODE, ((lnt0 >> 4) & 0xf));
			MIPITX_OUTREGBIT(MIPITX_DSI_DATA_LANE1_REG, DSI_PHY_REG[0]->MIPITX_DSI_DATA_LANE1,
				RG_DSI_LNT1_RT_CODE, ((lnt0 >> 0) & 0xf));
		}
		if(lnt1) {
			MIPITX_OUTREGBIT(MIPITX_DSI_DATA_LANE2_REG, DSI_PHY_REG[0]->MIPITX_DSI_DATA_LANE2,
				RG_DSI_LNT2_RT_CODE, ((lnt1 >> 28) & 0xf));
			MIPITX_OUTREGBIT(MIPITX_DSI_DATA_LANE3_REG, DSI_PHY_REG[0]->MIPITX_DSI_DATA_LANE3,
				RG_DSI_LNT3_RT_CODE, ((lnt1 >> 24) & 0xf));
			MIPITX_OUTREGBIT(MIPITX_DSI_CLOCK_LANE_REG, DSI_PHY_REG[0]->MIPITX_DSI_CLOCK_LANE,
				RG_DSI_LNTC_RT_CODE, ((lnt1 >> 20) & 0xf));
		}
	}

	if (DSI_MODULE_END(module) == 1) {
		if(lnt1) {
			MIPITX_OUTREGBIT(MIPITX_DSI_DATA_LANE0_REG, DSI_PHY_REG[1]->MIPITX_DSI_DATA_LANE0,
				RG_DSI_LNT0_RT_CODE, ((lnt1 >> 16) & 0xf));
			MIPITX_OUTREGBIT(MIPITX_DSI_DATA_LANE1_REG, DSI_PHY_REG[1]->MIPITX_DSI_DATA_LANE1,
				RG_DSI_LNT1_RT_CODE, ((lnt1 >> 12) & 0xf));
			MIPITX_OUTREGBIT(MIPITX_DSI_DATA_LANE2_REG, DSI_PHY_REG[1]->MIPITX_DSI_DATA_LANE2,
				RG_DSI_LNT2_RT_CODE, ((lnt1 >> 8) & 0xf));
			MIPITX_OUTREGBIT(MIPITX_DSI_DATA_LANE3_REG, DSI_PHY_REG[1]->MIPITX_DSI_DATA_LANE3,
				RG_DSI_LNT3_RT_CODE, ((lnt1 >> 4) & 0xf));
			MIPITX_OUTREGBIT(MIPITX_DSI_CLOCK_LANE_REG, DSI_PHY_REG[1]->MIPITX_DSI_CLOCK_LANE,
				RG_DSI_LNTC_RT_CODE, ((lnt1 >> 0) & 0xf));
		}
	}
	DISPDBG("PLL0 config: LNT0=0x%x,clk=0x%x,lan0=0x%x,lan1=0x%x,lan2=0x%x,lan3=0x%x\n",
	...
	DISPDBG("PLL1 config: LNT1=0x%x,clk=0x%x,lan0=0x%x,lan1=0x%x,lan2=0x%x,lan3=0x%x\n",
		...
		if (0!=data_Rate) {
			...
			*/
			...
			if (1 != dsi_params->ssc_disable) {
				...
				DISPDBG("PLL config:data_rate=%d,pcw_ratio=%d,pcw=%d,delta1=%d,pdelta1=0x%x\n",
...
#ifdef MACH_FPGA
	/* cp from cmm file */
	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_PHY_TIMECON0, 0x02010102);
	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_PHY_TIMECON1, 0x020a0308);
	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_PHY_TIMECON2, 0x02010100);
	DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_PHY_TIMECON3, 0x00020801);
