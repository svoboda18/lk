...
int mmc_set_write_protect_by_group(struct mmc_card *card,
                                   Region partition,unsigned long blknr,u32 blkcnt, STORAGE_WP_TYPE type);
#endif
...
void mmc_dump_ext_csd(struct mmc_card *card)
{
	...
	msdc_pr_info("[EXT_CSD] Min. write perf.(DDR,52MH,8b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_DDR_W_8_52]);
	msdc_pr_info("[EXT_CSD] Min. read perf. (DDR,52MH,8b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_DDR_R_8_52]);
	...
	msdc_pr_info("[EXT_CSD] Min. WR Perf.  (52MH,8b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_W_8_52]);
	msdc_pr_info("[EXT_CSD] Min. Read Perf.(52MH,8b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_R_8_52]);
	msdc_pr_info("[EXT_CSD] Min. WR Perf.  (26MH,8b,52MH,4b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_W_8_26_4_25]);
	msdc_pr_info("[EXT_CSD] Min. Read Perf.(26MH,8b,52MH,4b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_R_8_26_4_25]);
	msdc_pr_info("[EXT_CSD] Min. WR Perf.  (26MH,4b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_W_4_26]);
	msdc_pr_info("[EXT_CSD] Min. Read Perf.(26MH,4b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_R_4_26]);
	...
	msdc_pr_info("[EXT_CSD] Max. enhanced area size : %xh (%d kbytes)\n",
	...
	msdc_pr_info("[EXT_CSD] General purpose 1 size : %xh (%d kbytes)\n",
	...
	msdc_pr_info("[EXT_CSD] General purpose 2 size : %xh (%d kbytes)\n",
	       ...
	       ext_csd[EXT_CSD_HC_WP_GPR_SIZE] *
	       ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE]);
	msdc_pr_info("[EXT_CSD] General purpose 3 size : %xh (%d kbytes)\n",
	       ...
	       ext_csd[EXT_CSD_HC_WP_GPR_SIZE] *
	       ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE]);
	msdc_pr_info("[EXT_CSD] General purpose 4 size : %xh (%d kbytes)\n",
	       ...
	       ext_csd[EXT_CSD_HC_WP_GPR_SIZE] *
	       ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE]);
	msdc_pr_info("[EXT_CSD] Enh. user area size : %xh (%d kbytes)\n",
...
static void mmc_decode_ext_csd(struct mmc_card *card)
{
	...
	card->ext_csd.gp_sz_mult[0] = ext_csd[EXT_CSD_GP1_SIZE_MULT + 2] * 256 * 256 +
		ext_csd[EXT_CSD_GP1_SIZE_MULT + 1] * 256 + ext_csd[EXT_CSD_GP1_SIZE_MULT + 0];
	card->ext_csd.gp_sz_mult[1] = ext_csd[EXT_CSD_GP2_SIZE_MULT + 2] * 256 * 256 +
		ext_csd[EXT_CSD_GP2_SIZE_MULT + 1] * 256 + ext_csd[EXT_CSD_GP2_SIZE_MULT + 0];
	card->ext_csd.gp_sz_mult[2] = ext_csd[EXT_CSD_GP3_SIZE_MULT + 2] * 256 * 256 +
		ext_csd[EXT_CSD_GP3_SIZE_MULT + 1] * 256 + ext_csd[EXT_CSD_GP3_SIZE_MULT + 0];
	card->ext_csd.gp_sz_mult[3] = ext_csd[EXT_CSD_GP4_SIZE_MULT + 2] * 256 * 256 +
	...
	if (ext_csd[EXT_CSD_ERASE_GRP_DEF]&EXT_CSD_ERASE_GRP_DEF_EN) {
		...
		msdc_pr_info("EXT_CSD_ERASE_GRP_DEF is On, wp_size = %dMB\n", card->wp_size/2048);
	} else {
		card->wp_size =(card->csd.write_prot_grpsz+1) * card->csd.erase_sctsz;
		msdc_pr_info("EXT_CSD_ERASE_GRP_DEF is Off, wp_size = %dMB, csd.write_prot_grpsz = %d,csd.erase_sctsz = %d\n", card->wp_size/2048, card->csd.write_prot_grpsz, card->csd.erase_sctsz);
	}
	msdc_pr_info("[%s]: mmc_set_wp_size %dMB\n", __func__,card->wp_size/2048);
#endif
...
int mmc_switch_hs(struct mmc_host *host, struct mmc_card *card)
{
	...
	if ((status[16] & 0xF) != 1) {
		msdc_pr_info("[SD%d] HS mode not supported!\n", host->id);
		err = MMC_ERR_FAILED;
	} else {
...
int mmc_switch_uhs1(struct mmc_host *host, struct mmc_card *card, unsigned int mode)
{
	...
	if ((status[16] & 0xF) != mode) {
		msdc_pr_info("[SD%d] UHS-1 %s mode not supported!\n", host->id, smode[mode]);
		err = MMC_ERR_FAILED;
	} else {
		card->uhs_mode = mode;
...
int mmc_switch_drv_type(struct mmc_host *host, struct mmc_card *card, int val)
{
	...
	if ((status[15] & 0xF) != val) {
		msdc_pr_info("[SD%d] UHS-1 %s drv not supported!\n", host->id, type[val]);
		err = MMC_ERR_FAILED;
	} else {
...
int mmc_switch_max_cur(struct mmc_host *host, struct mmc_card *card, int val)
{
	...
	if (((status[15] >> 4) & 0xF) != val) {
		msdc_pr_info("[SD%d] UHS-1 %s max. current not supported!\n", host->id, curr[val]);
		err = MMC_ERR_FAILED;
	} else {
...
int mmc_verify_write_prot_type_by_group(struct mmc_card *card, unsigned long blknr,
                                        u32 blkcnt, STORAGE_WP_TYPE type)
{
	int err = MMC_ERR_FAILED;
	u32 count_wp;
	u32 count_wp_rest;
                                   ...
                                   Region partition,unsigned long blknr,u32 blkcnt, STORAGE_WP_TYPE type)
{
	...
	if (type == WP_PERMANENT || type == WP_DISABLE || type == WP_TEMPORARY) {
		msdc_pr_err("[%s]: Type Not Support\n", __func__);
		return 0;
	}
	if (type > WP_DISABLE || type < 0)  {
		msdc_pr_err("[%s]:%d type=%d\n", __func__,__LINE__,type);
		return err;
	}
	//check partition
	...
	if (err) {
		msdc_pr_err("[%s]: mmc_switch_part err %d\n", __func__,err);
		return err;
	}

	count_wp = blkcnt/card->wp_size;

	msdc_pr_info("[%s]: count_wp %d\n", __func__,count_wp);
...
//	struct mmc_host *host = mmc_get_host(dev_num);
	...
	if (type==WP_POWER_ON && partition == EMMC_PART_USER) {
		...
		msdc_pr_err("[%s]: WP Type Not Support\n", __func__);
		return 0;
...
int mmc_set_boot_wp(struct mmc_card *card, STORAGE_WP_TYPE type, Region partition)
{
	int err = MMC_ERR_FAILED;
	u8 value;
	...
	if (card->ext_csd.boot_wp & EXT_CSD_BOOT_WP_DIS_PWR_WP) {
		//msdc_pr_err("[%s]: EXT_CSD_BOOT_WP_DIS_PWR_WP is err set \n", __func__);
		goto out;
	}
...
int mmc_set_user_wp(struct mmc_card *card, STORAGE_WP_TYPE type, unsigned long blknr,
                    u32 blkcnt,Region partition)
{
	...
	if (card->ext_csd.usr_wp & US_PWR_WP_DIS) {
		msdc_pr_err("[%s]: US_PWR_WP_DIS is err set \n", __func__);
		goto out;
	}
	...
	if (card->ext_csd.usr_wp == value) {
		msdc_pr_info("[%s]: EXT_CSD_USR_WP alread set \n",__func__);
	} else {
		err = mmc_switch(card->host, card, EXT_CSD_CMD_SET_NORMAL,
		...
		msdc_pr_info("[%s]: card->ext_csd.usr_wp:%d\n", __func__,card->ext_csd.usr_wp);
	}
	...
	if (err) {
		msdc_pr_err("[%s]: restore US_PWR_WP_EN err %d\n", __func__,err);
		goto out;
	}
...
int mmc_dev_bread(struct mmc_card *card, unsigned long blknr, u32 blkcnt, u8 *dst)
{
	...
	do {
		...
		if (err == MMC_ERR_TIMEOUT ) {
			...
			msdc_pr_err("[SD%d] mmc_dev_bread TIMEOUT\n", host->id);
#endif
			break;
		}
	} while (retry--);
...
static int mmc_dev_bwrite(struct mmc_card *card, unsigned long blknr, u32 blkcnt, u8 *src)
{
	...
	do {
		...
		if (err == MMC_ERR_TIMEOUT ) {
#ifndef MSDC_ETT_SUPPRESS_LOG
			msdc_pr_err("[SD%d] mmc_dev_bwrite TIMEOUT\n", host->id);
#endif
			break;
		}
	} while (retry--);
...
int mmc_block_read(int dev_num, unsigned long blknr, u32 blkcnt, unsigned long *dst)
{
	...
	if (blknr * (blksz / MMC_BLOCK_SIZE) > card->nblks) {
		msdc_pr_err("[SD%d] Out of block range: blknr(%ld) > sd_blknr(%d)\n",
...
int mmc_init_mem_card(struct mmc_host *host, struct mmc_card *card, u32 ocr)
{
	 ...
	 */
	if (ocr & 0x7F) {
		msdc_pr_info("card claims to support voltages "
	...
	if (err != MMC_ERR_NONE) {
		s18a = (card->ocr >> 24) & 0x1;
		msdc_pr_err("[SD%d] ocr = 0x%X, s18a = %d \n", id, ocr, s18a);
	}
...
int mmc_init_card(struct mmc_host *host, struct mmc_card *card)
{
	...
	/* change clock */
	msdc_pr_info("before host->cur_bus_clk(%d)\n",host->cur_bus_clk);
	host->card = card;
	mmc_set_clock(host, mmc_card_ddr(card), card->maxhz);
	msdc_pr_info("host->cur_bus_clk(%d)\n",host->cur_bus_clk);
...
int mmc_init(int id, u32 trans_mode)
{
	...
	msdc_pr_info("[%s]: msdc%d, use hc erase size\n", __func__, id);
	if (mmc_card_mmc(card))
