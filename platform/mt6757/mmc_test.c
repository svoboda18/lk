...
write_1_test:
			...
			if ( (v != 1) && (!clock_gate_test) ) {
				msdc_pr_info("[SD%d] Write 1 Reg Failed in 0x%x[%d]=%d != %d\n",
				       id, base + msdc_reg_desc[i].offset, j, v,
			...
			if ( (v == 1) && (clock_gate_test) ) {
				msdc_pr_info("[SD%d] CG Write 1 Reg Failed in 0x%x[%d]=%d != %d\n",
				       id, base + msdc_reg_desc[i].offset, j, v,
				       msdc_reg_desc[i].reset[j] - '0');
				error = __LINE__;
			}
			...
			if ( !clock_gate_test ) {
				for (k = 0; k < 32; k++) {
					...
					if (v != (msdc_reg_desc[i].reset[k] - '0')) {
						msdc_pr_info("[SD%d] Affected by Write 1 to 0x%x[%d] and [%d]=%d != 1\n",
...
write_0_test:
			...
			if ( clock_gate_test ) msdc_clock(host, 0);
			{
				...
				if ( ( clock_gate_test ) && ( vvv!=0 ) ) msdc_pr_info("Reg=%x!\n", vvv);
			}
			...
			if ( (val_shall_be==1) && (v != 0) && ( clock_gate_test ) ) {
				msdc_pr_info("[SD%d] CG Write 0 Reg Failed in 0x%x[%d]=%d != %d\n",
				       id, base + msdc_reg_desc[i].offset, j, v,
				       msdc_reg_desc[i].reset[j] - '0');
				error = __LINE__;
			}
...
int mmc_test_mem_card(struct mmc_test_config *cfg)
{
	...
	while (forever || count--) {
			...
			msdc_pr_info("host->sclk(%d)\n",host->sclk);
			msdc_pr_info(TC_MSG, host->id, result == 0 ? "PASS" : "FAIL", tid++,
			       "test block erase\n");
#endif
			...
			if (cfg->tst_single) {
				...
				for (i = 0; i < total_blks; i++) {
					...
					if (ret != MMC_ERR_NONE) {
						msdc_pr_info("test single block write failed (%d)\n", i);
						result = -__LINE__;
						goto failure;
					}
				}
				...
				for (i = 0; i < total_blks && !result; i++) {
					...
					if (chk_result) {
						for (j = 0; j < blksz; j++) {
							if (buf[j] != pattern) {
								msdc_pr_info("compare,i=%d, j=%d, buf[j]=%d, pattern(%d), buf(0x%x)\n", i, j, buf[j], pattern, (unsigned int)buf);
				...
				if (result) {
					msdc_pr_info("[SD%d]\t\tread back pattern(0x%.2x) failed\n",
					       id, pattern);
					goto failure;
				}
#endif
			...
			msdc_pr_info("[SD0] HS400 change to HS200 TC41 \n");
			if (cfg->clock) {
				clkhz = card->maxhz < cfg->clock ? card->maxhz : cfg->clock;
...
int mmc_test_mem_card(struct mmc_test_config *cfg)
{
	...
	while (forever || count--) {
			msdc_pr_info("[TST] ==============================================\n");
			msdc_pr_info("[TST]buf address : 0x%x\n",buf);
			msdc_pr_info("[TST] BEGIN: %d/%d, No Stop(%d)\n",
			       (cfg->count != -1) ? cfg->count - count : 0,
			       (cfg->count != -1) ? cfg->count : 0, forever);
			msdc_pr_info("[TST] ----------------------------------------------\n");
...
//#endif /* end of MSDC_USE_SD30_USH50 || MSDC_USE_SD30_USH104 */
			...
			/* change clock */
			msdc_pr_info("host->sclk(%d), card->maxhz(%d), cfg->clock(%d)\n",host->sclk, card->maxhz, cfg->clock);
			if (cfg->clock) {
				clkhz = card->maxhz < cfg->clock ? card->maxhz : cfg->clock;
				mmc_set_clock(host, card->state, clkhz);
			}
...
//#if (1 == MSDC_USE_SD30)
			...
			if (cfg->tst_single) {
				...
				for (i = 0; i < total_blks; i++) {
					...
					for (k = 0; k < blksz; k++) {
						...
						//msdc_pr_info("0x%x ", buf[k]);
						//if ((k % 32) == 0)
						//msdc_pr_info("\n");
					}
#endif /* end of MSDC_NO_USE_RANDOM_TEST_PATTERN */

					//msdc_pr_info("signal block write: 0x%x\n", blknr + i);
					ret = mmc_block_write(id, blknr + i, 1, (unsigned long*)buf);
					if (ret != MMC_ERR_NONE) {
						msdc_pr_info("test single block write failed (%d)\n", i);
						result = -__LINE__;
				...
				for (i = 0; i < total_blks && !result; i++) {
					...
					//msdc_pr_info("signal block read: 0x%x\n", blknr + i);
					ret = mmc_block_read(id, blknr + i, 1, (unsigned long*)buf);
					if (ret != MMC_ERR_NONE) {
						result = -__LINE__;
						goto failure;
...
#if (1 == MSDC_NO_USE_RANDOM_TEST_PATTERN)
							if (buf[j] != pattern) {
								msdc_pr_info("ops:0x%x j(%d),buf[j](%d),pattern(%d)\n", blknr + i, j, buf[j], pattern);
#else
			...
			if (cfg->tst_multiple) {
				...
				for (i = 0; i < chunks; i++) {
					...
					//msdc_pr_info("multi block write: 0x%x\n(%d)", blknr + i* chunk_blks, chunk_blks);
					ret = mmc_block_write(id, blknr + i * chunk_blks,
					                      chunk_blks, (unsigned long*)buf);
					if (ret != MMC_ERR_NONE) {
						result = -__LINE__;
				...
				for (i = 0; i < chunks; i++) {
					...
					//msdc_pr_info("multi block read: 0x%x\n(%d)", blknr + i* chunk_blks, chunk_blks);
					ret = mmc_block_read(id, blknr + i * chunk_blks,
					                     chunk_blks, (unsigned long*)buf);
					if (ret != MMC_ERR_NONE) {
						msdc_pr_info("[SD%d]\t\tread %d blks failed(ret = %d blks)\n",
