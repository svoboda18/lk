...
static void _mtkfb_draw_block(unsigned int addr, unsigned int x, unsigned int y, unsigned int w, unsigned int h, unsigned int color)
{
	int i = 0;
	int j = 0;
	void* start_addr = addr+ALIGN_TO(CFG_DISPLAY_WIDTH, MTK_FB_ALIGNMENT)*4*y+x*4;
...
{
	int i = 0;
	int Bpp = disp_get_round_corner_Bpp();

	for (i = 0; i < h; i++) {
		memcpy(dst+picture_w*i*Bpp, left+Bpp*w*i, w*Bpp);
		memcpy(dst+(picture_w*(i+1)-w)*Bpp, right+Bpp*w*i, w*Bpp);
	}
}
