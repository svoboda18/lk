...
#define PLL_OSC_CON0		(0x10001B00)
#define PLL_OSC_CON1		(0x10001B04)
#define OSC_ULPOSC_DBG		(0x10006000)
#define OSC_ULPOSC_ADDR		(0x10006458)
...
static void disp_pwm_ulposc_cali(void)
{
	...
	OSC_REG_SET(OSC_ULPOSC_DBG, 0x0b160001);
	OSC_REG_SET(OSC_ULPOSC_ADDR, 0x1);
	udelay(30);
	OSC_REG_SET(OSC_ULPOSC_ADDR, 0x5);
	...
	if (cust_led_list) {
		...
		if ((strcmp(cust->name,"lcd-backlight") == 0) && (cust->mode == MT65XX_LED_MODE_CUST_BLS_PWM)) {
			...
			if (config_data->clock_source >= 0 && config_data->clock_source <= 3) {
				...
				PWM_REG_SET(CLK_CFG_7, (reg_val & ~0x3) | config_data->clock_source);
				PWM_MSG("disp_pwm_init : CLK_CFG_1 0x%x => 0x%x", reg_val, PWM_REG_GET(CLK_CFG_7));
			}
			...
			if ((config_data->clock_source == DISP_PWM_SRC_OSC_D4 || config_data->clock_source == DISP_PWM_SRC_OSC_D8)
				&& (pwm_div < 0x3FF)) {
