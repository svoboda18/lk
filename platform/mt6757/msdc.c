...
int msdc_register_partial_backup_and_reset(struct mmc_host* host)
{
	...
	for (i = 0; i < HS400_BACKUP_REG_NUM; i++) {
		...
		if (hs400_backup_reg_list[i].restore_func) {
			...
			if (err) {
				MSG(INF, "[%s]: failed to restore reg[0x%x][0x%x], expected value[0x%x], actual value[0x%x] err=0x%x",
...
void msdc_set_startbit(struct mmc_host *host, u8 start_bit)
{
	...
	switch (l_start_bit) {
		case 0:
			MSG(INF, "[info][%s %d] read data start bit at rising edge\n", __func__, __LINE__);
			break;
		case 1:
			MSG(INF, "[info][%s %d] read data start bit at falling edge\n", __func__, __LINE__);
			break;
		case 2:
			MSG(INF, "[info][%s %d] read data start bit at rising & falling edge\n", __func__, __LINE__);
			break;
		case 3:
			MSG(INF, "[info][%s %d] read data start bit at rising | falling edge\n", __func__, __LINE__);
			break;
	...
	switch (type) {
		case TYPE_CMD_RESP_EDGE:
			...
			if (mode == MSDC_SMPL_RISING || mode == MSDC_SMPL_FALLING) {
				...
				MSG(INF, "[%s]: SD%d invalid resp parameter: HS400=%d, type=%d, mode=%d\n", __func__, host->id, HS400, type, mode);
			}
			break;
		...
		case TYPE_WRITE_CRC_EDGE:
			...
			if (mode == MSDC_SMPL_RISING || mode == MSDC_SMPL_FALLING) {
				...
				MSG(INF, "[%s]: SD%d invalid crc parameter: HS400=%d, type=%d, mode=%d\n", __func__, host->id, HS400, type, mode);
			}
			break;
		...
		case TYPE_READ_DATA_EDGE:
			...
			if (mode == MSDC_SMPL_RISING || mode == MSDC_SMPL_FALLING) {
				...
				MSG(INF, "[%s]: SD%d invalid read parameter: HS400=%d, type=%d, mode=%d\n", __func__, host->id, HS400, type, mode);
			}
			break;
		...
		case TYPE_WRITE_DATA_EDGE:
			...
			if (mode == MSDC_SMPL_RISING|| mode == MSDC_SMPL_FALLING) {
				...
				MSG(INF, "[%s]: SD%d invalid write parameter: HS400=%d, type=%d, mode=%d\n", __func__, host->id, HS400, type, mode);
			}
			break;

		default:
			MSG(INF, "[%s]: SD%d invalid parameter: HS400=%d, type=%d, mode=%d\n", __func__, host->id, HS400, type, mode);
			break;
	}
}
...
int msdc_send_cmd(struct mmc_host *host, struct mmc_command *cmd)
{
	...
	if (opcode == MMC_CMD_SEND_STATUS) {
		...
		if (SDC_IS_BUSY()) {
			...
			if (timeout == 0) {
				error = MMC_ERR_TIMEOUT;
				MSG(INF, "[SD%d] CMD(%d): SDC_IS_BUSY timeout\n",
				       host->id, (opcode & ~(SD_CMD_BIT | SD_CMD_APP_BIT)));
				goto end;
			}
		}
...
int msdc_cmd(struct mmc_host *host, struct mmc_command *cmd)
{
	...
	if (err == MMC_ERR_BADCRC) {
		...
		if ( host->app_cmd!=2 ) { //Light 20121225, to prevent recursive call path: msdc_tune_cmdrsp->msdc_app_cmd->msdc_cmd->msdc_tune_cmdrsp
			...
			if (err != MMC_ERR_NONE) {
				MSG(INF, "[Err handle][%s:%d]tune cmd fail\n", __func__, __LINE__);
			}
		}
...
int msdc_abort_handler(struct mmc_host *host, int abort_card)
{
	...
	while (state != 4) { // until status to "tran"; //20130125 Comment out by Light
		...
		//#if defined(MMC_MSDC_DRV_CTP)
		if (err == MMC_ERR_BADCRC) {
			MSG(INF, "[Err handle][%s:%d]cmd13 crc error\n", __func__, __LINE__);
		...
		if (err == MMC_ERR_TIMEOUT) {
			MSG(INF, "[Err handle][%s:%d]cmd13 timeout\n", __func__, __LINE__);
			msdc_tune_update_cmdrsp(host, count++);
...
#else
		if (err != MMC_ERR_NONE) {
			MSG(INF, "[Err handle][%s:%d]cmd13 fail\n", __func__, __LINE__);
			goto out;
		}
#endif
		...
		if (state == 5 || state == 6) {
			if (abort_card) {
				...
				//#if defined(MMC_MSDC_DRV_CTP)
				if (err == MMC_ERR_BADCRC) {
					MSG(INF, "[Err handle][%s:%d]cmd12 crc error\n", __func__, __LINE__);
					msdc_tune_update_cmdrsp(host, count++);
				...
				if (err == MMC_ERR_TIMEOUT) {
					MSG(INF, "[Err handle][%s:%d]cmd12 timeout\n", __func__, __LINE__);
					msdc_tune_update_cmdrsp(host, count++);
...
#else
				if (err != MMC_ERR_NONE) {
					MSG(INF, "[Err handle][%s:%d]cmd12 fail\n", __func__, __LINE__);
					goto out;
				}
#endif
			}
...
out:
	MSG(INF, "[SD%d] data abort failed\n",host->id);
	return 1;
}
...
void msdc_brk_cmd(struct mmc_host *host)
{
	...
	if (tmo == 0)
		MSG(INF, "[%s]: SDC BUSY timeout happend, before send break cmd\n", __func__);
...
int msdc_pio_read(struct mmc_host *host, u32 *ptr, u32 size)
{
	...
	while (1) {
		...
		if (status & MSDC_INT_DATCRCERR) {
			...
			MSG(INF, "[SD%d] DAT CRC error (0x%x), Left:%d/%d bytes, RXFIFO:%d,dcrc:0x%x\n",
			       host->id, status, size, totalsz, MSDC_RXFIFOCNT(),dcrc);
			err = MMC_ERR_BADCRC;
			break;
		} else if (status & MSDC_INT_DATTMO) {
			MSG(INF, "[SD%d] DAT TMO error (0x%x), Left: %d/%d bytes, RXFIFO:%d\n",
			       host->id, status, size, totalsz, MSDC_RXFIFOCNT());
			err = MMC_ERR_TIMEOUT;
			break;
		} else if (status & MSDC_INT_ACMDCRCERR) {
			MSDC_GET_FIELD(SDC_DCRC_STS, SDC_DCRC_STS_POS|SDC_DCRC_STS_NEG, dcrc);
			MSG(INF, "[SD%d] AUTOCMD CRC error (0x%x), Left:%d/%d bytes, RXFIFO:%d,dcrc:0x%x\n",
			       host->id, status, size, totalsz, MSDC_RXFIFOCNT(),dcrc);
	...
	if (err != MMC_ERR_NONE) {
		...
		MSG(INF, "[SD%d] %d-bit PIO Read Error (%d)\n", host->id,
		       pio_bits, err);
	}
...
int msdc_pio_write(struct mmc_host *host, u32 *ptr, u32 size)
{
	...
	while (1) {
		...
		if (status & MSDC_INT_DATCRCERR) {
			MSG(INF, "[SD%d] DAT CRC error (0x%x), Left DAT: %d bytes\n",
			       host->id, status, size);
			err = MMC_ERR_BADCRC;
			break;
		} else if (status & MSDC_INT_DATTMO) {
			MSG(INF, "[SD%d] DAT TMO error (0x%x), Left DAT: %d bytes, MSDC_FIFOCS=%xh\n",
			       host->id, status, size, MSDC_READ32(MSDC_FIFOCS));
			err = MMC_ERR_TIMEOUT;
			break;
		} else if (status & MSDC_INT_ACMDCRCERR) {
			MSG(INF, "[SD%d] AUTO CMD CRC error (0x%x), Left DAT: %d bytes\n",
			       host->id, status, size);
			err = MMC_ERR_ACMD_RSPCRC;
			break;
		} else if (status & MSDC_INT_XFER_COMPL) {
...
int msdc_pio_get_sandisk_fwid(struct mmc_host *host, uchar *dst)
{
	...
	if (err != MMC_ERR_NONE) {
		if (derr != MMC_ERR_NONE) {
			MSG(INF, "[SD%d] Read data error (%d)\n", host->id, derr);
			msdc_abort_handler(host, 1);
		} else {
			MSG(INF, "[SD%d] Read error (%d)\n", host->id, err);
		}
	}
...
int msdc_pio_send_sandisk_fwid(struct mmc_host *host,uchar *src)
{
	...
	if (err != MMC_ERR_NONE) {
		if (derr != MMC_ERR_NONE) {
			MSG(INF, "[SD%d] Write data error (%d)\n", host->id, derr);
			msdc_abort_handler(host, 1);
		} else {
			MSG(INF, "[SD%d] Write error (%d)\n", host->id, err);
		}
	}
	return (derr == MMC_ERR_NONE) ? err : derr;
}
...
void msdc_config_clock(struct mmc_host *host, int ddr, u32 hz, u32 hs_timing)
{
	...
	MSG(INF, "[SD%d] SET_CLK(%dkHz): SCLK(%dkHz) MODE(%d) DDR(%d) DIV(%d) DS(%d) RS(%d)\n",
...
int msdc_switch_volt(struct mmc_host *host, int volt)
{
	...
	if ((MSDC_READ32(MSDC_PS) & ((1 << 24) | (0xF << 16))) == 0) {
		...
		if (status & MSDC_CFG_BV18PSS)
			...
			MSG(INF, "[%s] sd%d v18 switch failed, MSDC_CFG=0x%x\n", __func__, host->id, status);
			...
			do {
				for (rsmpl = 0; rsmpl < 2; rsmpl++) {
					...
					/* for debugging */
					{
						...
						MSG(INF, "[SD%d] <TUNE_CMD><%d><%s> CMD_RSP_TA_CNTR=%xh\n",
						       host->id, times, (result == MMC_ERR_NONE) ?
						       "PASS" : "FAIL", t_cmdrtc);
						if (host->cur_bus_clk > 100000000) {
							MSG(INF, "[SD%d] <TUNE_CMD%d><%d><%s> CMD_RSP_TA_CNTR=%xh, INT_DAT_LATCH_CK_SEL=%xh\n",
							       host->id, (cmd->opcode & (~(SD_CMD_BIT | SD_CMD_APP_BIT))), times, (result == MMC_ERR_NONE) ?
			...
			do {
				for (rdsmpl = 0; rdsmpl < 2; rdsmpl++) {
					...
					/* for debugging */
					{
						...
						times++;
						MSG(INF, "[SD%d] <TUNE_BREAD_%d><%s><cmd%d:0x%x> DCRC=%xh, ret=%d\n",
						       host->id, times, (result == MMC_ERR_NONE && dcrc == 0) ?
						       "PASS" : "FAIL", (nblks == 1 ? 17 : 18), src, dcrc, result);
						MSG(INF, "[SD%d] <TUNE_BREAD_%d><%s><cmd%d:0x%x> DATRDDLY0=%xh, DATRDDLY1=%xh, DSMPL=%xh\n",
						       host->id, times, (result == MMC_ERR_NONE && dcrc == 0) ?
						       "PASS" : "FAIL", (nblks == 1 ? 17 : 18), src, MSDC_READ32(MSDC_DAT_RDDLY0), MSDC_READ32(MSDC_DAT_RDDLY1), t_dspl);

						if (host->cur_bus_clk >= 100000000) {
							MSG(INF, "[SD%d] <TUNE_BREAD_%d><%s><cmd%d:0x%x> CKGEN_MSDC_DLY_SEL=%xh, INT_DAT_LATCH_CK_SEL=%xh\n",
							       host->id, times, (result == MMC_ERR_NONE && dcrc == 0) ?
			...
			do {
				for (wdsmpl = 0; wdsmpl < 2; wdsmpl++) {
					...
					/* for debugging */
					{
						...
						times++;
						MSG(INF, "[SD%d] <TUNE_BWRITE_%d><%s><cmd%d:0x%x> ret=%d, DSPL=%d, WRRDLY=%d, MSDC_DAT_RDDLY0=%xh\n",
						       host->id, times, result == MMC_ERR_NONE ? "PASS" : "FAIL", (nblks == 1 ? 24 : 25), (unsigned int)dst,
						       result, t_dspl, t_wrrdly, MSDC_READ32(MSDC_DAT_RDDLY0));

						if (host->cur_bus_clk >= 100000000) {
							MSG(INF, "[SD%d] <TUNE_BWRITE_%d><%s><cmd%d:0x%x> MSDC_PB1_WRDAT_CRCS_TA_CNTR=%xh\n",
							       host->id, times, (result == MMC_ERR_NONE) ? "PASS" : "FAIL", (nblks == 1 ? 24 : 25), (unsigned int)dst,
							       t_d_cntr);
						}
					}
...
int msdc_tune_uhs1(struct mmc_host *host, struct mmc_card *card)
{
	...
	for (i = 0; i < 13; i++) {
		...
		if (!status) {
			MSG(INF, "[SD%d] ACMD19_TRG(%d), STS(0x%x) Failed\n", host->id, i,
			       status);
			err = MMC_ERR_FAILED;
			goto out;
		}
...
int msdc_emmc_boot_start(struct mmc_host *host, u32 hz, int ddr, int mode, int ackdis, u8 hostbuswidth, u64 size)
{
	...
	if (!ackdis) {
		do {
			...
			if (sts & EMMC_STS_BOOTACKERR) {
				MSG(ERR, "[%s]: [SD%d] EMMC_STS(0x%x): boot up ack error\n", __func__, host->id, sts);
				err = MMC_ERR_BADCRC;
				goto out;
			} else if (sts & EMMC_STS_BOOTACKRCV) {
				MSG(ERR, "[%s]: [SD%d] EMMC_STS(0x%x): boot ack received\n",  __func__,host->id, sts);
				break;
			} else if (sts & EMMC_STS_BOOTACKTMO) {
#if 0
				test_timer2 = MSDC_READ32(0x10008048);
				test_timer1 = (test_timer2 - test_timer1) /6000;
				MSG(ERR, "[SD%d] EMMC_STS(%x): boot up ack timeout(%d ms)\n", host->id, sts,test_timer1);
				//test MT6583 ACK/DAT timeour modification test case
#endif
				MSG(ERR, "[%s]: [SD%d] EMMC_STS(0x%x): boot up ack timeout\n",  __func__,host->id, sts);
				err = MMC_ERR_TIMEOUT;
				goto out;
			} else if (sts & EMMC_STS_BOOTUPSTATE) {
				//MSG(ERR, "[%s]: [SD%d] EMMC_STS(%x): boot up mode state\n",  __func__, host->id, sts);
			} else {
				MSG(ERR, "[%s]: [SD%d] EMMC_STS(0x%x): boot up unexpected\n",  __func__,host->id, sts);
			}
		} while (1);
	}
	...
	do {
		...
		if (sts & EMMC_STS_BOOTCRCERR) {
			MSG(ERR, "[%s]: [SD%d] EMMC_STS(0x%x): boot up data crc error\n",  __func__,host->id, sts);
			err = MMC_ERR_BADCRC;
			goto out;
			...
			test_timer2 = MSDC_READ32(0x10008048);
			test_timer1 = (test_timer2 - test_timer1) /6000;
			MSG(ERR, "[%s]: [SD%d] EMMC_STS(%x): boot up data timeout(%d s)\n",  __func__,host->id, sts,test_timer1);
			//test MT6583 ACK/DAT timeour modification test case
#endif
			MSG(ERR, "[%s]: [SD%d] EMMC_STS(0x%x): boot up data timeout\n",  __func__,host->id, sts);
			err = MMC_ERR_TIMEOUT;
			goto out;
		}
	} while (1);
...
int msdc_emmc_boot_read(struct mmc_host *host, u64 size, u32 *to, int read_mode)
{
	...
	if (read_mode == MSDC_MODE_PIO) {
		...
		while (size) {
			sts = MSDC_READ32(EMMC_STS);
			if (sts & EMMC_STS_BOOTCRCERR) {
				MSG(ERR, "[SD%d] EMMC_STS(0x%x): boot up data crc error\n", host->id, sts);
				err = MMC_ERR_BADCRC;
				goto out;
			} else if (sts & EMMC_STS_BOOTDATTMO) {
				MSG(ERR, "[SD%d] EMMC_STS(0x%x): boot up data timeout error\n", host->id, sts);
				err = MMC_ERR_TIMEOUT;
				goto out;
			}
			/* Note. RXFIFO count would be aligned to 4-bytes alignment size */
			if ((size >=  MSDC_FIFO_THD) && (MSDC_RXFIFOCNT() >= MSDC_FIFO_THD)) {
				...
				do {
#ifdef MTK_MSDC_DUMP_FIFO
					MSDC_EMMC50_CFG_CRC_STS_SELMSG(INF, "0x%x ",MSDC_FIFO_READ32());
#else
					*to++ = MSDC_FIFO_READ32();
#endif
				} while (--left);
...
out:
		if (err) {
			MSG(ERR, "[SD%d] EMMC_BOOT: read boot code fail(%d), FIFOCNT=%d\n",
		...
		while (left_sz) {
			...
			MSDC_DMA_ON();
			//MSG(FIO, "nblks(%d),xfer_sz(%d),left_sz(%d)\n",nblks,xfer_sz,left_sz);
		...
		if (derr != MMC_ERR_NONE) {
			MSG(ERR, "[SD%d] EMMC boot read error(%d)\n", host->id,derr);
			msdc_abort_handler(host, 1);
		}
	}
...
void msdc_reset_timing_register(struct mmc_host *host)
{
	...
	MSDC_WRITE32(MSDC_PATCH_BIT1, 0xFFE24009);
...
int msdc_init(int id, struct mmc_host *host, int clksrc, int mode)
{
	...
	msdc_pr_info("[%s]: msdc%d Host controller intialization start, mode = %d\n", __func__, id, mode);
	...
	msdc_pr_info("set up GPIO for MSDC\n");
#endif
	...
	*/
	...
	msdc_pr_info("[%s]: msdc%d Host controller intialization done\n", __func__, id);
	return 0;
}
