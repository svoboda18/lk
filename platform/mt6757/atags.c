...
u8 *target_atag_commandline(u8 *buf, size_t buf_size, const char *commandline)
{
	const char *p;
	...
	/*
	 * ATAG size for ATAG_CMDLINE is aligned to a word and the unit of size is 4 bytes.
	 */
	struct tag_header *header = (struct tag_header *)buf;
	size_t cmd_len = strlen(p) + 1;
	size_t size_in_u32s = ROUNDUP(sizeof(struct tag_header) + cmd_len, 4) / 4;

	/* size in words */
	header->size = size_in_u32s;
	header->tag = ATAG_CMDLINE;

	char *data = (char *)(header + 1);
	size_t data_size = buf_size - sizeof(struct tag_header);

	strncpy(data, p, data_size - 1);
	data[data_size - 1] = '\0';

	return buf + (size_in_u32s * 4);
}
...
int target_fdt_model(void *fdt)
{
	...
	*/
	...
	spd = (segment & 0x07);     // bits [0:2]
	mode = (segment & 0x08) >> 3;   // bit [3]
	ca = (segment & 0x10) >> 4; // bit [4]
	id = (segment & 0xE0) >> 5; // bits [7:5]
	...
	dprintf(CRITICAL, "[LK] segment = 0x%x, code = %d\n", segment, code);

	if (code == -1)
		return -1;

	if (model_name[code] == NULL)
		return -1;
