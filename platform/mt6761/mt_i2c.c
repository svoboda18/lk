...
static inline void i2c_writel(mt_i2c * i2c, U16 offset, U16 value)
{
  ...
  mt_reg_sync_writel(value, (i2c->base) + (i2c->multi_offset) + (offset));
}

static inline U32 i2c_readl(mt_i2c * i2c, U16 offset)
{
  return DRV_Reg32((i2c->base) + (i2c->multi_offset) + (offset));
}
...
/***********************************I2C common Param **************************/
volatile U32 I2C_TIMING_REG_BACKUP[8]= {0};
...
static int mtk_i2c_calculate_speed(mt_i2c *i2c,
	unsigned int clk_src_in_khz,
	unsigned int speed_hkz,
	...
	unsigned int *timing_sample_cnt)
{
	...
	if (sclk > khz) {
		I2CERR("i2c%d %s mode: unsupported speed (%ldkhz)\n",
			i2c->id, (speed_hkz > MAX_FS_MODE_SPEED) ?  "HS" : "ST/FT", (long int)khz);
		return -EINVAL_I2C;;
	}

	/* Just for MT6761/MT6762/MT6765 */
...
S32 i2c_set_speed(mt_i2c *i2c)
{
	...
	unsigned int clk_src_in_khz = i2c->clk;
...
void _i2c_dump_info(mt_i2c *i2c)
{
	...
	I2CERR("I2C register:\n"
	       ...
	       I2CTAG"IO_CONFIG=%x,HS=%x,DEBUGSTAT=%x,EXT_CONF=%x,TRANSFER_LEN_AUX=%x,CLOCK_DIV=%x\n",
...
void _i2c_write_reg(mt_i2c *i2c)
{
	...
	i2c_writel(i2c, OFFSET_CLOCK_DIV, 0x404);
	...
	if (i2c->pushpull) {
		...
		i2c_writel(i2c, OFFSET_CONTROL, i2c->control_reg);
		i2c_writel(i2c, OFFSET_IO_CONFIG, 0x01a3);
	}
	...
	i2c_writel(i2c, OFFSET_INTR_STAT, (I2C_HS_NACKERR | I2C_ACKERR | I2C_TRANSAC_COMP));
	/*Clear fifo address*/
	if (i2c->multi_offset != 0)
		   i2c_writel(i2c, OFFSET_FIFO_ADDR_CLR, 0x05);
	else
		   i2c_writel(i2c, OFFSET_FIFO_ADDR_CLR, 0x01);
...
S32 _i2c_check_para(mt_i2c *i2c)
{
	...
	if (i2c->msg_buf == NULL) {
      ...
      /* Just used for multi-channel of MT6761/MT6762/MT6765 */
      switch (i2c->id ) {
      case I2C3:
              if (i2c->addr == 0x57 || i2c->addr == 0x50)
                    i2c->multi_offset = 0x200;/* RT5738A/B */
		 else
			i2c->multi_offset = 0x100;
              break;
      case I2C2:
              i2c->multi_offset = 0x100;
              break;
      ...
      case I2C6:
              ...
              break;
      default:
	  	i2c->multi_offset = 0x00;
             break;
  }

#if 0
  I2CERR(" [i2c%d] addr:0x%x, multi_offset:0x%x\n",
                      (int)i2c->id, (int)i2c->addr, (int)i2c->multi_offset);
#endif
err:
	return ret;
