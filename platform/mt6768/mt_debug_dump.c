...
static void lk_wdt_dump_regs(struct atf_aee_regs *regs)
{
	...
	if (regs->regs[REG_SP]!= 0) {
		unsigned long length, len_ahead = 64;

		if (current_thread->stack_size > 0) {
			length = (unsigned long)(current_thread->stack + current_thread->stack_size)- (unsigned long)regs->regs[REG_SP] + len_ahead;
		...
		dprintf(CRITICAL, "top of stack at 0x%08lx:\n", (unsigned long)regs->regs[REG_SP]);
		hexdump((void*)(unsigned long)(regs->regs[REG_SP] - len_ahead), length);
	}

	dump_target_threads_stack();
}
...
void lk_register_wdt_callback(void)
{
	...
	if (g_boot_arg->boot_mode != DOWNLOAD_BOOT) {
		uint32_t start, size;
#ifdef MTK_SMC_ID_MGMT
		atf_aee_debug_addr = (void *) mt_secure_call(MTK_SIP_LK_WDT, (unsigned long) &lk_wdt_dump, 0, 0, 0);
#else
...
#endif  // !defined(MACH_FPGA) && !defined(FPGA_BOOT_FROM_LK)
}
