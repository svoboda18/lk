...
int fgauge_get_current(bool *fg_is_charging, int *data)
{
	...
	while (fg_get_data_ready_status() == 0) {
		m++;
		if (m > 1000) {
			dprintf(CRITICAL, "[fgauge_read_current] fg_get_data_ready_status timeout1! ret:%d\n", ret);
			break;
		}
	}
	...
	m = 0;
		while (fg_get_data_ready_status() != 0) {
			m++;
			if (m > 1000) {
				dprintf(CRITICAL, "[fgauge_read_current] fg_get_data_ready_status 2! ret:%d\n", ret);
				break;
			}
		}
...
{
	unsigned int reset_sel, new_reset_sel;
	...
	new_reset_sel = pmic_get_register_value(PMIC_RG_FGADC_RST_SRC_SEL);

	dprintf(CRITICAL, "dis_gm3_src_sel: is_dis:%d reset_sel=%d new=%d, temp:%d\n",
		is_dis, reset_sel, new_reset_sel, res);
