		...
		unsigned int wdata, unsigned int *rdata)
{
	...
	if (return_value != 0) {
		PWRAPERR("wait_for_fsm_idle fail, ret=%x\n", return_value);
		goto FAIL;
	}
RETRY:
	wacs_write = write << 31;
	...
	if (write == 0) {
		...
		if (return_value != 0) {
			PWRAPERR("wait_for_fsm_vldclr fail, ret=%x\n", return_value);
			return_value += 1;
			goto FAIL;
		}
...
#else
	pwrap_write_nochk(PMIC_DEW_CRC_EN_ADDR, 0x1);
	pwrap_write_nochk(PMIC_EXT_DEW_CRC_EN_ADDR, 0x1);
	WRAP_WR32(PMIC_WRAP_CRC_EN, 0x1);
...
static void _pwrap_enable(void)
{
	...
	WRAP_WR32(PMIC_WRAP_HPRIO_ARB_EN, 0xfb27f);
#endif
...
static signed int _pwrap_reset_pattern(void)
{
	...
	*/
	...
	WRAP_WR32(PMIC_WRAP_PRIORITY_USER_SEL_0, 0x02030100);
	WRAP_WR32(PMIC_WRAP_ARBITER_OUT_SEL_0, 0x02030100);
