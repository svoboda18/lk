...
#endif
#define CCCI_SMEM_SIZE_UDC_NONCACHE ((7*1024+448)*1024)
#define CCCI_SMEM_SIZE_UDC_CACHE (128*1024)
...
#define MPU_REGION_ID_MD1_ROM           13
#define MPU_REGION_ID_MD_DSP1           14 /*DSP RO*/
#define MPU_REGION_ID_MD_DSP2           15 /*DSP RW*/
#define MPU_REGION_ID_MD_DRDI           16
#define MPU_REGION_ID_MD1_MCURW_HWRW    17
#define MPU_REGION_ID_MD1_MCURW_HWRO    18
#define MPU_REGION_ID_MD1_MCURO_HWRW    19
/* #define MPU_REGION_ID_PADDING1          15 */
#define MPU_REGION_ID_PADDING2          20
#define MPU_REGION_ID_PADDING3          21
#define MPU_REGION_ID_PADDING4          22
/*  #define MPU_REGION_ID_PADDING5          19*/
#define MPU_REGION_ID_MD_PROTECT        30
#define MPU_REGION_ID_MD1_CCB           23
#define MPU_REGION_ID_MD1_SMEM          24
...
static const mpu_att_t mpu_att_default[MPU_REGION_ID_TOTAL_NUM][MPU_MDOMAIN_ID_TOTAL_NUM] = {
	...
	/*11*/{}, /*Tinysys*/
	/*12*/{}, /*AMMS*/
	/*13*/{ SEC_R_NSEC_R,   SEC_R_NSEC_R,  [2 ... 6] = FORBIDDEN, SEC_R_NSEC_R,     [8 ... 15] = FORBIDDEN},
	/*14*/{ SEC_R_NSEC_R,   SEC_R_NSEC_R,  [2 ... 6] = FORBIDDEN, SEC_R_NSEC_R,     [8 ... 15] = FORBIDDEN},
	/*15*/{ SEC_R_NSEC_R,   NO_PROTECTION, [2 ... 6] = FORBIDDEN, NO_PROTECTION,    [8 ... 15] = FORBIDDEN},
	/*16*/{ SEC_R_NSEC_R,   SEC_R_NSEC_R, [2 ... 6] = FORBIDDEN, SEC_R_NSEC_R,    [8 ... 15] = FORBIDDEN}, /*DRDI*/
	/*17*/{ SEC_R_NSEC_R,   NO_PROTECTION, [2 ... 6] = FORBIDDEN, NO_PROTECTION,    [8 ... 15] = FORBIDDEN},
	/*18*/{ SEC_R_NSEC_R,   NO_PROTECTION, [2 ... 6] = FORBIDDEN, SEC_R_NSEC_R,     [8 ... 15] = FORBIDDEN},
	/*19*/{ SEC_R_NSEC_R,   SEC_R_NSEC_R,  [2 ... 6] = FORBIDDEN, NO_PROTECTION,    [8 ... 15] = FORBIDDEN},
	...
	/*22*/{ SEC_R_NSEC_R,   FORBIDDEN,     [2 ... 6] = FORBIDDEN, FORBIDDEN,    [8 ... 15] = FORBIDDEN},
	/*23*/{ NO_PROTECTION,  NO_PROTECTION, [2 ... 6] = FORBIDDEN, NO_PROTECTION,    [8 ... 15] = FORBIDDEN},
	/*24*/{ NO_PROTECTION,  NO_PROTECTION, FORBIDDEN, NO_PROTECTION,[4 ... 6] = FORBIDDEN, NO_PROTECTION,    [8 ... 15] = FORBIDDEN},
...
static const int free_mpu_region[] = {MPU_REGION_ID_PADDING2,
                                      MPU_REGION_ID_PADDING3, MPU_REGION_ID_PADDING4, MPU_REGION_ID_MD_PROTECT, -1
                                     };
...
int plat_apply_hw_remap_for_md_smem(void *addr, int size)
{
	...
	*/
	...
	//plat_amms_pos_notify_secure(addr);
	return 0;
}
...
void plat_post_hdr_info(void* hdr, int ver, int id)
{
	if (id == MD_SYS1) {
		...
		udc_support_at_img = ((struct md_check_header_v6*)hdr)->udc_support;
...
static int cal_share_mem_layout(int load_flag)
{
	...
	unsigned int smem_align_padding_size;
	...
	int smem_dfd_size = CCCI_SMEM_SIZE_DFD;
	...
	if (udc_support_at_img) {
		udc_size.noncache_size = CCCI_SMEM_SIZE_UDC_NONCACHE;
		udc_size.cache_size = CCCI_SMEM_SIZE_UDC_CACHE;

		cacheable_buf_sz = md1_ccb_size + udc_size.cache_size;
	} else
		cacheable_buf_sz = md1_ccb_size;
	/* for cacheable buf */
	if (cacheable_buf_sz != 0)
		cacheable_buf = ccci_request_mem(cacheable_buf_sz,
				0x80000000LL, 0x2000000L);
	if (cacheable_buf == NULL) {
		ccb_info.ccb_data_buffer_addr = 0;
		ccb_info.ccb_data_buffer_size = 0;
		ALWAYS_LOG("allocate ccb data buffer share memory fail\n");
	} else {
		/* ccb_info is cache info which include ccb and udc */
		ccb_info.ccb_data_buffer_addr = (unsigned long long)((unsigned long)cacheable_buf);
		ccb_info.ccb_data_buffer_size = md1_ccb_size + udc_size.cache_size;
		...
		if (insert_ccci_tag_inf("udc_layout", (char*)&udc_size,
			sizeof(struct _udc_info)) < 0)
			ALWAYS_LOG("insert udc_layout fail\n");
	}
	...
	/*    AP    <-->   MD1    */
	{
		...
		if (udc_support_at_img)
			smem_info.ap_md1_smem_size += udc_size.noncache_size;
		 ...
		 */
		smem_info.ap_md1_smem_size += smem_dfd_size;
	...
	if (amms_pos_size_at_img) {
		amms_pos_offset = smem_info.ap_md1_smem_size;
		smem_align_padding_size =
			(smem_info.ap_md1_smem_size + 0x400000 -1)&(~(0x400000 -1));
		smem_align_padding_size -= smem_info.ap_md1_smem_size;
		amms_pos_size_at_img += smem_align_padding_size;
		smem_info.ap_md1_smem_size += amms_pos_size_at_img;
		smem_align_padding_size = 0;
		if (smem_align_padding_size) {
			/*pass smem align padding size to Kernel*/
			if (insert_ccci_tag_inf("smem_align_padding_size",
				(char*)&smem_align_padding_size, sizeof(int)) < 0)
				ALWAYS_LOG("insert pos_align_size fail\n");
		...
		plat_amms_pos_notify_secure((void *)(smem_info.base_addr));
	}
	...
	/*pass DFD size to Kernel*/
	if (insert_ccci_tag_inf("smem_dfd_size", (char*)&smem_dfd_size, sizeof(int)) < 0)
		ALWAYS_LOG("insert smem_dfd_size fail\n");
