...
{
#if 1
	// This function is needed only when it is necessary to limit the
	// number of CPU cores during boot up for thermal reasons.
	char	buf[16];
	dprintf(INFO,"[CPU SMP] Check eFuse value : 0x%x\n", get_devinfo_with_index(120));
	if (((get_devinfo_with_index(120)) & (0x1)) == 0)
		sprintf(buf, "maxcpus=%d", 8);
	else
		sprintf(buf, "maxcpus=%d", 4);
	cmdline_append(buf);
#endif
}
...
int mboot_recovery_load_misc(uint32_t *misc_addr, uint32_t size)
{
	...
	ret = mboot_recovery_load_raw_part("misc", misc_addr, size);
