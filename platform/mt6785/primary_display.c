...
extern void disp_path_clock_on(char *str);
...
void _dump_lcm_info_by_video_printf(void)
{
	...
	if (l && p) {
		...
		if (p->type == LCM_TYPE_DSI) {
			video_printf("[LCM] LANE_NUM: %d, data_format: %d\n",
				p->dsi.LANE_NUM, p->dsi.data_format.format);
#ifdef MY_TODO
#error
#endif
			video_printf("[LCM] vact: %d, vbp: %d, vfp: %d, vact_line: %d,"
			...
			video_printf("[LCM] pll_select: %d, pll_div1: %d,"
				" pll_div2: %d, fbk_div: %d,fbk_sel: %d, rg_bir: %d\n",
				p->dsi.pll_select,
				p->dsi.pll_div1,
				p->dsi.pll_div2,
				p->dsi.fbk_div,
				p->dsi.fbk_sel,
				p->dsi.rg_bir);
			video_printf("[LCM] rg_bic: %d, rg_bp: %d, PLL_CLOCK: %d,"
				...
				p->dsi.cont_clock);
			video_printf("[LCM] lcm_ext_te_enable: %d,"
...
int primary_display_init(char *lcm_name)
{
	...
	memcpy(&(data_config.round_corner_params), &(lcm_param->round_corner_params), sizeof(LCM_ROUND_CORNER));
#endif
...
int primary_display_suspend(void)
{
	...
	if (display_init == 0) {
		dprintf(CRITICAL, "skip primary_display_suspend() before init\n");
		return -1;
	}
...
UINT32 DISP_GetFBRamSize(void)
{
	...
	static bool fb_size_cal = 0;
	static UINT32 frame_buf_size = 0;

	if (!fb_size_cal) {
		UINT32 body_buf_size = ALIGN_TO(DISP_GetScreenWidth(), MTK_FB_ALIGNMENT) * ALIGN_TO(DISP_GetScreenHeight(), MTK_FB_ALIGNMENT) *
					((DISP_GetScreenBpp() + 7) >> 3);

		UINT32 header_buf_size = (ALIGN_TO(DISP_GetScreenWidth(), MTK_FB_ALIGNMENT) * ALIGN_TO(DISP_GetScreenHeight(), MTK_FB_ALIGNMENT)) /
					(OVL_TILE_W * OVL_TILE_H) * OVL_HEADER_SIZE_PER_TILE_BYTES;

		header_buf_size = ALIGN_TO(header_buf_size, OVL_HEADER_ALIGN_BYTES);

		frame_buf_size = (body_buf_size + body_buf_size) * DISP_GetPages();
		fb_size_cal = 1;
	}
...
unsigned int DISP_GetVRamSize(void)
{
	...
	if (0 == vramSize) {
		...
		DISPDBG("^^ DISP_GetFBRamSize: %u bytes\n", vramSize);
		fb_ram_size = vramSize;
		vramSize += DAL_GetLayerSize();
		DISPDBG("^^ DAL_GetLayerSize: %u bytes\n", vramSize-fb_ram_size);
#if defined(MTK_ROUND_CORNER_SUPPORT) && !defined(DISP_HW_RC)
		fb_ram_size = vramSize;
...
#endif
		DISPDBG("^^ DISP_GetVRamSize: %u bytes\n", vramSize);
	}
