...
void rtc_enable_k_eosc(void)
{
	...
	pmic_config_interface(PMIC_EOSC_CALI_START_ADDR, 1, PMIC_EOSC_CALI_START_MASK, PMIC_EOSC_CALI_START_SHIFT);
...
U16 rtc_rdwr_fg_bits(U16 *val)
{
	U16 almth;

	if (val) {
		almth = RTC_Read(RTC_AL_MTH) & ~RTC_AL_MTH_FG_MASK;
		almth |= (*val & (RTC_AL_MTH_FG_MASK >> RTC_AL_MTH_FG_SHIFT)) << RTC_AL_MTH_FG_SHIFT;
		RTC_Write(RTC_AL_MTH, almth);
		rtc_write_trigger();
	}

	return (RTC_Read(RTC_AL_MTH) & RTC_AL_MTH_FG_MASK) >> RTC_AL_MTH_FG_SHIFT;
}
