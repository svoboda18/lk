...
#define CACHABLE_SMEM_MAX_SIZE     (64*1024*1024)
...
static int cal_share_mem_size(void)
{
	...
	if (amms_pos_size_at_img) {
		...
		if (smem_align_padding_size) {
			smem_info.ap_md1_smem_size += smem_align_padding_size;
	...
	if (md1_phy_cap_size > (MAX_SMEM_SIZE - CACHABLE_SMEM_MAX_SIZE - smem_info.total_smem_size))
		md1_phy_cap_size = (MAX_SMEM_SIZE - CACHABLE_SMEM_MAX_SIZE - smem_info.total_smem_size);
	/* 2M for MD MPU include SIB header */
	if (md1_phy_cap_size > 2 * 1024 * 1024)
...
#define SMEM_MD_USIP_SIZE  (384*1024)
#define SMEM_MD_PADDING_SIZE  (640*1024)

static csmem_item_t csmem_layout[] = {
{0, 0, SMEM_MD_CONSYS_SIZE,	 SMEM_USER_RAW_MD_CONSYS},
{0, 0, SMEM_MD_USIP_SIZE,	 SMEM_USER_RAW_USIP},
{0, 0, SMEM_MD_PADDING_SIZE,	 SMEM_USER_NON_PADDING},
...
{
	int i;
	int count = sizeof(ccb_support_tbl) / sizeof(struct ccb_gear_id_mapping);
...
static void set_ccb_gear_val(unsigned int gear_id)
{
	...
	if (ret < 0) {
		...
		ALWAYS_LOG("set env[md1_ccb_cap_gear]%d; ret = %d\n",
				gear_id, ret);
...
static int cal_share_mem_layout(int load_flag)
{
	...
	for (i = 0; i < sizeof(csmem_layout)/sizeof(csmem_item_t); i++) {
		...
		case SMEM_USER_RAW_MD_CONSYS:
			...
			break;
		case SMEM_USER_NON_PADDING:
			if (i != (sizeof(csmem_layout)/sizeof(csmem_item_t) - 1))
				csmem_layout[i].csmem_buffer_size =
					((cacheable_buf_sz + (0x100000 - 1))&(~(0x100000 - 1)))
					- cacheable_buf_sz;
			else
				csmem_layout[i].csmem_buffer_size = 0;
			break;
		...
		if (csmem_layout[i].item_cnt == SMEM_USER_RAW_UDC_DESCTAB)
			csmem_layout[i].csmem_buffer_size = udc_size.cache_size;
		cacheable_buf_sz += csmem_layout[i].csmem_buffer_size;
	}

	if (cacheable_buf_sz != 0)
		cacheable_buf = ccci_request_mem(cacheable_buf_sz, 0x90000000LL, 0x2000000L);

	if (cacheable_buf_sz == NULL) {
		...
		for (i = 0; i < sizeof(csmem_layout)/sizeof(csmem_item_t); i++) {
			if (i == 0) {
				csmem_layout[0].csmem_buffer_addr =
					csmem_info.csmem_buffer_addr;
				csmem_layout[0].md_offset = 0;
			} else {
				cacheable_buf =
					csmem_layout[i - 1].csmem_buffer_addr +
					csmem_layout[i - 1].csmem_buffer_size;
				csmem_layout[i].csmem_buffer_addr =
					(unsigned long long)((unsigned long)cacheable_buf);

				csmem_layout[i].md_offset =
					csmem_layout[i - 1].md_offset +
					csmem_layout[i - 1].csmem_buffer_size;
			}
		...
		ALWAYS_LOG("udc_noncache_size: 0x%x,udc_cache_size: 0x%x\n",
			udc_size.noncache_size, udc_size.cache_size);
	}
...
#define MPU_REGION_ID_PADDING1          19
...
static const mpu_att_t mpu_att_default[MPU_REGION_ID_TOTAL_NUM][MPU_MDOMAIN_ID_TOTAL_NUM] = {
	...
	/*18*/{ SEC_R_NSEC_R,   FORBIDDEN,     [2 ... 6] = FORBIDDEN, FORBIDDEN,    [8 ... 15] = FORBIDDEN}, /* WIFI Driver */
	...
	/*23*/{ SEC_R_NSEC_R,   NO_PROTECTION, NO_PROTECTION, [3 ... 15] = FORBIDDEN},/*MD-Consys Direct Path*/
...
static const int free_mpu_region[] = {
	MPU_REGION_ID_PADDING1, MPU_REGION_ID_PADDING2, MPU_REGION_ID_MD_PROTECT, -1};
...
long long plat_ccci_get_ld_md_plat_setting(char cfg_name[])
{
	if (strcmp(cfg_name, "share_memory_size") == 0) {
		...
		return (long long)cal_share_mem_size();
	}
