...
void mtk_wdt_doe_setup(void)
{
	...
	offset = fdt_path_offset(lk_drv_fdt, TOPRGU_DTS_NAME);
	if (offset < 0) {
		WDTERR("cannot find toprgu dts node, enable apwdt by default\n");
		apwdt_en = 1;
	} else {
		data = fdt_getprop(lk_drv_fdt, offset, "apwdt_en", &len);
		if (len > 0 && data)
...
#else
	apwdt_en = 1;
	WDTLOG("DEVICE_TREE_SUPPORT disabled, \n");
#endif
	if (apwdt_en == 0) {
		mtk_wdt_disable();
