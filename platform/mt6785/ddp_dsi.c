...
void DSI_exit_ULPS(DISP_MODULE_ENUM module)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (_dsi_context[i].dsi_params.IsCphy) {
			DSI_OUTREGBIT(NULL, struct DSI_CPHY_CON0_REG,
				DSI_REG[i]->DSI_CPHY_CON0, CPHY_EN, 1);
...
void DSI_clk_HS_mode(DISP_MODULE_ENUM module, cmdqRecHandle cmdq, bool enter)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (enter) {
#ifdef MACH_FPGA
                        DSI_OUTREG32(NULL, DSI_REG_BASE[i] + DISP_REG_DSI_PHY_LCPAT, 0x55);
                        DSI_OUTREG32(NULL, DSI_REG_BASE[i] + DISP_REG_DSI_PHY_LCCON, 0);
#endif
...
void DSI_CPHY_clk_switch(DISP_MODULE_ENUM module, void* cmdq, int on)
{
#ifndef MACH_FPGA
	int i = 0;

	DDPDUMP("===>DSI_CPHY_clk_switch \n");
...
void DSI_DPHY_clk_switch(DISP_MODULE_ENUM module, void* cmdq, int on)
{
#ifndef MACH_FPGA
	int i = 0;
...
enum DSI_STATUS DSI_BIST_Pattern_Test(DISP_MODULE_ENUM module, void* cmdq, bool enable, unsigned int color)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (enable) {
			...
			dprintf(INFO, "DSI_BIST_Pattern_Test SELF_PAT_MODE\n");
...
void DSI_DPHY_Calc_VDO_Timing(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params)
{
	int i = 0;
...
void DSI_CPHY_Calc_VDO_Timing(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params)
{
	int i = 0;
	//unsigned int line_byte;
	unsigned int horizontal_sync_active_byte;
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (dsi_params->mode == SYNC_EVENT_VDO_MODE || dsi_params->mode == BURST_VDO_MODE
			...
			if (dsi_params->horizontal_sync_active * dsiTmpBufBpp < 10 * lane_num + 26 + 5)
				horizontal_sync_active_byte = 4;
			else
				horizontal_sync_active_byte =
			    		(dsi_params->horizontal_sync_active * dsiTmpBufBpp - 10 * lane_num - 26);

			if (dsi_params->horizontal_backporch * dsiTmpBufBpp < 12 * lane_num + 26 + 5)
				horizontal_backporch_byte = 4;
			else
				horizontal_backporch_byte =
			    		(dsi_params->horizontal_backporch * dsiTmpBufBpp - 12 * lane_num - 26);
		}

		data_phy_cycle = _dsi_context[i].data_phy_cycle;

		if (dsi_params->horizontal_frontporch * dsiTmpBufBpp < 8 * lane_num + 28 + 2 * data_phy_cycle *lane_num  + 9)
			horizontal_frontporch_byte = 8;
		else if ((dsi_params->horizontal_frontporch * dsiTmpBufBpp > 8 * lane_num + 28 + 2 * data_phy_cycle *lane_num  + 8) &&
			(dsi_params->horizontal_frontporch * dsiTmpBufBpp < 8 * lane_num + 28 + 2 * data_phy_cycle *lane_num  + 2*(_dsi_context[i].HS_TRAIL + 1)*lane_num - 6*lane_num - 12))
			horizontal_frontporch_byte = 2*(_dsi_context[i].HS_TRAIL + 1)*lane_num - 6*lane_num - 12;
		else
			horizontal_frontporch_byte = dsi_params->horizontal_frontporch * dsiTmpBufBpp - 8 * lane_num - 28 - 2 * data_phy_cycle * lane_num;
		...
		DSI_OUTREG32(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_BLLP_WC, ALIGN_TO((horizontal_bllp_byte), 2));
	}
}
...
void DSI_Config_VDO_Timing(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (_dsi_context[i].dsi_params.IsCphy)
			DSI_CPHY_Calc_VDO_Timing(module, cmdq, dsi_params);
		else
			DSI_DPHY_Calc_VDO_Timing(module, cmdq, dsi_params);
	}
}
...
void fill_mipitx_reg1(unsigned int value, unsigned int addr, unsigned num)
{
	unsigned int i = 0;
	unsigned int t_value = 0, t_addr = 0;

	for (i = 0; i < num; i++) {
		t_addr = addr + i * 0x4;
		t_value = (value >> i) & 0x1;
		MIPITX_OUTREG32(t_addr, t_value);
	}
}

void fill_mipitx_reg2(unsigned int value, unsigned int addr)
{
	unsigned int i = 0;
	unsigned t_value = 0;

	for (i = 0; i < 3; i++) {
		t_value |= (((value >> i) & 0x1) << i * 8);
	}

	MIPITX_OUTREG32(addr, t_value);
}

void fill_mipitx_impedance(DISP_MODULE_ENUM module, void* cmdq)
{
	unsigned int tmp = 0, i = 0, j = 0;
	unsigned int value = 0;
	unsigned int mipitx_base;

	/* fix wrong default value */
	for (i = 0x1CC; i <= 0x5CC; i += 0x100) {
		mipitx_base = DSI_PHY_REG[0] + i;
		MIPITX_OUTREG32(mipitx_base, 0x100);
	}

	/* 0x11C10190 */
	value = INREG32(0x11C10190);
	if (value) {
		fill_mipitx_reg1(value & 0xf, 0x11E50214, 4);
		fill_mipitx_reg1((value >> 4)  & 0xf, 0x11E50200, 4);
		fill_mipitx_reg1((value >> 8)  & 0xf, 0x11E50114, 4);
		fill_mipitx_reg1((value >> 12) & 0xf, 0x11E50100, 4);
	}

	/* 0x11C10194 */
	value = INREG32(0x11C10194);
	if (value) {
		fill_mipitx_reg1((value >> 8)   & 0xf, 0x11E50514, 4);
		fill_mipitx_reg1((value >> 12)  & 0xf, 0x11E50500, 4);
		fill_mipitx_reg1((value >> 16)  & 0xf, 0x11E50414, 4);
		fill_mipitx_reg1((value >> 20)  & 0xf, 0x11E50400, 4);
		fill_mipitx_reg1((value >> 24)  & 0xf, 0x11E50314, 4);
		fill_mipitx_reg1((value >> 28)  & 0xf, 0x11E50300, 4);
	}

	/* 0x11C10198 */
	value = INREG32(0x11C10198);
	if (value) {
		fill_mipitx_reg2((value >> 2)  & 0x7, 0x11E505CC);
		fill_mipitx_reg2((value >> 5)  & 0x7, 0x11E505C8);
		fill_mipitx_reg2((value >> 8)  & 0x7, 0x11E504CC);
		fill_mipitx_reg2((value >> 11) & 0x7, 0x11E504C8);
		fill_mipitx_reg2((value >> 14) & 0x7, 0x11E503CC);
		fill_mipitx_reg2((value >> 17) & 0x7, 0x11E503C8);
		fill_mipitx_reg2((value >> 20) & 0x7, 0x11E502CC);
		fill_mipitx_reg2((value >> 23) & 0x7, 0x11E502C8);
		fill_mipitx_reg2((value >> 26) & 0x7, 0x11E501CC);
		fill_mipitx_reg2((value >> 29) & 0x7, 0x11E501C8);
	}
}

void DSI_CPHY_clk_setting(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params)
{
	int i = 0;
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (dsi_params->lane_swap_en) {
			MIPITX_OUTREG32(DSI_PHY_REG[i]+MIPITX_PHY_SEL0, dsi_params->PHY_SEL0);
			MIPITX_OUTREG32(DSI_PHY_REG[i]+MIPITX_PHY_SEL1, dsi_params->PHY_SEL1);
			MIPITX_OUTREG32(DSI_PHY_REG[i]+MIPITX_PHY_SEL2, dsi_params->PHY_SEL2);
			MIPITX_OUTREG32(DSI_PHY_REG[i]+MIPITX_PHY_SEL3, dsi_params->PHY_SEL3);
		} else {
...
void DSI_DPHY_clk_setting(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params)
{
	int i = 0;
	unsigned int j = 0;
	unsigned int k = 0;
	...
	/* struct mipitx_impedance m_mipitx_impedance[3]; */
	unsigned int mipitx_base = 0;
	unsigned int mipitx_addr = 0;
	unsigned int mipitx_value = 0;
	unsigned int tmp;
	unsigned int efuse_addr[3] = {0x11C10190, 0x11C10194, 0x11C10198};
	...
	fill_mipitx_impedance(module, cmdq);

#if 0
	/* fill mipitx impedance */
	m_mipitx_impedance[0] = (struct mipitx_impedance){3, 0x11F10190, 11};
	m_mipitx_impedance[1] = (struct mipitx_impedance){6, 0x11F10194, 27};
	m_mipitx_impedance[2] = (struct mipitx_impedance){1, 0x11F10198, 27};

	mipitx_base = DSI_PHY_REG[0]+MIPITX_D2P_RTCODE0;
	for (i = 0; i < 3; i++) {
		mipitx_value = INREG32(m_mipitx_impedance[i].base);
		for (j = 0; j < m_mipitx_impedance[i].num_to_fill; j++) {
			tmp = (mipitx_value>>(m_mipitx_impedance[i].offset_start-j*5))&0x1F;
			if (tmp == 0)
			    tmp = 0x10;
...
void DSI_CPHY_TIMCONFIG(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params)
{
...
#define NS_TO_CYCLE(n, c)   ((n) / (c))
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DISP_REG_SET_FIELD(cmdq, DSI_PHY_TIMCON2_FLD_DA_HS_SYNC, DSI_REG_BASE[i] + DISP_REG_DSI_PHY_TIMCON2, 0x06);
...
void DSI_DPHY_TIMCONFIG(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params)
{
	struct DSI_PHY_TIMCON0_REG timcon0;
	struct DSI_PHY_TIMCON1_REG timcon1;
	struct DSI_PHY_TIMCON2_REG timcon2;
...
int ddp_dsi_config(DISP_MODULE_ENUM module, disp_ddp_path_config *config, void *cmdq_handle)
{
	...
	if (config->dsi_config.mode != CMD_MODE ||( (config->dsi_config.switch_mode_enable == 1) && (config->dsi_config.switch_mode != CMD_MODE))) {
		...
		if (config->dsi_config.mode != CMD_MODE && config->dsi_config.lfr_enable) {
			...
			dprintf(CRITICAL,"lfr en %d mode= %d type=%d,skip_num %d\n",enable,mode,type,skip_num);
...
{
	int rst_pin;
	rst_pin = lcm_util_get_pin("lcm_rst_out0_gpio");
	if (rst_pin < 0) {
...
#else
	lcd_enp_pin = lcm_util_get_pin("lcd_bias_enp0_gpio");
	lcd_enn_pin = lcm_util_get_pin("lcd_bias_enn0_gpio");
