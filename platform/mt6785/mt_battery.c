...
void gauge_read_IM_current(void *data)
{
		...
		dprintf(CRITICAL,"[gauge_read_IM_current] : FG_CURRENT = %x\r\n",
			 uvalue16);
		...
		if (g_fg_is_charging == true)
			dprintf(CRITICAL,
			"[gauge_read_IM_current] current(charging) = %d mA\r\n",
			dvalue);
		else
			dprintf(CRITICAL,
			"[gauge_read_IM_current] current(discharging) = %d mA\r\n",
			dvalue);

		/* Auto adjust value */
		if (R_FG_VALUE != 100) {
			dprintf(CRITICAL,
			"[gauge_read_IM_current] Auto adjust value due to the Rfg is %d\n Ori current=%d, ",
			R_FG_VALUE, dvalue);

			dvalue = (dvalue * 100) / R_FG_VALUE;

			dprintf(CRITICAL,"[gauge_read_IM_current] new current=%d\n",
			dvalue);
		}

		dprintf(CRITICAL,"[gauge_read_IM_current] ori current=%d\n", dvalue);

		dvalue = ((dvalue * CAR_TUNE_VALUE) / 1000);

		dprintf(CRITICAL,"[gauge_read_IM_current] final current=%d (ratio=%d)\n",
