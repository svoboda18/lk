...
##############################################################################
# The setting DEVELOP_STAGE may be overwritten by the one in project make file.
##############################################################################
### DEVELOP_STAGE ?= FPGA
ifeq ($(DEVELOP_STAGE), FPGA)
DEFINES += MACH_FPGA
# DEFINES += FPGA_BOOT_FROM_LK  # Turn this option OFF when jump from Preloader to LK. Otherwise, turn it ON.
# DEFINES += FPGA_SET_BOOT_ARGUMENT  # Turn this option ON only for LK self test (boot from LK).
# DEFINES += FPGA_PRINT_BOOT_ARGUMENT  # Print boot arguments for FPGA early porting
# DEFINES += MACH_FPGA_NO_DISPLAY #Turn off this option when lk display ready
...
# DEFINES += MSDC_ENABLE_DMA_MODE  # Turn this option ON by MSDC owners when DMA mode is ready.
# DEFINES += NO_PMIC
# DEFINES += NO_BOOT_MODE_SEL
DEFINES += NO_PLL_TURN_OFF
# DEFINES += NO_BAT_INIT
...
DEFINES += CONFIG_UART_FPGA_CLK=10000000
...
ifeq ($(MTK_INDIRECT_ACCESS_SUPPORT), yes)
	DEFINES += MTK_INDIRECT_ACCESS_SUPPORT
endif
...
ifeq ($(MTK_MCIF_SUPPORT), yes)
        DEFINES += MTK_MCIF_SUPPORT
endif

MTK_SECURITY_OEM_WITH_DMCERT_ONLY_SUPPORT := no
ifeq ($(MTK_SECURITY_OEM_WITH_DMCERT_ONLY_SUPPORT), yes)
	DEFINES += MTK_SECURITY_OEM_WITH_DMCERT_ONLY_SUPPORT
endif
...
MTK_SSUSB := yes
ifeq ($(MTK_SSUSB), yes)
INCLUDES += -I$(LK_TOP_DIR)/platform/common/mt_ssusb
OBJS += \
	$(LOCAL_DIR)/mt_ssusb/mt_ssusbphy.o

MTK_SSUSBPHY_A60810 := yes
DEFINES += USB3_PHY_SIFV2
#DEFINES += SUPPORT_U3
endif

DEFINES += MTK_PICACHU_SUPPORT
...
OBJS += \
	...
	$(LOCAL_DIR)/ccci_lk_md_power_ctrl.o \
...
ifeq ($(MTK_DFD_ENABLE_CACHE_DUMP), yes)
DEFINES += MTK_DFD_ENABLE_CACHE_DUMP
endif

ifeq ($(MTK_INFRA_REG_SET_CLR), yes)
DEFINES += MTK_INFRA_REG_SET_CLR
endif
...
OBJS += $(LOCAL_DIR)/mt_adsp.o
DEFINES += MTK_AUDIODSP_SUPPORT
endif

ifeq ($(SPM_FW_USE_PARTITION), yes)
DEFINES += SPM_FW_USE_PARTITION
DEFINES += SPM_FW_USE_MD32
endif
...
ifeq ($(MTK_ILDO_SUPPORT),yes)
OBJS += $(LOCAL_DIR)/mt_ildo.o
endif

ifeq ($(MTK_ENABLE_GENIEZONE),yes)
DEFINES += WITH_GZ_MD_SHAREMEM=1
...
MTK_MT6360_PMIC_SUPPORT := yes
ifeq ($(MTK_MT6360_PMIC_SUPPORT),yes)
	OBJS += $(LOCAL_DIR)/mt6360_pmic.o
