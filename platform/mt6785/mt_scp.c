...
/******************************************************************************
* AP read/write SCP SRAM through APB bus which can only read/write data in
* 4-byte unit.
******************************************************************************/
void scp_set_platform_name(struct scp_region_info_st *scp_region_info)
{
	int index, max_idx, max_word_idx;
	char *chip = PLATFORM;
	unsigned int *platform = (unsigned int*)scp_region_info->platform;
	/* Use 32-bit type so that "words" is aligned. Round-up to ensure enough space. */
	unsigned int words[(MAX_PLATFORM_LEN + 3) / 4];
	char *byte_in_words = (char*)words;

	memset(words, 0, sizeof(words));

	max_idx = MAX_PLATFORM_LEN - 1;  // for NULL
	for (index = 0; index < max_idx; index++) {
		byte_in_words[index] = toupper(chip[index]);
		if (chip[index] == '\0')
			break;
	}
	byte_in_words[index] = '\0';

	/* Copy data in word because of APB bus limitation. */
	max_word_idx = (MAX_PLATFORM_LEN / 4);
	for (index = 0; index < max_word_idx; index++) {
		platform[index] = words[index];
	}
}
...
static int verify_load_scp_image(char *part_name, void *addr)
{
	...
	*/
	...
	if (l1c_size > 0) {
		 ...
		 * size, SCP_L1C_BACKUO_OFFSET - SCP_L1C_OFFSET.
		 */
		if (l1c_size > SCP_L1C_BACKUO_OFFSET - SCP_L1C_OFFSET)
			ASSERT(0);
	...
	************************/
	...
	scp_set_platform_name(scp_region_info);
	...
	dprintf(INFO, "scp_region_info->platform: %s\n", scp_region_info->platform);
