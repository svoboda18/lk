...
#define CCB_DATA_BUF_DEFAULT_GEAR  2 /* NOTE: This value may be different at different platform */
...
typedef struct _mpu_cfg {
	...
	unsigned int end;
	int region;
	mpu_att_t permission;
	int relate_region;
} mpu_cfg_t;

#define MPU_REGION_ID_MD1_ROM           8
#define MPU_REGION_ID_MD1_MCURW_HWRO    9
#define MPU_REGION_ID_MD1_MCURO_HWRW    10
#define MPU_REGION_ID_MD1_MCURW_HWRW    13
#define MPU_REGION_ID_MD_DRDI           7 /*DRDI*/
#define MPU_REGION_ID_MD_DSP1           11 /*DSP RO*/
#define MPU_REGION_ID_MD_DSP2           12 /*DSP RW*/
/* #define MPU_REGION_ID_PADDING1          13 */
#define MPU_REGION_ID_PADDING2          14
#define MPU_REGION_ID_PADDING3          15
#define MPU_REGION_ID_PADDING4          16
#define MPU_REGION_ID_PADDING5          17
#define MPU_REGION_ID_MD_PROTECT        18
#define MPU_REGION_ID_MD1_CCB           19
#define MPU_REGION_ID_MD1_SMEM          20
...
static const mpu_att_t mpu_att_default[MPU_REGION_ID_TOTAL_NUM][MPU_MDOMAIN_ID_TOTAL_NUM] = {
	/*===================================================================================================================*/
	/* No |  | D0(AP)    | D1(MD1)      | D2(CONN) | D3(Rsv)  | D4(MM)       | D5(Rsv )      | D6(MFG)      | D7(MDHW)  |*/
	/*--------------+----------------------------------------------------------------------------------------------------*/
	/* 0*/{}, /*Secure OS*/
	/* 1*/{}, /*ATF*/
	/* 2*/{}, /*Secure Memory*/
	/* 3*/{}, /*Set in TEE Trust UI*/
	/* 4*/{}, /*Security CCCI share memory(MD0 Secure Share)*/
	/* 5*/{}, /*Security CCCI share memory(MD1 Secure Share)*/
	/* 6*/{}, /*MD Dynamic*/
	/* 7*/{ SEC_R_NSEC_R,	SEC_R_NSEC_R,  [2 ... 6] = FORBIDDEN, SEC_R_NSEC_R},
	/* 8*/{ SEC_R_NSEC_R,	SEC_R_NSEC_R,  [2 ... 6] = FORBIDDEN, SEC_R_NSEC_R},
	/* 9*/{ SEC_R_NSEC_R,	NO_PROTECTION, [2 ... 6] = FORBIDDEN, SEC_R_NSEC_R},
	/*10*/{ SEC_R_NSEC_R,	SEC_R_NSEC_R,  [2 ... 6] = FORBIDDEN, NO_PROTECTION},
	/*11*/{ SEC_R_NSEC_R,	SEC_R_NSEC_R,  [2 ... 6] = FORBIDDEN, SEC_R_NSEC_R},
	/*12*/{ SEC_R_NSEC_R,	NO_PROTECTION, [2 ... 6] = FORBIDDEN, NO_PROTECTION},
	/*13*/{ SEC_R_NSEC_R,	NO_PROTECTION, [2 ... 6] = FORBIDDEN, NO_PROTECTION},
	/*14*/{ SEC_R_NSEC_R,	FORBIDDEN,     [2 ... 6] = FORBIDDEN, FORBIDDEN},
	/*15*/{ SEC_R_NSEC_R,	FORBIDDEN,     [2 ... 6] = FORBIDDEN, FORBIDDEN},
	/*16*/{ SEC_R_NSEC_R,	FORBIDDEN,     [2 ... 6] = FORBIDDEN, FORBIDDEN},
	/*17*/{ SEC_R_NSEC_R,	FORBIDDEN,     [2 ... 6] = FORBIDDEN, FORBIDDEN},
	/*18*/{ NO_PROTECTION,	SEC_R_NSEC_R,  [2 ... 3] = FORBIDDEN, NO_PROTECTION, FORBIDDEN, SEC_R_NSEC_RW,
		[7] = FORBIDDEN},
	/*19*/{ NO_PROTECTION,	NO_PROTECTION, [2 ... 6] = FORBIDDEN, NO_PROTECTION},
	/*20*/{ NO_PROTECTION,	NO_PROTECTION, [2 ... 6] = FORBIDDEN, NO_PROTECTION},
	/*21*/{}, /*WIFI EMI FW*/
	/*22*/{},/*WDT*/
	/*23*/{ NO_PROTECTION,	FORBIDDEN,  [2 ... 3] = FORBIDDEN, NO_PROTECTION, FORBIDDEN, SEC_R_NSEC_RW,
		[7] = FORBIDDEN},
};

#define MPU_STR_BUF_SIZE	64

static void get_mpu_attr_str(int lock, mpu_att_t curr_attr, char buf[], int size)
{
	char ch = lock?'L':'U';
...
static mpu_att_t get_mpu_region_default_access_att(int region, int lock)
{
...
static unsigned long long mpu_attr_calculate(int region_id, unsigned int request_attr)
{
	...
	/* MPU region lock */
	return SET_ACCESS_PERMISSON(1, tmp_mpu_att[7], tmp_mpu_att[6],
	...
	while (curr->region != -1) {
		if (clear) {
			emi_mpu_set_region_protection(  0,      /*START_ADDR*/
			                                ...
			                                curr->region,   /*region*/
			                                curr_attr);
			ALWAYS_LOG("Clr MPU:S:0x%x E:0x%x A:<%d>[0~7]%lld-%lld-%lld-%lld-%lld-%lld-%lld-%lld\n",  0, 0,
...
void plat_notify_secure(unsigned long base_addr)
{
	...
	mt_secure_call(MTK_SIP_LK_AMMS_MD_BASE_ADDR_AARCH32, base_addr, 0, 0);
	addr_get = mt_secure_call(MTK_SIP_LK_AMMS_GET_MD_BASE_ADDR_AARCH32, 0, 0, 0);
...
static int md1_mpu_setting_process(void *p_md_ld_info, void *p_mem_info, mpu_cfg_t *mpu_tbl_hdr)
{
	...
	if (didi_region_idx >= 0) {
		mpu_tbl_hdr[curr_idx].permission = get_mpu_region_default_access_att(MPU_REGION_ID_MD_DRDI, 1);
...
int plat_send_mpu_info_to_platorm(void *p_md_ld_info, void *p_mem_info)
{
	...
	if (md_id == MD_SYS1) {
		...
		/* RO part */
		mpu_tbl[s_g_curr_mpu_num].permission = get_mpu_region_default_access_att(MPU_REGION_ID_MD_DSP1, 1);
		...
		/* RW part */
		mpu_tbl[s_g_curr_mpu_num].permission = get_mpu_region_default_access_att(MPU_REGION_ID_MD_DSP2, 1);
...
static int cal_share_mem_layout(int load_flag)
{
	...
	ALWAYS_LOG("allocate ccb data buffer 0x%x [%d]\n", md1_ccb_size, md1_ccb_cap_gear);
	...
	if (md1_ccb_size != 0)
		ccb_data_buf = ccci_request_mem(md1_ccb_size, 0x78000000LL, 0x2000000L);
...
int plat_apply_platform_setting(int load_md_flag)
{
	...
	mpu_tbl[s_g_curr_mpu_num].end = (unsigned int)smem_info.base_addr + smem_info.ap_md1_smem_offset
	                                + smem_info.ap_md1_smem_size + md1_phy_cap_size;
	mpu_tbl[s_g_curr_mpu_num].end = ((mpu_tbl[s_g_curr_mpu_num].end + 0xFFFF)&(~0xFFFF)) - 1;
	s_g_curr_mpu_num++;

	/* add for ccb data buffer mpu */
	mpu_tbl[s_g_curr_mpu_num].region = MPU_REGION_ID_MD1_CCB;
	mpu_tbl[s_g_curr_mpu_num].permission = get_mpu_region_default_access_att(MPU_REGION_ID_MD1_CCB, 0);
	...
	mpu_tbl[s_g_curr_mpu_num].end = (unsigned int)ccb_info.ccb_data_buffer_addr + ccb_info.ccb_data_buffer_size;
		//ccb_data_buf + CCB_DATA_BUF_SIZE;
	mpu_tbl[s_g_curr_mpu_num].end = ((mpu_tbl[s_g_curr_mpu_num].end + 0xFFFF)&(~0xFFFF)) - 1;
	s_g_curr_mpu_num++;
