...
{
	int i = 0, module;
	unsigned int *modules = ddp_get_scenario_list(scenario);
	...
	for (i = 0; i < module_num; i++) {
		...
		if (ddp_modules_driver[module] && ddp_modules_driver[module]->connect) {
			int prev = i == 0 ? DISP_MODULE_NONE: modules[i - 1];
			int next = i == module_num - 1 ? DISP_MODULE_NONE : modules[i + 1];
...
int dpmgr_destroy_path(disp_path_handle dp_handle)
{
	...
	DDP_MANAGER_CONTEXT * content = _get_context();
	DISP_LOG_I("destroy path handle 0x%p on scenario %s\n",handle,ddp_get_scenario_name(handle->scenario));
	if (handle != NULL) {
...
int dpmgr_path_add_dump(disp_path_handle dp_handle, ENGINE_DUMP engine,int encmdq )
{
	...
	DISP_MODULE_ENUM wdma = ddp_is_scenario_on_primary(handle->scenario) ? DISP_MODULE_WDMA0: DISP_MODULE_WDMA0;
	// update contxt
	DDP_MANAGER_CONTEXT * context = _get_context();
...
int dpmgr_path_config(disp_path_handle dp_handle, disp_ddp_path_config *config, int encmdq)
{
	...
	for (i=0; i< module_num; i++) {
		...
		if (ddp_modules_driver[module_name] != 0) {
			...
			/* now all PQ modules are bypassed by itself in LK, don't need this, just for future using */
...
int dpmgr_check_status(disp_path_handle dp_handle)
{
	...
	ddp_path_handle handle = (ddp_path_handle)dp_handle;
	unsigned int * modules = ddp_get_scenario_list(handle->scenario);
	unsigned int module_num = ddp_get_module_num(handle->scenario);

	/*dump log may take a long time, prevent WDT*/
	DISP_LOG_E("%s, feed dog in display, prevent WDT\n", __func__);
	mtk_wdt_restart();
