...
#ifdef CFG_ENABLE_PICACHU
extern void picachu_init(void);
#endif
		...
		for (tags = (void *)BOOT_ARGUMENT_LOCATION; tags->hdr.size; tags = boot_tag_next(tags)) {
			switch (tags->hdr.tag) {
				...
				case BOOT_TAG_PLAT_DBG_INFO:
					...
					for (i = 0; i < (int)(g_boot_arg->plat_dbg_info_max); i++) {
				...
				case BOOT_TAG_EMI_INFO:
					g_boot_arg->emi_info.dram_type = tags->u.emi_info.dram_type;
					g_boot_arg->emi_info.ch_num = tags->u.emi_info.ch_num;
					g_boot_arg->emi_info.rk_num = tags->u.emi_info.rk_num;
					break;
...
void platform_init(void)
{
	...
	dprintf(CRITICAL," MACH_TYPE[0d%d]\n",  MACH_TYPE);
	dprintf(CRITICAL," ==LK info ==\n");
	...
	PROFILING_START("smi apply register");
	...
	if (g_boot_arg->boot_mode == DOWNLOAD_BOOT) {
		...
		if(g_boot_mode != META_BOOT){
			PROFILING_START("disp update");
			mt_disp_update(0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT); /* show logo */
			PROFILING_END();
		}
#endif
