...
{
	// constant
	static const unsigned int blank_overhead = 125; //it is 1.15, need to divide 100 later
	...
	ultra_high_level = pre_ultra_low_level;
	...
	DISP_REG_SET(handle, idx*DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_MEM_GMC_SETTING_0,
	             pre_ultra_low_level|(pre_ultra_high_level<<16) | (1<<30));
	DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_1,
					 ultra_low_level|(ultra_high_level<<16) | (1<<30));
	...
	if (1) { //UFOE bypassed, enable RDMA underflow intr, else disable RDMA underflow intr
		...
		DISP_REG_SET_FIELD(handle,FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD, idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_FIFO_CON, 78);
	} else {
		DISP_REG_SET_FIELD(handle,FIFO_CON_FLD_FIFO_UNDERFLOW_EN, idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_FIFO_CON, 0);
	...
	DISP_REG_SET_FIELD(handle,FIFO_CON_FLD_FIFO_PSEUDO_SIZE, idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_FIFO_CON, fifo_size);
#endif
               ...
               void * handle)  // ourput setting
{
	...
	/*Becareful, refer to golden setting*/
	rdma_set_ultra(idx, width, height, bpp, 60, handle);

	/*Check rdma setting*/
	fifo_size = DISP_REG_GET_FIELD(FIFO_CON_FLD_FIFO_PSEUDO_SIZE, idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_FIFO_CON);
	output_valid = DISP_REG_GET_FIELD(FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD, idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_FIFO_CON);
	if(output_valid > fifo_size) {
		DDPERR("RDMA Fifo_size: %d is SMALLER than output_valid: %d \n",fifo_size, output_valid);
		ASSERT(0);
	}
...
{
	unsigned int idx = rdma_index(module);
	unsigned long module_base = DISP_REG_RDMA_INT_ENABLE + DISP_RDMA_INDEX_OFFSET * idx;
