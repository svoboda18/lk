	...
	for (i = 1; i < module_num; i++) {
		for (j = 0; j < DDP_SEL_IN_NUM; j++) {
			if (module_list[i] == sel_in_map[j].id) {
				...
				ASSERT(step >= 0);
				for (k = 0; k < 4; k++) {
					if ((int)sel_in_map[j].id_bit_map[k] == -1)
						break;
...
{
	DISP_MODULE_ENUM module_name = DISP_MODULE_NONE;
