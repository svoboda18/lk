...
static int config_color(DISP_MODULE_ENUM module, disp_ddp_path_config* pConfig, void* cmdq)
{
	...
	dprintf(ALWAYS, "config color dirty = %d\n", pConfig->dst_dirty);
...
{
	dprintf(ALWAYS, "config ccorr dirty = %d\n", pConfig->dst_dirty);
	if (!pConfig->dst_dirty) {
		return 0;
	}
...
{
	dprintf(ALWAYS, "config aal dirty = %d\n", pConfig->dst_dirty);
	if (!pConfig->dst_dirty) {
		return 0;
	}
...
{
	dprintf(ALWAYS, "config gamma dirty = %d\n", pConfig->dst_dirty);
	if (!pConfig->dst_dirty) {
		return 0;
	}
