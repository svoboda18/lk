...
#define RT9466_LK_DRV_VERSION "1.0.6_MTK"
...
static int rt9466_i2c_block_read(struct rt9466_info *info, u8 cmd, int len,
	u8 *data)
{
	struct mt_i2c_t *i2c = &info->i2c;
...
static int rt9466_reset_chip(struct rt9466_info *info)
{
	...
	ret = rt9466_clr_bit(info, RT9466_REG_CHG_CTRL1, RT9466_MASK_HZ_EN);
	if (ret < 0)
...
static int rt9466_set_battery_voreg(struct rt9466_info *info, u32 voreg)
{
	...
	reg_voreg = rt9466_closest_reg(RT9466_BAT_VOREG_MIN,
		RT9466_BAT_VOREG_MAX, RT9466_BAT_VOREG_STEP, voreg);

	dprintf(CRITICAL, "%s: bat voreg = %d\n", __func__, voreg);

	return rt9466_i2c_update_bits(info, RT9466_REG_CHG_CTRL4,
		reg_voreg << RT9466_SHIFT_BAT_VOREG, RT9466_MASK_BAT_VOREG);
}
...
{
	dprintf(INFO, "%s\n", __func__);
	return (enable ? rt9466_set_bit : rt9466_clr_bit)
...
static int rt9466_init_setting(struct rt9466_info *info)
{
	...
	ret = rt9466_i2c_block_read(info, RT9466_REG_CHG_STATC, 5, evt);
	if (ret != I2C_OK)
		dprintf(CRITICAL, "%s: read irq fail\n", __func__);
...
static int rt_charger_set_mivr(struct mtk_charger_info *mchr_info, u32 mivr)
{
	...
	/* Find corresponding reg value */
	u8 reg_mivr = rt9466_closest_reg(RT9466_MIVR_MIN, RT9466_MIVR_MAX,
...
static int rt_charger_sw_reset(struct mtk_charger_info *mchr_info)
{
	...
	ret = rt9466_mask_all_irq(info);
	if (ret != I2C_OK)
		dprintf(CRITICAL, "%s: mask irq failed\n", __func__);
 ...
 * Revision Note
 * 1.0.6
 * (1) Modify function names & make code simpler
 *
 * 1.0.5
 * (1) Modify init sequence, select ilimit to AICR -> disable HW ilim
 * (2) Add i2c_block_read
 * (3) Disable HZ before reset chip
 *
 * 1.0.4
