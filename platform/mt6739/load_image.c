...
int mboot_android_load_bootimg_hdr(char *part_name, uint32_t addr)
{
	...
	if (g_is_64bit_kernel == 1) {
		...
		snprintf(cmdline, UART_EARLY_CONSOLE_LEN, " earlyprintk=mt6739,0x%x ", AP_UART0_BASE);
	}
...
int mboot_load_mntl(void)
{
	...
	u32 koaddr;
	char *buf;
	union mkimg_hdr *img_hdr;
	int *hdr_img_size;
#ifdef MTK_SECURITY_SW_SUPPORT
	u32 img_auth_required = 0;
	u32 policy_entry_idx = 0;
	u32 img_vfy_time = 0;
	u32 ret = 0;
#endif
	...
	buf = malloc(512);
	if (!buf) {
		pal_log_err("mboot_load_mntl, buf = NULL\n");
		return -ENOMEM;
	}

#ifdef MTK_SECURITY_SW_SUPPORT
	img_vfy_time = get_timer(0);
	policy_entry_idx = get_policy_entry_idx("mnb");
	img_auth_required = get_vfy_policy(policy_entry_idx);
	if (img_auth_required) {
		mtk_wdt_restart();
		ret = sec_img_auth_init(part_name, "mnb", 0);
		if (ret) {
			pal_log_err("%s cert vfy fail...(0x%x)\n", part_name, ret);
			assert(0);
		}
		pal_log_info("[SBC] %s cert vfy(%d ms)\n", part_name, (unsigned int)get_timer(img_vfy_time));
	}
#endif

	len = partition_read(part_name, 0, (uchar*)buf, 512);
	if (len != 512) {
		pal_log_err("[mboot_load_mntl] partition_read read fail %d\n", len);
		free(buf);
		return -EIO;
	}
	img_hdr = (union mkimg_hdr *)buf;
	if ((MKIMG_MAGIC == img_hdr->info.magic) && (MKIMG_EXT_MAGIC == img_hdr->info.ext_magic)) {
		offset = 512;
		img_size = img_hdr->info.dsz;
		pal_log_err("[mboot_load_mntl] new format offset %d img_size %d\n", offset, img_size);
	} else {
		pal_log_err("[mboot_load_mntl] unknown format\n");
		free(buf);
		return -EINVAL;
	}
	alimg_size = ALIGN(img_size+4, 0x10000);
	pal_log_err("[mboot_load_mntl] img_size: %d, alimg_size: %d\n"  , img_size, alimg_size);

	if (alimg_size > 1024*1024) {
		pal_log_err("[mboot_load_mntl] image size is too large %d bytes\n", img_size);
		free(buf);
		return -ENOMEM;
	}
	...
	if (!koaddr) {
		pal_log_err("[mboot_load_mntl] reserve kobuffer failed\n");
		free(buf);
		return -ENOMEM;
	}
	hdr_img_size = (int *)koaddr;
	*hdr_img_size = img_size;
	len = partition_read(part_name, offset, (uchar*)koaddr + sizeof(int), img_size);
	if (len != img_size) {
		pal_log_err("[mboot_load_mntl] partition_read read fail %d\n", len);
		free(buf);
		return -EIO;
	}
	pal_log_err("[mboot_load_mntl] Load '%s' partition to 0x%08lX (%d bytes in %ld ms)\n", part_name, (unsigned long)koaddr, img_size, get_timer(begin));
	free(buf);
	...
	if (img_auth_required) {
		mtk_wdt_restart();
		img_vfy_time = get_timer(0);
		ret = sec_img_auth((uchar*)koaddr + sizeof(int), img_size);
		if (ret) {
			pal_log_err("[SBC] img %s vfy fail...(0x%x)\n", part_name, ret);
			assert(0);
		}
