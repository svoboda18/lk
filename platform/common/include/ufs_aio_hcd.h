...
#define UTRD_NUTRS      ((UTRD_SIZE+UFS_PLATFORM_CACHE_LINE_SIZE-1) / UTRD_SIZE)
...
#define QUERY_REQ_TIMEOUT 55000 /* msec */    // original: 30
...
enum {
    CUSTOM_FORCE_PROVISION      = 0x1,
    CUSTOM_TW_GB                = 0x2,
    CUSTOM_TW_NO_RED            = 0x4,
    CUSTOM_HPB_REGION_COUNT     = 0x8,
    CUSTOM_SET_DONE             = 0x80000000
};
...
struct ufs_platform_info {
	void    *hci_base;
	void    *pericfg_base;
	void    *mphy_base;

	u32     reg_ufs_pericfg;
	u32     reg_ufs_pericfg_rst_n_bit;
	u32     reg_ufs_pericfg_ldo_n_bit;
	u32     reg_ufs_pericfg_lp_n_bit;
};
...
struct ufs_hba {
	...
	int (* read_descriptor)(struct ufs_hba * hba, enum desc_idn desc_id, int desc_index, u8 selector, u8 *buf, u32 size);
	int (* write_descriptor)(struct ufs_hba * hba, enum desc_idn desc_id, int desc_index, u8 selector, u8 *buf, u32 size);
...
#define UFS_DEVICE_QUIRK_NEED_CONFIGURATION_FOR_UNUSED_LU   (1 << 30)

#define UFS_DEVICE_QUIRK_NEED_8_BYTE_ALIGNED_BUF            (1 << 29)
 ...
 */
#define UFS_DEVICE_QUIRK_LIMITED_RPMB_MAX_RW_SIZE           (1 << 28)
...
extern int          ufs_aio_dma_read_desc(struct ufs_hba * hba, enum desc_idn desc_id, int desc_index, u8 selector, u8 *buf, u32 size);
extern int          ufs_aio_pio_read_desc(struct ufs_hba * hba, enum desc_idn desc_id, int desc_index, u8 selector, u8 * buf, u32 buf_size);

extern int          ufs_aio_dma_write_desc(struct ufs_hba *hba, enum desc_idn idn, int index, u8 selector, u8 *src_buf, u32 buf_len);
extern int          ufs_aio_pio_write_desc(struct ufs_hba *hba, enum desc_idn idn, int index, u8 selector, u8 *src_buf, u32 buf_len);
...
extern int          ufs_aio_check_lu_cfg(struct ufs_hba *hba);
...
#if defined(MTK_UFS_DRV_DA)
extern int          ufs_aio_get_device_buf_alignment_req(struct ufs_hba *hba);
extern void         ufshcd_irq_handler(unsigned int irq);
#endif

#if defined(MTK_UFS_DRV_PRELOADER)
extern int          ufs_aio_get_boot_lu(struct ufs_hba *hba, u32 *b_boot_lun);
extern u64          ufs_aio_rpmb_get_lu_size(void);
extern int          ufs_aio_rpmb_get_rw_size(void);
#endif
