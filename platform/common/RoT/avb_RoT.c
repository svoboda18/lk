...
void send_root_of_trust_info(void)
{
	...
	smc_ret |= smc_call(MTK_SIP_LK_ROOT_OF_TRUST_AARCH32, *(p_hash), *(p_hash + 1),
			    *(p_hash + 2), *(p_hash + 3));
	smc_ret |= smc_call(MTK_SIP_LK_ROOT_OF_TRUST_AARCH32, *(p_hash + 4),
			    *(p_hash + 5), *(p_hash + 6), *(p_hash + 7));
	smc_ret |= smc_call(MTK_SIP_LK_ROOT_OF_TRUST_AARCH32, device_lock_state,
			    g_boot_state, g_vb_version.os_version, g_vb_version.os_patchlevel);
	smc_ret |= smc_call(MTK_SIP_LK_ROOT_OF_TRUST_AARCH32, *(p_vbmeta_digest),
			    *(p_vbmeta_digest + 1), *(p_vbmeta_digest + 2), *(p_vbmeta_digest + 3));
	smc_ret |= smc_call(MTK_SIP_LK_ROOT_OF_TRUST_AARCH32, *(p_vbmeta_digest + 4),
