...
void ufs_aio_scsi_cmd_write_buffer(struct ufs_aio_scsi_cmd *cmd, u32 tag, unsigned long *buf, u32 size)
{
	memset(cmd->cmd_data, 0, MAX_CDB_SIZE);
...
int ufs_aio_dma_read(struct ufs_hba * hba, u32 lun, u32 blk_start, u32 blk_cnt, unsigned long * buf)
{
	...
	UFS_DBG_LOGV("[UFS] info: ufs_aio_dma_read,L:%d,S:%d,C:%d,buf:%p\n", lun, blk_start, blk_cnt, buf);
...
int ufs_aio_dma_write(struct ufs_hba * hba, u32 lun, u32 blk_start, u32 blk_cnt, unsigned long * buf)
{
	...
	UFS_DBG_LOGV("[UFS] info: ufs_aio_dma_write,L:%d,S:%d,C:%d,buf:%p\n", lun, blk_start, blk_cnt, buf);
...
int ufs_aio_dma_ffu(struct ufs_hba * hba, unsigned long * buf, u32 size)
{
	struct ufs_aio_scsi_cmd cmd;
	int tag;
	int ret;

	UFS_DBG_LOGV("[UFS] info: ufs_aio_dma_ffu,buf:%p,size:%d\n", buf, size);

	if (!ufshcd_get_free_tag(hba, &tag))
		return -1;

	ufs_aio_scsi_cmd_write_buffer(&cmd, tag, buf, size);
	ret = ufshcd_queuecommand(hba, &cmd);
...
int ufs_aio_pio_write(struct ufs_hba * hba, u32 lun, u32 blk_start, u32 blk_cnt, unsigned long * buf)
{
	...
	if (!pBufOut) {
		UFS_DBG_LOGD("%s #%d: pBufOut is null.\n", __FUNCTION__, __LINE__);
...
int ufs_aio_otp_lock_req(char *otp_part_name)
{
	...
	ret = ufs_aio_block_read(UFS_DEFAULT_HOST_ID, UFS_LU_2,
