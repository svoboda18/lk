...
/* alignment handler
 * MSDC/UFS HW dma needs to be 4 byte alignment
 * MSDC/UFS PIO mode needs to be 4 byte align */
#define DMA_BUF_ALIGN_SIZE   4
...
unsigned long mt_part_get_blksz(void)
{
	part_dev_t *pdev = mt_part_get_device();
	if (pdev)
		return pdev->blkdev->blksz;
	else
		return 0;
}

unsigned long mt_part_get_erasesz(void)
{
	part_dev_t *pdev = mt_part_get_device();
	if (pdev)
		return pdev->blkdev->erasesz;
	else
		return 0;
}
