...
static int mt_part_generic_read(part_dev_t *dev, u64 src, uchar *dst, int size)
{
	int dev_id = dev->id;
	uchar *buf;
	...
	if (blkcnt > 0) {
		if (dst_start) {
			...
			while (blkcnt >= blkcnt_buf) {
				if ((blkdev->block_read(dev_id, blknr, blkcnt_buf, (unsigned long *)buf)) != blkcnt_buf) {
					mutex_release(&g_part_lock);
					if (buf)
						free(buf);
					return -EIO;
			...
			if (blkcnt > 0) {
				if ((blkdev->block_read(dev_id, blknr, blkcnt, (unsigned long *)buf)) != blkcnt) {
					mutex_release(&g_part_lock);
					if (buf)
						free(buf);
					  return -EIO;
	...
	if (blkcnt > 0) {
		if (src_start) {
			...
			while (blkcnt >= blkcnt_buf) {
				...
				if ((blkdev->block_write(dev_id, blknr, blkcnt_buf, (unsigned long *)buf)) != blkcnt_buf) {
					mutex_release(&g_part_lock);
					if (buf)
						free(buf);
					return -EIO;
			...
			if (blkcnt > 0) {
				...
				if ((blkdev->block_write(dev_id, blknr, blkcnt, (unsigned long *)buf)) != blkcnt) {
					mutex_release(&g_part_lock);
					if (buf)
						free(buf);
						return -EIO;
...
static int mt_part_generic_erase(int dev_num,u64 start_addr,u64 len)
{
	return -1;
}
