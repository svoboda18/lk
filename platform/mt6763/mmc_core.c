...
void mmc_set_clock(struct mmc_host *host, int ddr, u32 hz)
{
	...
	if (host->card) {
		if (mmc_card_hs400(host->card)) {
			...
			ddr = 0;
		} else if (mmc_card_ddr(host->card) && (host->caps & MMC_CAP_DDR)) {
			ddr = 1;
		} else {
			ddr = 0;
		}
                                   ...
                                   Region partition, unsigned long blknr,u32 blkcnt, STORAGE_WP_TYPE type)
{
	...
	if (type == WP_PERMANENT || type == WP_DISABLE || type == WP_TEMPORARY) {
		msdc_pr_err("[%s] Not-supported WP type (%d)\n", __func__, type);
		return 0;
	}
	if (type > WP_DISABLE || type < 0)  {
		msdc_pr_err("[%s] Unknow WP type (%d)\n", __func__, type);
		return err;
	}
	//check partition
	if (partition == EMMC_PART_UNKNOWN || partition == EMMC_PART_BOOT2
		...
		msdc_pr_err("[%s] Not-supported WP partition (%d)\n", __func__, partition);
		return 0;
	}

	if (partition >= EMMC_PART_END || partition < 0) {
		msdc_pr_err("[%s] Unknow WP partition (%d)\n", __func__, partition);
		return err;
	}
...
//	struct mmc_host *host = mmc_get_host(dev_num);
	...
	if (type == WP_POWER_ON && partition == EMMC_PART_USER) {
		...
		if ((blknr % card->wp_size) != 0) {
			msdc_pr_err("[%s] wp start addr (0x%lx) is not wp_size (0x%x) aligned\n",
				__func__, blknr, card->wp_size);
			return err;
		}

		if ((blkcnt % card->wp_size) != 0) {
			msdc_pr_err("[%s] wp blk count (0x%x) is not wp_size (0x%x) aligned\n",
				__func__, blkcnt, card->wp_size);
			return err;
		}

		if (blknr + blkcnt > card->nblks) {
			msdc_pr_err("[%s] wp end addr (0x%lx) exceed device capacity (0x%x) aligned\n",
				__func__, blknr + blkcnt, card->nblks);
			return err;
		}
...
#if defined(FEATURE_MMC_RD_TUNING) || defined(__MTK_SLT_)
	int retry = 1;
#else
	int retry = 3;
#endif
