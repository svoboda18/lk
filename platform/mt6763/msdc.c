...
void msdc_enable_dcm(struct mmc_host *host, int on, u8 div1, u8 div2)
{
	...
	if (on) {
		/* Attension: write 0 is enable */
		MSDC_SET_FIELD(MSDC_PATCH_BIT1, MSDC_PB1_DCM_EN, 0);

		/* Not supported on Whitney */
		/*
		if (host->id == 0) {
...
void msdc_set_startbit(struct mmc_host *host, u8 start_bit)
{
	...
	switch (l_start_bit) {
		case 0:
			MSG(INF, "read data start bit at rising edge\n");
			break;
		case 1:
			MSG(INF, "read data start bit at falling edge\n");
			break;
		case 2:
			MSG(INF, "read data start bit at rising & falling edge\n");
			break;
		case 3:
			MSG(INF, "read data start bit at rising | falling edge\n");
			break;
	...
	switch (type) {
		case TYPE_CMD_RESP_EDGE:
			...
			if (mode == MSDC_SMPL_RISING || mode == MSDC_SMPL_FALLING) {
				...
				msdc_pr_err("[%s]: SD%d invalid resp parameter: HS400=%d, type=%d, mode=%d\n", __func__, host->id, HS400, type, mode);
			}
			break;
		...
		case TYPE_WRITE_CRC_EDGE:
			...
			if (priv->tuning_mode == MSDC_PATH_USE_ASYNC_FIFO) {
				if (mode == MSDC_SMPL_RISING || mode == MSDC_SMPL_FALLING) {
					...
					msdc_pr_err("[%s]: SD%d invalid read parameter: HS400=%d, type=%d, mode=%d\n", __func__, host->id, HS400, type, mode);
				}
				...
				if (mode == MSDC_SMPL_RISING || mode == MSDC_SMPL_FALLING) {
					...
					msdc_pr_err("[%s]: SD%d invalid crc parameter: HS400=%d, type=%d, mode=%d\n", __func__, host->id, HS400, type, mode);
				}
			}
			break;
		...
		case TYPE_WRITE_DATA_EDGE:
			...
			if (mode == MSDC_SMPL_RISING|| mode == MSDC_SMPL_FALLING) {
				...
				msdc_pr_err("[%s]: SD%d invalid write parameter: HS400=%d, type=%d, mode=%d\n", __func__, host->id, HS400, type, mode);
			}
			break;

		default:
			msdc_pr_err("[%s]: SD%d invalid parameter: HS400=%d, type=%d, mode=%d\n", __func__, host->id, HS400, type, mode);
			break;
	}
}
...
int msdc_cmd(struct mmc_host *host, struct mmc_command *cmd)
{
	...
	if (err == MMC_ERR_BADCRC) {
		...
		if ( host->app_cmd!=2 ) { //Light 20121225, to prevent recursive call path: msdc_tune_cmdrsp->msdc_app_cmd->msdc_cmd->msdc_tune_cmdrsp
			...
			if (err != MMC_ERR_NONE) {
				msdc_pr_err("[Err handle][%s:%d]tune cmd fail\n", __func__, __LINE__);
			}
		}
...
int msdc_abort_handler(struct mmc_host *host, int abort_card)
{
	...
	while (state != 4) { // until status to "tran"; //20130125 Comment out by Light
		...
		//#if defined(MMC_MSDC_DRV_CTP)
		if (err == MMC_ERR_BADCRC) {
			msdc_pr_err("[Err handle][%s:%d]cmd13 crc error\n", __func__, __LINE__);
		...
		if (err == MMC_ERR_TIMEOUT) {
			msdc_pr_err("[Err handle][%s:%d]cmd13 timeout\n", __func__, __LINE__);
			msdc_tune_update_cmdrsp(host, count++);
		...
		#else
		if (err != MMC_ERR_NONE) {
			msdc_pr_err("[Err handle][%s:%d]cmd13 fail\n", __func__, __LINE__);
			goto out;
		}
		#endif
		...
		if (state == 5 || state == 6) {
			if (abort_card) {
				...
				//#if defined(MMC_MSDC_DRV_CTP)
				if (err == MMC_ERR_BADCRC) {
					msdc_pr_err("[Err handle][%s:%d]cmd12 crc error\n", __func__, __LINE__);
					msdc_tune_update_cmdrsp(host, count++);
				...
				if (err == MMC_ERR_TIMEOUT) {
					msdc_pr_err("[Err handle][%s:%d]cmd12 timeout\n", __func__, __LINE__);
					msdc_tune_update_cmdrsp(host, count++);
				...
				#else
				if (err != MMC_ERR_NONE) {
					msdc_pr_err("[Err handle][%s:%d]cmd12 fail\n", __func__, __LINE__);
					goto out;
				}
				#endif
			}
...
void msdc_reset_timing_register(struct mmc_host *host)
{
	...
	MSDC_WRITE32(MSDC_PATCH_BIT0, 0x403C0007);
...
void msdc_init_tune_path(struct mmc_host *host, unsigned int path, int hs400)
{
	...
	if (path==MSDC_PATH_USE_ASYNC_FIFO) {
		...
		MSDC_SET_FIELD(MSDC_PATCH_BIT1, MSDC_PB1_GET_BUSY_MA, 1);

		#if !defined(FPGA_PLATFORM)
		if (hs400) {
			MSDC_SET_FIELD(MSDC_PATCH_BIT2, MSDC_PB2_CRCSTSENSEL, 3);
			MSDC_SET_FIELD(MSDC_PATCH_BIT2, MSDC_PB2_RESPSTENSEL, 3);
		} else {
			MSDC_SET_FIELD(MSDC_PATCH_BIT2, MSDC_PB2_CRCSTSENSEL, 1);
			MSDC_SET_FIELD(MSDC_PATCH_BIT2, MSDC_PB2_RESPSTENSEL, 1);
		}
		#endif
...
int msdc_init(int id, struct mmc_host *host, int clksrc, int mode)
{
	...
	if ((id >= MSDC_MAX_NUM) || (id < 0)) {
		msdc_pr_err("[SD%d] Host index error\n", id);
		return -1;
	}

	base = baddr[id];
	top_base = top_baddr[id];
