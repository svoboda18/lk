...
typedef struct selection_s {
	unsigned int id;
	unsigned int id_bit_map[BIT_NUM];
...
unsigned int module_list_scenario[DDP_SCENARIO_MAX][DDP_ENING_NUM] = {
	/*PRIMARY_DISP */
	{
		DISP_MODULE_OVL0, DISP_MODULE_OVL0_2L, DISP_MODULE_RDMA0, DISP_MODULE_COLOR0,
...
static mout_t mout_map[DDP_MOUT_NUM] = {
	...
	{DISP_MODULE_OVL0,
		...
		{DISP_MODULE_OVL0_2L, 1 << 4}, {DISP_MODULE_NONE, 0} },
		0, 0},/* bit1 mdp_wrot, bit3 mdp_rsz */

	/* OVL0_2L_MOUT */
	{DISP_MODULE_OVL0_2L,
		{{DISP_MODULE_RDMA0, 1 << 0}, {DISP_MODULE_WDMA_VIRTUAL0, 1 << 2},
		{DISP_MODULE_OVL1_2L, 1 << 4}, {DISP_MODULE_NONE, 0} },
		0, 0},

	/* OVL1_2L_MOUT */
	{DISP_MODULE_OVL1_2L,
		{{DISP_MODULE_RDMA0, 1 << 0}, {DISP_MODULE_WDMA_VIRTUAL0, 1 << 2},
		{DISP_MODULE_RDMA1, 1 << 4}, {DISP_MODULE_NONE, 0} },
		0, 0},
...
static sel_t sel_in_map[DDP_SEL_IN_NUM] = {
	/* COLOR_SEL */
	{DISP_MODULE_CCORR0, {DISP_MODULE_COLOR0, DISP_MODULE_RDMA0, DISP_MODULE_NONE}, 0, 0},

	/* RDMA_SEL */
	{DISP_MODULE_RDMA0, {DISP_MODULE_OVL0, DISP_MODULE_OVL0_2L,
		DISP_MODULE_OVL1_2L, DISP_MODULE_NONE}, 0, 0},

	/* WDMA_SEL */
	{DISP_MODULE_WDMA_VIRTUAL1, {DISP_MODULE_WDMA_VIRTUAL0,
								DISP_MODULE_DITHER0, DISP_MODULE_NONE}, 0, 0},
	...
	{DISP_MODULE_WDMA0, {DISP_MODULE_NUM, DISP_MODULE_NUM,
						 DISP_MODULE_NUM, DISP_MODULE_WDMA_VIRTUAL1, DISP_MODULE_NONE}, 0, 0},
	/* DSI_SEL */
	{DISP_MODULE_DSI0, {DISP_MODULE_DITHER0, DISP_MODULE_RDMA0,
		DISP_MODULE_NUM, DISP_MODULE_RDMA1, DISP_MODULE_NONE}, 0, 0},

	/* DPI0_SEL */
	{DISP_MODULE_DPI, {DISP_MODULE_NUM, DISP_MODULE_RDMA0,
		DISP_MODULE_RDMA1, DISP_MODULE_DITHER0, DISP_MODULE_NONE}, 0, 0},
};
...
static unsigned int ddp_get_module_num_l(unsigned int *module_list)
{
	unsigned int num = 0;
	while (*(module_list + num) != DISP_MODULE_NONE) {
		num++;
...
static void ddp_connect_path_l(unsigned int *module_list, void *handle)
{
	int i, j, k;
	int step = 0;
	unsigned int mout = 0;
	...
	for (i = 0; i < module_num - 1; i++) {
		for (j = 0; j < DDP_MOUT_NUM; j++) {
			if (module_list[i] == mout_map[j].id) {
				...
				for (k = 0; k < 5; k++) {
					if (mout_map[j].out_id_bit_map[k].m == DISP_MODULE_NONE)
						break;
	...
	for (i = 0; i < module_num - 1; i++) {
		for (j = 0; j < DDP_SEL_OUT_NUM; j++) {
			if (module_list[i] == sel_out_map[j].id) {
				...
				ASSERT(step < module_num);
				for (k = 0; k < 4; k++) {
					if (sel_out_map[j].id_bit_map[k] == DISP_MODULE_NONE)
						break;
	...
	for (i = 1; i < module_num; i++) {
		for (j = 0; j < DDP_SEL_IN_NUM; j++) {
			if (module_list[i] == sel_in_map[j].id) {
				...
				ASSERT(step >= 0);
				for (k = 0; k < 4; k++) {
					if (sel_in_map[j].id_bit_map[k] == DISP_MODULE_NONE)
						break;
...
static void ddp_check_path_l(unsigned int *module_list)
{
	int i, j;
	unsigned int k;
	int step = 0;
...
static void ddp_disconnect_path_l(unsigned int *module_list, void *handle)
{
	int i, j, k;
	int step = 0;
	unsigned int mout = 0;
...
/* id: mutex ID, 0~5 */
static int ddp_mutex_set_l(int mutex_id, unsigned int *module_list, DDP_MODE ddp_mode, void *handle)
{
	int i = 0;
...
static void ddp_check_mutex_l(int mutex_id, unsigned int *module_list, DDP_MODE ddp_mode)
{
	int i = 0;
		      ...
		      DISP_MODULE_ENUM module)
{
	...
	ASSERT(module_list_scenario[ddp_scenario][DDP_ENING_NUM - 1] == DISP_MODULE_NONE);
	...
	{
		unsigned int *modules = ddp_get_scenario_list(ddp_scenario);
