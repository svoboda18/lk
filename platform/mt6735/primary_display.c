...
static void _build_cmdq_trigger_loop(void)
{
	...
	if (primary_display_is_video_mode()) {
		...
		//xuecheng, no internal te on fpga now.
...
void _dump_lcm_info_by_video_printf(void)
{
	...
	if (l && p) {
		...
		if (p->type == LCM_TYPE_DSI) {
			video_printf("[LCM] LANE_NUM: %d,data_format:(%d,%d,%d,%d)\n",
				p->dsi.LANE_NUM,
				p->dsi.data_format.color_order,
				p->dsi.data_format.format,
				p->dsi.data_format.padding,
				p->dsi.data_format.trans_seq);
#ifdef ROME_TODO
#error
#endif
			video_printf("[LCM] vact:%d, vbp:%d, vfp:%d, vact_line:%d, hact:%d, hbp:%d, hfp:%d, hblank:%d\n",
			...
			video_printf("[LCM] pll_select:%d, pll_div1:%d, pll_div2:%d, fbk_div:%d ,fbk_sel:%d, rg_bir:%d\n",
				p->dsi.pll_select, p->dsi.pll_div1, p->dsi.pll_div2,
				p->dsi.fbk_div,	p->dsi.fbk_sel, p->dsi.rg_bir);
			video_printf("[LCM] rg_bic:%d, rg_bp:%d, PLL_CLOCK:%d, dsi_clock:%d\n",
				p->dsi.rg_bic, p->dsi.rg_bp, p->dsi.PLL_CLOCK, p->dsi.dsi_clock);
			video_printf("[LCM] ssc_range:%d, ssc_disable:%d, compatibility_for_nvk:%d, cont_clock:%d\n",
				p->dsi.ssc_range, p->dsi.ssc_disable, p->dsi.compatibility_for_nvk, p->dsi.cont_clock);
...
int primary_display_init(char *lcm_name)
{
	...
	if ((0 == mt_disp_get_dfo_setting("LCM_FAKE_WIDTH", &lcm_fake_width)) && (0 == mt_disp_get_dfo_setting("LCM_FAKE_HEIGHT", &lcm_fake_height))) {
		...
		if (0 != primary_display_change_lcm_resolution(lcm_fake_width, lcm_fake_height)) {
...
{
	// TODO: we should store the video/cmd mode in runtime, because  will support cmd/vdo dynamic switch
...
int primary_display_get_vsync_interval(void)
{
	...
	/* use interrupt to detect lcm connection */
...
// TODO: could we use the same header file between lk/kernel to avoid definition non-match issue?
#define DAL_BPP                 (2)
