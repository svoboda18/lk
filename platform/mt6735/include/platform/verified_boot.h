...
#define SBOOT_PART_BOOT_NAME     "boot"
#define SBOOT_PART_RECOVERY_NAME "recovery"
#define SBOOT_PART_DTBO_NAME     "odmdtbo"
#define SBOOT_PART_LOGO_NAME     "logo"
...
#define PERSIST_VALUE_OFFSET PERSIST_UNSPECIFIED_OFFSET

/* secure boot flow */
int sec_func_init(unsigned long long pl_start_addr);
int seclib_set_oemkey(unsigned char *pubk, unsigned int size);
unsigned int seclib_image_check(char *img_name, unsigned char msg_en);
int sec_boot_check(int try_lock);
int sec_logo_check(void);
int sec_dtbo_check(void);
int sec_usbdl_verify_da(unsigned char*, unsigned int, unsigned char*, unsigned int);
int android_verified_boot(char *part_name, char *target);
