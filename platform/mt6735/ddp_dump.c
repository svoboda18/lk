...
static void  mmsys_config_dump_analysis(void)
{
	...
	for (i = 0; i < 32; i++) {
		pos = clock_on;
		if ((valid0 & (1 << i)) == 0) {
			len = snprintf(pos, sizeof(clock_on), "%s:-",ddp_signal_0(i));
		} else {
			len = snprintf(pos, sizeof(clock_on),"%s:Valid,",ddp_signal_0(i));
		}
		pos+=len;
		if ((ready0 & (1 << i)) == 0) {
			len = snprintf(pos, sizeof(clock_on), "-");
		} else {
			len = snprintf(pos, sizeof(clock_on), "Ready");
		}
		DDPMSG("%s\n",clock_on);
	}
	if (0) { //for (i = 0; i < 24; i++)
		pos = clock_on;
		if ((valid1 & (1 << i)) == 0) {
			len = snprintf(pos, sizeof(clock_on), "%-26s:Not Valid,",ddp_signal_1(i));
		} else {
			len = snprintf(pos, sizeof(clock_on), "%-26s:    Valid,",ddp_signal_1(i));
		}
		pos+=len;
		if ((ready1 & (1 << i)) == 0) {
			len = snprintf(pos, sizeof(clock_on), "Not Ready");
		} else {
			len = snprintf(pos, sizeof(clock_on), "    Ready");
		}
		DDPMSG("%s\n",clock_on);
...
static void aal_dump_analysis(void)
{
	...
	DDPMSG("aal: bypass=%d, relay=%d, en=%d, w=%d, h=%d, in(%d,%d), out(%d,%d)\n",
