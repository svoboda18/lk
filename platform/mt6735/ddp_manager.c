...
int dpmgr_destroy_path(disp_path_handle dp_handle)
{
	...
	if (handle != NULL) {
		...
		DISP_LOG_I("destroy path handle, dp_handle is NULL\n");
	}
	return 0;
}
...
int dpmgr_path_remove_dump(disp_path_handle dp_handle, int encmdq )
{
	...
	ddp_disconnect_path(DDP_SCENARIO_PRIMARY_OVL_MEMOUT,cmdqHandle);
...
int dpmgr_path_config(disp_path_handle dp_handle, disp_ddp_path_config * config, int encmdq)
{
	...
	int module_num = ddp_get_module_num(handle->scenario);
	cmdqRecHandle cmdqHandle = encmdq ? handle->cmdqhandle : NULL;
	DISP_LOG_I("path config ovl %d, rdma %d, wdma %d, dst %d on handle 0x%x scenario %s \n",
	           config->ovl_dirty,
	           config->rdma_dirty,
	           config->wdma_dirty,
	           config->dst_dirty,
...
int dpmgr_init(void)
{
	...
	//return disp_register_irq_callback(dpmgr_irq_handler);
	return 0;
}
