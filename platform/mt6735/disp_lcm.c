...
void _dump_lcm_info(disp_lcm_handle *plcm)
{
	...
	if (l && p) {
		...
		if (p->type == LCM_TYPE_DSI) {
			DISPCHECK("[LCM] LANE_NUM: %d,format: %d,vertical_sync_active: %d\n",
			          p->dsi.LANE_NUM, p->dsi.data_format.format,  p->dsi.vertical_sync_active);
#ifdef ROME_TODO
#error
#endif
			DISPCHECK
			("[LCM] vact: %d, vbp: %d, vfp: %d, vact_line: %d, hact: %d, hbp: %d, hfp: %d, hblank: %d\n",
...
int disp_lcm_init(disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (LCM_TYPE_DSI == plcm->params->type) {
			...
			DSI_set_cmdq(_get_dst_module_by_lcm(plcm), NULL, data_array, 1, 1);   /* read maximum return size. */
