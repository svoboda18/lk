...
static void fastboot_boot_menu(void)
{
	video_clean_screen();
	video_set_cursor(video_get_rows()/2, 0);
	video_printf("Select Boot Mode:\n[VOLUME_UP to select.  VOLUME_DOWN is OK.]\n\n");
...
static int fastboot_data_part_wipe()
{
	...
	if (err != PART_OK) {
		ret = set_env("unlock_erase", "fail");
		if (ret)
			dprintf(CRITICAL,"unlock_erase fail\n");
		ret = -1;
	} else {
...
static int fastboot_oem_unlock_chk(void)
{
	int ret = B_OK;
...
static int fastboot_oem_lock_chk()
{
#define TRY_LOCK 1
	int ret = B_OK;
	int inFactory = 0;
...
void fastboot_oem_key(const char *arg, void *data, unsigned sz)
{
	...
	if (key_length != UNLOCK_KEY_SIZE) {
		...
		strncpy(fb_unlock_key_str, arg + 1, sizeof(fb_unlock_key_str));
...
void fastboot_oem_unlock(const char *arg, void *data, unsigned sz)
{
	int ret = B_OK;
	char msg[128] = {0};
	unsigned int unlock_allowed = 0;
...
void fastboot_oem_lock(const char *arg, void *data, unsigned sz)
{
	int ret = B_OK;
	char msg[128] = {0};
	int inFactory = 0;
