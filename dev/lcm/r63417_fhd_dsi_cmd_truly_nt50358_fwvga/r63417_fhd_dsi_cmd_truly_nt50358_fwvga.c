...
#define dsi_set_null(cmd, count, ppara, force_update)   lcm_util.dsi_set_null(cmd, count, ppara, force_update)
...
static void lcm_init(void)
{
	...
	if(ret)
		dprintf(CRITICAL, "[LK]r63417----tps65132----cmd=%0x--i2c write error----\n",cmd);
	else
		dprintf(CRITICAL, "[LK]r63417----tps65132----cmd=%0x--i2c write success----\n",cmd);
#else
...
{
	push_table(lcm_suspend_setting, sizeof(lcm_suspend_setting) / sizeof(lcm_suspend_setting[0]), 1);
	//SET_RESET_PIN(0);
	MDELAY(10);
	mt_set_gpio_mode(GPIO_65132_EN, GPIO_MODE_00);
	mt_set_gpio_dir(GPIO_65132_EN, GPIO_DIR_OUT);
...
#ifdef BUILD_LK
	dprintf(CRITICAL, "%s, LK r63417 debug: r63417 id = 0x%08x\n", __func__, id);
#else
	printk("%s, kernel r63417 horse debug: r63417 id = 0x%08x\n", __func__, id);
#endif
...
{
#ifdef BUILD_LK
	dprintf(CRITICAL,"%s,lk r63417 backlight: level = %d\n", __func__, level);
#else
	printk("%s, kernel r63417 backlight: level = %d\n", __func__, level);
#endif
