...
#ifdef BUILD_LK
	dprintf(INFO, "=====>compare id for test %s, id = 0x%08x,buffer[1] = %x,buffer[2] = %x,buffer[3] = %x\n",
...
static unsigned int lcm_esd_check(void)
{
	...
	if((buffer[0] == 0x80) && (buffer[1] == 0x73) && (buffer[2] == 0x4) &&
		(buffer1[5] == 0x11) && (buffer1[6] == 0x11))
		return FALSE;
	else
		return TRUE;
 #endif
