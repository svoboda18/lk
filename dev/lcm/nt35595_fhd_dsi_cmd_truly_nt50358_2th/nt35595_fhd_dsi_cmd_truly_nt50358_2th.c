...
#define GPIO_LCM1_RST_PIN GPIO_LCM_RST2_PIN	/* dws */
#endif
#endif
...
bool lcm1_rst_pin_is_config = false;

#if (MTK_DUAL_DISPLAY_SUPPORT == 2)
static void lcm1_set_reset_pin(UINT32 value)
{
	if (!lcm1_rst_pin_is_config) {
		mt_set_gpio_mode(GPIO_LCM1_RST_PIN, GPIO_MODE_00);
		mt_set_gpio_dir(GPIO_LCM1_RST_PIN, GPIO_DIR_OUT);
		lcm1_rst_pin_is_config = true;
	}

	if (value == 1)
		mt_set_gpio_out(GPIO_LCM1_RST_PIN, GPIO_OUT_ONE);
	else if (value == 0)
		mt_set_gpio_out(GPIO_LCM1_RST_PIN, GPIO_OUT_ZERO);
}
#endif
...
#if (MTK_DUAL_DISPLAY_SUPPORT == 2)
	lcm1_set_reset_pin(1);
	MDELAY(1);
	lcm1_set_reset_pin(0);
	MDELAY(10);
