...
void mt_irq_set_mask(unsigned int irq)
{
	...
	DRV_WriteReg32(INT_CFG_INDIRECT_ACCESS, value);
...
void mt_irq_set_mask(unsigned int irq)
{
	...
	value = DRV_Reg32(INT_MSK_CTL0 + (reg_index * 4));
	value |= (0x1 << offset);
	DRV_WriteReg32(INT_MSK_CTL0 + (reg_index * 4), value);
...
void mt_irq_set_polarity(unsigned int irq, unsigned int polarity)
{
	...
	/* unmask irq for gic600 */
	if ((result >> GICD_V3_IIDR_PROD_ID) == GICD_V3_IIDR_GIC600)
